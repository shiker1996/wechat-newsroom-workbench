...
| 成稿与排版 | 按锁定简报执行规划、类型化初稿、去 AI、审稿、SEO 与 2000 字门禁，保留全部阶段产物；配图工作台管理来源与版权状态，按用户明确操作上传 CDN，并真实调用预渲染、HTML 规范化和无 `div` 门禁技能脚本；AI 非视觉生成 900×383 公众号封面图（AI 只做排版决策，确定性模板渲染保证出图） |
