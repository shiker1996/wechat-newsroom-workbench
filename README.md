...
| 链路 | 覆盖能力 |
|---|---|
| 采集与研判 | 日常 / 突发批次、阶段状态与中断恢复；Reddit、RSSHub 与 GitHub 项目发现（Trending、增长搜索与 AI 兴趣发现三通道，AI 按账号内容支柱生成查询组并做相关性打分过滤），每个来源单独记录状态、耗时、条数与错误；热点历史检索；热点全景（事件指纹语义聚簇、加权词云、媒体覆盖排名、国内 / 全球 / 国外分栏与多源筛选） |
| 选题与编辑 | 文章池与图文池独立评分、排名、去重和历史覆盖提示；编辑室 AI 流式回应且一次只追问一个关键问题，将明确回答沉淀为结构化决策，执行 WRITE_NOW / 实践 / 未决问题门禁；回答中粘贴的链接自动抓取入库（`candidate_sources`），以「用户补充来源」注入事实基座 |
| 成稿与排版 | 按锁定简报执行规划、类型化初稿、去 AI、审稿、SEO 与统一字数门禁（默认 1300–2000 可见字符，配置入口 `articleLength`），保留全部阶段产物；配图工作台管理来源与版权状态，按用户明确操作上传 CDN，并真实调用预渲染、HTML 规范化和无 `div` 门禁技能脚本；AI 非视觉生成 900×383 公众号封面图（AI 只做排版决策，确定性模板渲染保证出图） |
...
7. 在“文章封面图”页选择主题（10 套内置封面主题，或 AI 按文章调性自动匹配 / 在主题中心用 AI 生成封面主题）后点击“生成封面图”。AI 只做排版决策（断行、关键词高亮、组件组合），产出规格经校验后由确定性模板渲染为 900×383 PNG；规格任一不合规整体回退安全版式，保证永远出图。产物保存为 `images/cover.png`，可预览和下载，重复生成覆盖旧图。
...
默认路由包括晚点、TechCrunch、虎嗅、Solidot、ReadHub、界面、Anthropic、36Kr 热榜，以及 GitHub Trending 的日/周/月榜；还会按配置执行 GitHub 新项目发现（含可选的 AI 兴趣仓库发现通道：LLM 按账号内容支柱生成 Search 查询组，再对结果做兴趣相关性打分过滤，字段见 [docs/configuration.md](./docs/configuration.md) 的 `githubDiscovery.aiQueries`）。实际路由以 `config.local.json` 覆盖后的运行配置为准。
