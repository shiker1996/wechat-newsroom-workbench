import { requestGitHubJson } from '../lib/integrations/github-api.mjs';
...
export async function discoverGitHubRepositories(items,config={},onProgress=()=>{},onSourceResult=()=>{}){
  ...
  const createdAfter=new Date(Date.now()-Math.max(1,Number(config.createdWithinDays||30))*86400000).toISOString().slice(0,10);const minStars=Math.max(1,Number(config.minStars||1000));const limit=Math.max(1,Math.min(100,Number(config.limit||30)));const query=`stars:>=${minStars} created:>=${createdAfter} fork:false archived:false`;const startedAt=new Date().toISOString();const started=Date.now();
