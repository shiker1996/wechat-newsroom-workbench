...
输出契约为 `social_card_storyboard`。每个页面必须同时返回明确的 `kind` 和 `role`，每个内容块必须带 `source_refs`；承载核心机制、架构数字、性能对比、技术路径或事实边界的内容块必须输出语义化 `visual` 意图（重点程度、语义色调、受控图标和短徽章），普通背景块可省略；不得输出字号、颜色值、HTML 或 CSS。只输出严格 JSON，不输出发布文案、HTML、CSS 或截图指令。封面最多 1 个核心内容块，结尾页最多 2 个内容块；整组优先控制在 4–6 页，相邻职责应优先合并，不能因为职责数量机械一页一责。
