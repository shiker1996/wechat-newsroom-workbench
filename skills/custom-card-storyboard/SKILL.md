...
输出契约为 `social_card_storyboard`。教程使用分步结构，清单使用筛选标准与条目结构，观点使用论点、证据与反方边界结构。承载关键事实、数字、主体、对比、边界或操作路径的内容块必须输出语义化 `visual` 意图（重点程度、语义色调、受控图标和短徽章），普通背景块可省略；不得输出字号、颜色值、HTML 或 CSS。至少保留一页事实边界。
