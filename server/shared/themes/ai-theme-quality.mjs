...
// 封面主题的字阶/间距/形状字段集与文章不同，相似度与差异对比使用封面专属范围
const COVER_NUMBER_RANGES={titlePx:[30,56],titleLineHeight:[1.1,1.6],eyebrowPx:[14,24],subtitlePx:[16,26],metaPx:[12,20],paddingXPx:[24,64],paddingYPx:[20,56],gapPx:[8,32],metaBottomPx:[12,48],badgeRadiusPx:[0,16]};
const COVER_TYPE_KEYS=['family','headingFamily','titlePx','titleLineHeight','eyebrowPx','subtitlePx','metaPx'],COVER_SHAPE_KEYS=['badgeRadiusPx'];
...
export function themeVisualSimilarity(left,right){
  ...
  const colors=average(COLOR_KEYS.filter((key)=>left.tokens?.colors?.[key]&&right.tokens?.colors?.[key]).map((key)=>colorSimilarity(left.tokens.colors[key],right.tokens.colors[key]))),leftValues=entries(left),rightValues=entries(right),ranges=target==='cover'?COVER_NUMBER_RANGES:NUMBER_RANGES;
  const numbers=average(Object.entries(ranges).map(([key,range])=>numberSimilarity(leftValues[key],rightValues[key],range)));
  const categorical=average((target==='cover'?['family','headingFamily']:['family','headingFamily','shadow']).map((key)=>leftValues[key]===rightValues[key]?1:0));
...
function differences(candidate,reference){
  ...
  const typeKeys=target==='cover'?COVER_TYPE_KEYS:['family','headingFamily','bodyPx','h1Px','h2Px','lineHeight','letterSpacingEm'],changedType=typeKeys.filter((key)=>candidate.tokens.typography?.[key]!==reference.tokens.typography?.[key]);if(changedType.length)items.push({group:'字体与字阶',summary:`${changedType.length} 项阅读参数不同`,fields:changedType.map((key)=>`tokens.typography.${key}`)});
  const shapeKeys=target==='cover'?COVER_SHAPE_KEYS:['radiusPx','borderWidthPx','shadow'],changedShape=shapeKeys.filter((key)=>candidate.tokens.shape?.[key]!==reference.tokens.shape?.[key]);if(changedShape.length)items.push({group:'形状与层次',summary:`${changedShape.length} 项质感参数不同`,fields:changedShape.map((key)=>`tokens.shape.${key}`)});
...
export function compactThemeSignatures(themes,target){return themes.filter((theme)=>targetOf(theme)===target).map((theme)=>({id:theme.id,label:theme.label,colors:target==='cover'?{page:theme.tokens.colors.page,text:theme.tokens.colors.text,muted:theme.tokens.colors.muted,accent:theme.tokens.colors.accent,accentSecondary:theme.tokens.colors.accentSecondary}:{background:theme.tokens.colors.background,surface:theme.tokens.colors.surface,text:theme.tokens.colors.text,accent:theme.tokens.colors.accent,accentSecondary:theme.tokens.colors.accentSecondary},type:target==='cover'?{family:theme.tokens.typography.family,headingFamily:theme.tokens.typography.headingFamily,titlePx:theme.tokens.typography.titlePx}:{family:theme.tokens.typography.family,headingFamily:theme.tokens.typography.headingFamily,h1Px:theme.tokens.typography.h1Px},shape:theme.tokens.shape,...(target!=='cover'&&theme[target]?{recipes:theme[target].recipes,components:target==='social'?resolveSocialComponents(theme):resolveArticleComponents(theme)}:{})}));}
