...
import { matchSocialTemplate, templateMatchMetadata } from './social-template-matcher.mjs';
...
export function validateAiThemeCandidate(candidate,{target}={}){
  ...
  // 封面主题：targetConfig 可省略；携带时只允许 {spec} 形式的主题内置构图（归一化阶段已校验）
  if(target==='cover'?candidate.targetConfig!==undefined&&!object(candidate.targetConfig):!object(candidate.targetConfig))issues.push(issue('targetConfig','TYPE',target==='cover'?'必须省略或为对象':'必须是目标配置对象'));
  if(target==='cover'&&object(candidate.targetConfig))issues.push(...unknownFields(candidate.targetConfig,new Set(['spec']),'targetConfig'));
...
export function composeAiThemeDefinition(candidate,{target,id,version='0.1.0',templateMatchContext={}}={}){
  const value=validateAiThemeCandidate(candidate,{target});
  const targetConfig=structuredClone(value.targetConfig);
  // 模板包属于系统字段：即使模型把它嵌入 targetConfig，也必须由程序重新匹配。
  if(target==='social'){
    delete targetConfig.templatePack;
    delete targetConfig.templateMatch;
  }
  const definition={
    schemaVersion:1,
    id,
    ...
    tokens:value.tokens,
    ...(target==='cover'
      ? (value.targetConfig?.spec?{cover:{spec:value.targetConfig.spec}}:{})
      : {[target]:targetConfig}),
  };
  if(target==='social'){
    const match=matchSocialTemplate({definition,preferences:templateMatchContext.preferences,designSummary:value.designSummary});
