...
import { colorContrast, mixHex } from './color-utils.mjs';
...
export function compileSocialTheme(theme){
  ...
  if(recipes.list==='hard-accent')css+=`${scope} .page li{background:var(--accent);color:var(--inverse);border-color:var(--ink);box-shadow:3px 3px 0 var(--ink)}`;
  ...
  if(recipes.code==='accent-panel')css+=`${scope} .code-block pre{background:var(--accent);color:var(--inverse);border:max(2px,var(--border-width)) solid var(--ink);box-shadow:4px 4px 0 var(--ink)}`;
  ...
  if(recipes.surface==='brutalist')css+=`${scope} .page{background:color-mix(in srgb,var(--page) 82%,var(--accent2))}${scope} .page:after{width:150px;height:150px;border:18px solid var(--accent);border-radius:0;right:-72px;top:-62px;transform:rotate(9deg)}${scope} .brand{padding:4px 7px;background:var(--ink);color:var(--inverse)}${scope} .page-number{font-size:15px;color:var(--ink)}${scope} .page-content-stack{padding-block:22px;gap:10px;border:4px solid var(--ink);box-shadow:10px 10px 0 var(--ink)}${scope} .eyebrow{width:max-content;padding:4px 7px;background:var(--accent2);color:var(--ink)}${scope} .page h1{font-weight:900}${scope} .content-block h2{text-decoration:underline;text-decoration-thickness:3px;text-decoration-color:var(--accent)}${scope} .page li{border:2px solid var(--ink);background:var(--accent2)}${scope} .code-block pre{border:3px solid var(--ink)}${scope} .note-block{border:3px solid var(--ink);border-left:10px solid var(--accent)}`;
  if(recipes.surface==='brutalist')css+=`${scope} .page li:before{content:"";background:var(--ink);border:2px solid var(--accent);box-shadow:2px 2px 0 var(--accent2);z-index:1}`;
  ...
  if(recipes.decoration==='orbit')css+=`${scope} .page:after{border-radius:50%;box-shadow:0 0 0 9px color-mix(in srgb,var(--accent2) 10%,transparent)}`;
  if(recipes.decoration==='soft-blur')css+=`${scope} .page:after{width:218px;height:218px;border:0;border-radius:50%;right:-108px;top:-94px;background:radial-gradient(circle,color-mix(in srgb,var(--accent2) 34%,transparent) 0%,color-mix(in srgb,var(--accent) 14%,transparent) 48%,transparent 72%);filter:blur(16px);opacity:var(--decoration-opacity)}`;
  ...
  if(recipes.coverTitle==='highlight-block')css+=`${scope} .page-cover h1{width:100%;max-width:100%;padding:0;background:transparent;color:var(--inverse);font-weight:850;line-height:1.04;letter-spacing:-.04em}${scope} .page-cover h1 .cover-title-line{display:block;width:fit-content;max-width:100%;margin:3px 0;padding:5px 10px;background:var(--accent);color:inherit;box-shadow:4px 0 0 var(--accent2);transform:translateX(0);overflow-wrap:anywhere}${scope} .page-cover h1 .cover-title-line:nth-child(even){margin-left:8px;background:var(--code);color:${colorVar(highlightTitleTextRole)};box-shadow:-4px 0 0 var(--accent2)}${scope} .page-cover h1 .cover-title-line:nth-child(3n){margin-left:3px}`;
  ...
  if(recipes.list==='hard-accent')css+=`${scope} .page li{color:var(--inverse)}`;
  ...
  // 组件级列表配置在主题 CSS 的末段生成，必须在其后恢复结尾页的
  // 反色语义，否则会出现 inverse 浅色文字配 page 浅色背景。
  css+=`${scope} .page-ending li{color:var(--inverse);background:rgba(255,255,255,.08);border-color:color-mix(in srgb,var(--inverse) 28%,transparent)}`;
  const usageMap={
    ...
    'social.templatePack.id':['template-pack'],'social.templatePack.version':['template-pack'],
    'social.templateMatch.schemaVersion':['template-match'],'social.templateMatch.packId':['template-match'],'social.templateMatch.source':['template-match'],'social.templateMatch.confidence':['template-match'],'social.templateMatch.reasonCode':['template-match'],'social.templateMatch.score':['template-match'],'social.templateMatch.runnerUpScore':['template-match'],'social.templateMatch.margin':['template-match'],'social.templateMatch.reason':['template-match'],'social.templateMatch.signals':['template-match'],
  ...
  const customPack=definition.social.templatePack?.roles&&definition.social.templatePack.roleTemplates?definition.social.templatePack:null;
  if(customPack){
    for(const field of ['label','renderer','proposalId','source','fallbackTemplate','css','surface','roleTemplates'])usageMap[`social.templatePack.${field}`]=['template-pack'];
    for(const [role,value] of Object.entries(customPack.roles||{}))for(const field of ['template','supportedBlocks','maxBlocks','maxItems','notes'])usageMap[`social.templatePack.roles.${role}.${field}`]=[`template-role-${role}`];
    for(const field of ['density','decoration','headingTreatment'])usageMap[`social.templatePack.surface.${field}`]=['template-surface'];
    for(const role of Object.keys(customPack.roleTemplates||{}))usageMap[`social.templatePack.roleTemplates.${role}`]=['template-role'];
  }
