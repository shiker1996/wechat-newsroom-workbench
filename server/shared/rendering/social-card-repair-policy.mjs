...
function blockDisplayText(block) {
  ...
  if (Array.isArray(block.items)) return block.items.map((item) => typeof item === 'string' ? item : JSON.stringify(item)).join('；').trim();
  if (Array.isArray(block.rows)) return block.rows.flat().map(String).join('；').trim();
...
function validateAddFactBlockOperation(pages, operation, { knownSourceRefs = [], maxFactBlocksAdded = 1, supplementSlots = null, factIndex = null, allowSemanticRenderTypes = true } = {}) {
  ...
  if (!block) issues.push(`P${pageNumber} 补充内容块缺少 block`);
    ...
    if (candidate) {
      ...
      const renderedText = blockDisplayText(block);
      const sourceText = String(display.sourceText || '').replace(/\s+/g, ' ').trim();
      const normalizedRendered = renderedText.replace(/\s+/g, ' ').trim();
      if (['pending', 'needs-localization'].includes(display.status)) {
        if (!normalizedRendered) issues.push(`P${pageNumber} 补充事实候选必须先生成 display_text，不能生成空补充块`);
        else if (normalizedRendered === sourceText) issues.push(`P${pageNumber} 不能直接使用事实候选的原始内容，需先生成 display_text`);
        else if (String(block.type || '') !== 'code' && !/[\u3400-\u9fff]/u.test(normalizedRendered)) issues.push(`P${pageNumber} 补充事实必须生成中文或技术展示文案`);
      }
    }
