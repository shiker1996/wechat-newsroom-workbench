...
const BLOCKING = new Set(['overflow','clipped','horizontal_overflow','overfilled','text_too_small','text_invisible','invalid_page_grid_structure','missing_content_stack','empty_page_body']);
...
const LABELS={underfilled:'内容不足',overfilled:'内容过多',overflow:'内容溢出',clipped:'内容被裁切',horizontal_overflow:'横向溢出',vertical_imbalance:'垂直失衡',text_too_small:'文字过小',text_invisible:'文字不可见',invalid_page_grid_structure:'页面结构异常',missing_content_stack:'页面结构异常',empty_page_body:'页面无内容'};
export function layoutAuditFailureMessage(report,maxLayoutAttempts) { const failed=(Array.isArray(report?.pages)?report.pages:[]).filter((page)=>!page.valid),details=failed.map((page)=>{const labels=(Array.isArray(page?.issues)?page.issues:[]).map((issue)=>LABELS[issue]||issue).join('、'),utilization=Number.isFinite(Number(page?.utilization))?`（版面利用率 ${page.utilization}%）`:'';return `P${page.page} ${labels}${utilization}`;}).join('；'),has=(names)=>failed.some((page)=>(Array.isArray(page?.issues)?page.issues:[]).some((issue)=>names.includes(issue))),advice=[];if(has(['underfilled']))advice.push('内容不足的页：补充内容块、增加列表条目或扩写段落');if(has(['overfilled','overflow','clipped','horizontal_overflow']))advice.push('内容放不下的页：删减、拆分或缩短文字');if(has(['text_invisible']))advice.push('文字不可见的页：检查主题文字色、色块背景和页面内容块的颜色配置');if(!advice.length)advice.push('调整问题页的内容或构图');return `布局审计 ${maxLayoutAttempts} 轮后仍未通过：${details}。自动修复（构图回退、舒展排版、AI 改写）已穷尽，请打开该候选的图文编辑器，在「02 卡片故事板」中修改对应页面后重新「生成整组图文」——${advice.join('；')}。`; }
