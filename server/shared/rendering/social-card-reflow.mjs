...
function reflowPage(page, capacity, pageIndex, { heightAdvisory = false } = {}) {
  ...
  const initialHardReasons=initial.reasons.filter((reason)=>reason!=='estimated-height');
  if (!initial.overCapacity || initial.nearFit || (heightAdvisory&&!initialHardReasons.length)) return { pages: [compacted.page], operations: compacted.operations, preflight: initial };
  ...
  const fallbackPages = pages.map((item, index) => {
    ...
    if (!estimate.overCapacity || estimate.nearFit || (heightAdvisory&&!hardReasons.length)) return item;
 ...
 */
export function mergeUnderfilledContinuationPages({ pages = [], capacityProfile = null, slack = 1.08, heightAdvisory = false } = {}) {
  const output = (Array.isArray(pages) ? pages : []).map(clone);
  const operations = [];
  ...
  while (index < output.length - 1) {
    ...
    const hardReasons = estimate.reasons.filter((reason) => reason !== 'estimated-height');
    if (hardReasons.length || (!heightAdvisory&&estimate.estimatedHeightPx > estimate.bodyHeightPx * factor)) {
      index += 1;
      continue;
    }
    output.splice(index, 2, candidate);
 ...
 */
export function balanceContinuationPages({ pages = [], capacityProfile = null, underfillThreshold = 0.62, heightAdvisory = false } = {}) {
  const output = (Array.isArray(pages) ? pages : []).map(clone);
  const operations = [];
  ...
  while (changed && guard < Math.max(1, output.length * 3)) {
    ...
    for (let index = 0; index < output.length - 1; index += 1) {
      ...
      for (const moveSecondToFirst of directions) {
        ...
        if (hardReasons.length || (!heightAdvisory&&(firstCandidateEstimate.overCapacity || secondCandidateEstimate.overCapacity))) continue;
...
export function rebalanceContinuationPages({ pages = [], capacityProfile = null, mergeSlack = 1.08, underfillThreshold = 0.62, heightAdvisory = false } = {}) {
  const balanced = balanceContinuationPages({ pages, capacityProfile, underfillThreshold, heightAdvisory });
  const packed = mergeUnderfilledContinuationPages({ pages: balanced.pages, capacityProfile, slack: mergeSlack, heightAdvisory });
  return {
    pages: packed.pages,
...
export function compileTemplateAwareCardPlan({ cardPlan = [], capacityProfile = null, maxPages = 9, absoluteMaxPages = null, mergeSlack = 1.08, heightAdvisory = false } = {}) {
  ...
  for (let index = 0; index < normalizedSource.length; index += 1) {
    ...
    const capacity = capacityProfileForRole(capacityProfile, role);
    const result = reflowPage(page, capacity, index,{heightAdvisory});
    pages.push(...result.pages);
    operations.push(...result.operations);
    preflight.push({ page: index + 1, role, ...result.preflight });
  }
  const packed = rebalanceContinuationPages({ pages, capacityProfile, mergeSlack, heightAdvisory });
