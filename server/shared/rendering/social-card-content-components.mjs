...
import { socialCardFactDisplaySpec } from './social-card-fact-index.mjs';
import {
...
function contentSizeVariants(content, renderCandidates, { summaryVariant = false, displayTextStatus = 'ready', displayBudgetChars = 56 } = {}) {
  const normal = { title: text(content?.title), text: text(content?.text), item: content?.item == null ? null : structuredClone(content.item) };
  const variants = [];
  const capacityPlaceholder = (length) => ({
    title: normal.title,
    // 仅供静态容量估算；不作为最终页面文案输出。
    text: '补充说明'.repeat(Math.ceil(Math.max(18, Number(length) || 56) / 4)).slice(0, Math.max(18, Number(length) || 56)),
    item: normal.item == null ? null : structuredClone(normal.item),
  });
  ...
  if (displayTextStatus === 'pending' || displayTextStatus === 'needs-localization') {
    // 所有来源事实先按固定展示预算估算，绝不按 source_text 的原始长度
    // 筛选。真正的 display_text 由内容计划调整器生成后再过守卫。
    variants.unshift({ id: 'slot-fit', mode: 'slot-fit', fontScale: 1, content: { ...normal }, capacityContent: capacityPlaceholder(28) });
    variants.push({ id: 'capacity-normal', mode: 'capacity', fontScale: 1, content: { ...normal }, capacityContent: capacityPlaceholder(displayBudgetChars) });
  }
  const pendingCapacity = displayTextStatus === 'pending' || displayTextStatus === 'needs-localization';
  variants.push({ id: 'normal', mode: 'normal', fontScale: 1, content: normal, ...(pendingCapacity ? { capacityContent: capacityPlaceholder(displayBudgetChars) } : {}) });
  variants.push({ id: 'typography-106', mode: 'typography', fontScale: 1.06, content: structuredClone(normal), ...(pendingCapacity ? { capacityContent: capacityPlaceholder(displayBudgetChars) } : {}) });
  variants.push({ id: 'typography-112', mode: 'typography', fontScale: 1.12, content: structuredClone(normal), ...(pendingCapacity ? { capacityContent: capacityPlaceholder(displayBudgetChars) } : {}) });
  return variants;
}
...
function factComponent(candidate, index) {
  ...
  const display = socialCardFactDisplaySpec(candidate);
  const content = { title: displayLabel, text: display.displayText, item: null };
  return {
    schemaVersion: SOCIAL_CARD_PAGE_COMPONENT_SCHEMA_VERSION,
    ...
    displayTextStatus: display.status,
    displayLanguage: display.displayLanguage,
    // 不使用 raw source_text 的长度；纯英文事实按固定中文展示预算估算。
    displayBudgetChars: ['pending', 'needs-localization'].includes(display.status) ? 56 : null,
...
export function estimateSocialCardContentComponent(component, { page = {}, capacity = {}, renderType = '', variantId = '' } = {}) {
  ...
  const capacityComponent = ['pending', 'needs-localization'].includes(component?.displayTextStatus) && variant?.capacityContent
    ? { ...component, content: variant.capacityContent, sizeVariants: [{ ...variant, content: variant.capacityContent }] }
    : component;
  const capacityBlock = capacityComponent === component
    ? block
    : renderSocialCardContentComponent(capacityComponent, renderType, { variantId: variant?.id });
  const candidatePage = { ...page, content_blocks: [...currentBlocks, capacityBlock] };
      ...
      for (const component of supplements) {
        ...
        for (const renderType of renderTypes) {
          ...
          for (const variant of variants) {
            ...
            // 先用与页面装箱共用的静态模型淘汰明确超容量的候选；
            // 浏览器审计仍是最终门禁，负责处理主题和字体造成的测量误差。
            if (!estimate.fits) continue;
            scoped.push({
              ...component,
  ...
  for (const layoutPage of targets) {
    ...
    for (const slot of slots) {
      ...
      const candidates = supplements
        ...
        // 程序兜底没有内容创作能力，不能把 sourceText 原样塞进卡片；
        // 所有 pending 候选只交给 AI add_component 生成 display_text。
        .filter((component) => !['pending', 'needs-localization'].includes(component.displayTextStatus))
...
function scoreJointPackingPlan(pages = [], capacityProfile = null) {
  ...
  for (let index = 0; index < pages.length; index += 1) {
    ...
    const overflowPenalty = hardReasons.length ? 1000 + Math.max(0, utilization - 1) * 500 : 0;
    ...
    diagnostics.push({ page: index + 1, role, utilization, target, overCapacity: estimate.overCapacity, hardReasons });
  }
