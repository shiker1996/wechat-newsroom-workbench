...
import { hasEditorialPatch } from '../../domain/editorial-patch.mjs';
...
2. 已能形成编辑决策时：{"type":"final","assistantReply":"给作者的回复（含围绕缺失项的追问）","briefUpdates":{}}——底稿字段（research_basis/angle/thesis/confirmed_facts/author_opinions 等）放在 briefUpdates 内，只写本轮有变化的字段；多值字段用 append/remove/clear 增量更新，单值字段用 replace/set 明确替换，不要再套 output 层。
...
export async function runEditorialAgentTurn({ gateway, store, registry, candidateId, provider, answer = '', events = [], retrieve = null, workspaceRoot, projectPath = '', onEvent = () => {}, budget = {}, suppliedUrls = [], allowedCapabilities = null, signal = null }) {
  ...
  const toolInstruction = nativeTools ? '需要读取资料时，请使用 API 提供的原生工具调用，不要在文本中输出 tool_requests JSON；完成判断后仍只返回 final JSON。' : `${ENVELOPE_INSTRUCTION}\n可用工具：${JSON.stringify(catalog)}`;
  ...
  const agent = await runConversationAgent({ entryPoint: 'editorial', registry, catalog, messages, store, budget, signal, toolContext: { batchId: candidate.batch_id, candidateId, skillId: 'editorial-room-chat', provider: provider || gateway.config.defaultProvider, workspaceRoot, allowedRoots: buildAllowedRoots(workspaceRoot, projectPath), allowedCapabilities: catalog.map((item) => item.capability) }, onEvent, resolveArguments: adaptation.resolveArguments,
    ...
    modelStep: async ({ messages: history, step, signal: modelSignal, emit }) => {
      ...
      const input = { provider, purpose: 'editorial-room', batchId: candidate.batch_id, candidateId, maxOutputTokens: Math.min(3500, providerConfig.maxOutputTokens), jsonMode: !nativeTools, tools: nativeToolDefinitions, nativeTools, webSearch: false, signal: modelSignal, messages: history };
  ...
  const hasBriefUpdates = parsed.briefUpdates && typeof parsed.briefUpdates === 'object' && Object.values(parsed.briefUpdates).some(hasEditorialPatch); if (!String(parsed.assistantReply || '').trim() && !hasBriefUpdates) throw new Error('编辑室本轮未产出有效回复（模型输出为空），请重发上一条回答');
