...
import { runBreakingAnalysisPipeline } from '../../articles/index.mjs';

export async function runAutoPipeline({ gateway, store, config, job, batch, maxAgeHours, onProgress }) {
  if (batch?.batch_type === 'breaking') return runBreakingAnalysisPipeline({ gateway, store, batchId: job.batchId, provider: job.provider, workspaceRoot: config.workspaceRoot, onProgress });
  store.updateBatch(job.batchId, { stage: 'synthesis', status: 'running' });
  job.phase = 'tag';
  let result = await tagBatch({ gateway, store, batchId: job.batchId, provider: job.provider, force: false, maxAgeHours, workspaceRoot: config.workspaceRoot, runId: job.id, onProgress });
  ...
  const cardResult = await ensureBatchEventCards({ gateway, store, batchId: job.batchId, provider: job.provider, workspaceRoot: config.workspaceRoot, maxAgeHours, runId: job.id, onProgress });
  ...
  job.phase = 'research';
  const research = await runResearchPipeline({ gateway, store, batchId: job.batchId, provider: job.provider, workspaceRoot: config.workspaceRoot, maxAgeHours, onProgress });
