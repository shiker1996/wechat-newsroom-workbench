...
import { retryPipelineFailure } from './pipeline-failure-retry.mjs';

export const AI_JOB_TYPES = Object.freeze(['tag', 'retag', 'event-cards', 'research', 'pipeline-failure-retry', 'breaking-analysis', 'article', 'daily', 'tutorial', 'typeset', 'social-card', 'social-card-beautify', 'cover-image', 'auto']);
export const BATCH_LEVEL_AI_JOB_TYPES = new Set(['tag', 'retag', 'event-cards', 'research', 'pipeline-failure-retry', 'breaking-analysis', 'auto', 'daily']);
...
export function createAiJobHandlers({ store, gateway, config, log, onThinking = () => {} }) {
  ...
  return new Map([
    ...
    ['research', async ({ job, maxAgeHours, options }) => { const result = await runResearchPipeline({ gateway, store, batchId: job.batchId, provider: job.provider, workspaceRoot: config.workspaceRoot, maxAgeHours, resumeFrom: options.resumeFrom || '', onProgress: progress(job) }); for (const failure of store.listPipelineFailures(job.batchId, { statuses: ['open', 'retrying'], stages: ['research'] })) store.resolvePipelineFailure(failure.id); return result; }],
    ['pipeline-failure-retry', async ({ job, maxAgeHours, options }) => retryPipelineFailure({ failureId: options.failureId, batchId: job.batchId, provider: job.provider, store, gateway, config, maxAgeHours, onProgress: progress(job) })],
