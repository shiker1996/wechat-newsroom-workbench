...
export const LEGACY_CONFIGURATION_FALLBACK_REMOVAL_VERSION='0.7.0';
...
export class ExtensionConfigurationService {
  ...
  describe({extensionType,extensionId,manifest,fallbackValues={}}) {
    ...
    // 运行时只读取统一配置仓库；fallbackValues 仅保留参数兼容，不再参与业务解析。
    fallbackValues={};
    ...
    const values={...schemaDefaults(schema),...storedValue,...getCredentialFields(this.root,profile,secretFields(schema))};
    ...
    return {extensionType,extensionId,schema,configured,status:configured?'ready':'needs_configuration',issues,
      ...
      configHash:stored?.config_hash||hash(values),source:stored||credentialFields.size?'unified':'defaults',legacyFallbackFields,
      legacyFallbackRemovalVersion:LEGACY_CONFIGURATION_FALLBACK_REMOVAL_VERSION,
      emptyValuePolicy:{ordinary:'preserve_and_validate',secret:'empty_preserves_existing'} };
  }
  save({extensionType,extensionId,manifest,input,fallbackValues={}}) {
    ...
    const complete={...values,...existing,...secretInput}; const issues=validateConfigurationValue(schema,complete);
    ...
    return {...this.describe({extensionType,extensionId,manifest}),updatedAt:record.updated_at};
  }
  ...
  resolve({extensionType,extensionId,manifest,fallbackValues={}}) {
    const state=this.describe({extensionType,extensionId,manifest});
    ...
    return {...state,values:{...schemaDefaults(manifest.configuration),...declaredFallback(manifest.configuration,stored?.value||{}),...getCredentialFields(this.root,profile,secretFields(manifest.configuration))},
      snapshot:this.snapshot({extensionType,extensionId,manifest})};
  }
}
