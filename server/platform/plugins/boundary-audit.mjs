...
export function readPluginBoundaryBaseline(root){const file=path.join(root,'test','fixtures','plugin-boundary-baseline.json'),value=JSON.parse(fs.readFileSync(file,'utf8'));if(value.schemaVersion!==1||!Array.isArray(value.violations))throw new Error('plugin-boundary-baseline.json 格式无效');return value;}
