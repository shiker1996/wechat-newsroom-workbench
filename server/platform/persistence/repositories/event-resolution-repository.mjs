  ...
  upsertShadow(batchId, shadow) {
    ...
    try {
      ...
      const upsertEvent = this.db.prepare(`INSERT INTO event_records
        ...
        last_update_at=CASE WHEN excluded.event_state IN ('new_event','new_update') THEN excluded.last_update_at ELSE event_records.last_update_at END,status=excluded.status,
      ...
      for (const event of events) {
        ...
        for (const [index, hotspotId] of (event.hotspot_ids || []).entries()) {
          ...
          const isNewInformation = newInformationIds.has(numericId);
          upsertMembership.run(event.event_id, numericId, batchId, isNewInformation && eventState === 'new_update' ? 'update' : (index === 0 ? 'primary' : 'duplicate'), method, matchConfidence,
            isNewInformation ? 1 : 0, now, now);
          memberships += 1;
        }
      }
      this.db.exec('COMMIT');
