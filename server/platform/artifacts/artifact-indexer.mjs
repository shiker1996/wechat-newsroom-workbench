...
export function resolveArtifactRelativeAsset(artifactFile, relativePath, roots) {
  const artifactPath = path.resolve(artifactFile);
  const artifactDir = path.dirname(artifactPath);
  const resolved = path.resolve(artifactDir, String(relativePath || ''));
  const insideArtifactDir = resolved.startsWith(`${artifactDir}${path.sep}`);
  if (!insideArtifactDir || !isInsideRoots(resolved, roots)) return null;
