...
export class ToolRegistry{
  ...
  async execute(capability,input={},context={},preferences={}){
    const started=performance.now(),startedAt=new Date().toISOString(),candidates=this.resolveCandidates(capability,preferences).filter(({manifest})=>!Array.isArray(context.implementationScope?.[capability])||context.implementationScope[capability].some((item)=>item.plugin===manifest.id&&item.version===manifest.version)),resolutionId=context.resolutionId||`tool-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,9)}`;let configurationState=null,previous=null,last=null;
