...
import { validatePluginManifestBase } from '../plugin-sdk/manifest-contract.mjs';
...
export async function loadPluginManifests({ pluginsRoot, allowlist = [] }) {
  ...
  for (const id of allowlist) {
    ...
    for(const field of ['requiredCapabilities','optionalCapabilities']){
      ...
      if(!Array.isArray(values)||values.some((value)=>typeof value!=='string'||!value.trim())||new Set(values).size!==values.length){
        throw new Error(`插件 ${id} ${field} 无效`);
      }
    }
    if((manifest.requiredCapabilities||[]).some((value)=>(manifest.optionalCapabilities||[]).includes(value))){
      throw new Error(`插件 ${id} 的 requiredCapabilities 与 optionalCapabilities 不能重复`);
    }
