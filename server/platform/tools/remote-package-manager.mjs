...
function hashManifest(value){return `sha256:${crypto.createHash('sha256').update(JSON.stringify(canonical(value))).digest('hex')}`;}
...
export function validateRemotePluginManifest(input){
  ...
  for(const field of ['requiredCapabilities','optionalCapabilities']){const values=manifest[field]||[];if(!Array.isArray(values)||values.some((item)=>typeof item!=='string'||!item.trim())||new Set(values).size!==values.length)throw new Error(`远程插件 ${field} 无效`);}
  if((manifest.requiredCapabilities||[]).some((item)=>(manifest.optionalCapabilities||[]).includes(item)))throw new Error('远程插件能力依赖不能同时声明为必需和可选');
...
export function installRemotePlugin(root,input,{expectedManifestHash=''}={}){
  ...
  const manifestHash=hashManifest(manifest);if(expectedManifestHash&&manifestHash!==expectedManifestHash)throw new Error('远程插件 Manifest 校验和不匹配');
  ...
  const item={id:manifest.id,name:manifest.name,version:manifest.version,status:'disabled',manifest,manifestHash,installedAt:new Date().toISOString()};
...
export function setRemotePluginStatus(root,id,status){
  ...
  if(status==='enabled'&&(!item.manifestHash||hashManifest(item.manifest)!==item.manifestHash))throw new Error('远程插件 Manifest 完整性校验失败');
