...
export function installToolPlugin({workspaceRoot,directory,builtinIds=[],expectedContentHash=''}){
  let staging=null,target=null,archived=null,identity={pluginId:'',version:''};
  try{
    const checked=validateToolPluginDirectory(directory);
    if(expectedContentHash&&checked.contentHash!==expectedContentHash)throw new Error('插件包校验和不匹配');
...
export function rollbackToolPlugin(root,id,version){
  if(!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(String(id||''))||!/^[0-9A-Za-z][0-9A-Za-z.-]{0,63}$/.test(String(version||''))||String(version).includes('..'))throw new Error('回滚插件 ID 或版本无效');
