...
// implementationHealthy：调用方传入 healthByCapability（Map<capability, verdict>，见 server/platform/tools/health-check.mjs）时
...
export function buildCapabilityGraph({root,tools=[],collectors=[],collectionSources=[],routes={},configurationState=()=>({configured:true}),healthByCapability=null}){
  ...
  for(const source of collectionSources.filter((item)=>item.enabled!==false))consumers.push({id:String(source.id),name:source.label||source.source_key,type:'collection-source',enabled:true,dependencies:[{capability:`cap_collect_${String(source.source_type||'').replace(/[^A-Za-z0-9_]/g,'_')}`,requirement:'required',failurePolicy:'block',source:'collection-source'}]});
