...
// 工具的启用与关闭统一由「技能与工具 → 信息工具」槽位配置管控，创作表单不再提供单独开关。
// 搜索与检索结果属于外部资料，等同于 user_material 级素材：必须保留来源归属，不得写成作者亲历。
// 文档检索只扫描插件配置中明确授权的目录（兼容期由调用方传入旧 documentSearch.roots）。

const SEARCH_SLOTS = [
  { slotId: 'web-search', field: 'web_search' },
  { slotId: 'news-search', field: 'news_search' },
  { slotId: 'document', field: 'document_search' },
];
...
async function attachDocumentSearch({ fact, root, slot, roots, toolContext, notes }) {
  ...
  if (!authorized.length) {
    notes.push('文档检索未配置授权知识库目录，请前往系统与配置中心完成配置');
    return null;
  }
  const merged = [];
  const warnings = [];
