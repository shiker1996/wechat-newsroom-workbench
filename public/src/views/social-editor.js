  ...
  document.getElementById('custom-social-submit')?.addEventListener('click',async()=>{
    ...
    try{
      const data=await request(`/api/batches/${encodeURIComponent(batch.id)}/custom-social-candidates`,{method:'POST',body:JSON.stringify({
        ...
        enableWebSearch:document.getElementById('custom-enable-web-search').checked,enableNewsSearch:document.getElementById('custom-enable-news-search').checked,enableDocumentSearch:document.getElementById('custom-enable-document-search').checked,
      })});
