...
const titles = { dashboard: '今日值班', batches: '每日批次', overview: '热点全景', topics: '选题池', editorial: '编辑室', editor: '文章编辑器', preview: '排版预览', hotspots: '热点档案', artifacts: '产物柜', system: '采集控制', sources: '订阅源台账', models: '模型中心', logs: '日志', calendar: '内容日历' };

const viewModules = {
  dashboard: '', batches: '', overview: {load:['/app-pool-editorial.js']},
  topics: '/app-pool-editorial.js', editorial: {load:['/app-pool-editorial.js','/app-editor-production.js']},
  editor: '/app-editor-production.js', preview: '/app-editor-production.js',
  hotspots: '', artifacts: '',
  system: '', sources: '', models: '/app-models-logs.js',
  logs: '/app-models-logs.js', calendar: '',
};
const loadedScripts = new Set();
function ensureModule(view) {
  var entry = viewModules[view];
  if (!entry) return Promise.resolve();
  var urls = typeof entry === 'string' ? [entry] : (entry.load || []);
  ...
  return Promise.all(urls.map(function(url) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      s.src = url;
      s.async = false;
      s.onload = function() { loadedScripts.add(url); resolve(); };
      s.onerror = function() { reject(new Error('模块加载失败: ' + url)); };
...
// 给按钮加上 loading click 监听
function loadingClick(selector, label, fn) {
  const el = $(selector);
  if (!el) return;
  el.addEventListener('click', () => withLoading(el, label, fn));
}
...
function renderRankingList(items, container) {
  container.innerHTML = items.map(function(item) {
    var reason = item.eliminationReason ? '<span class="muted" style="font-size:9px">' + escapeHtml(item.eliminationReason) + '<\/span>' : '<span class="muted" style="font-size:9px">已入池<\/span>';
    var cls = item.inPool ? 'ranking-row in-pool' : 'ranking-row';
    var btn = item.inPool ? '' : '<button class="text-button" data-ranking-add="' + item.hotspotId + '" style="font-size:9px">加入候选<\/button>';
...
function renderCandidates(candidates) {
  ...
  $('#candidate-list').innerHTML = candidates.length ? candidates.map(function(item) {
    var card = '<article class="candidate-card ' + (item.composite ? 'composite' : '') + '" data-id="' + escapeHtml(item.candidate_id) + '">';
    card += '<h4>' + escapeHtml(item.hotspot_title) + (item.composite ? ' <span class="composite-tag">综合<\/span>' : '') + '<\/h4><div class="candidate-meta"><span>' + escapeHtml(item.pool_role) + '<\/span><span>' + (item.composite ? '多源综合' : escapeHtml(item.source_name || item.source_group || item.source)) + '<\/span><span>风险 ' + escapeHtml(item.risk_level) + '<\/span><\/div>';
    card += '<div class="score-strip">' + ['h','b','p','s','d','f'].map(function(k) { return '<span>' + k.toUpperCase() + '<b>' + scoreValue(item[k + '_score']) + '<\/b><\/span>'; }).join('') + '<\/div>';
    card += '<div class="candidate-actions"><span class="status-pill">' + escapeHtml(item.brief_status || item.status) + '<\/span><button class="text-button" data-editorial-id="' + item.id + '">进入编辑室 →<\/button>' + (item.status !== 'locked' && item.status !== 'drafting' && item.status !== 'review' && item.status !== 'preview' && item.status !== 'published' ? '<button class="text-button muted" data-remove-candidate="' + item.id + '">移除<\/button>' : '') + '<\/div>';
    card += '<\/article>';
    return card;
  }).join('') : '<div class="empty-state">暂无候选。在热点全景中通过事件归纳卡片创建综合选题，AI 研判后自动填充评分。<\/div>';
}
...
async function loadSimilarArticles(id) {
  const container = $('#similar-articles');
  if (!container) return;
  try {
    const similar = await request('/api/candidates/' + id + '/similar');
    if (similar.length) {
      container.innerHTML = '<b>历史覆盖</b>' + similar.map(function(s) {
        return '<div><a data-cal-article="'+s.id+'">' + escapeHtml(s.title.slice(0, 30)) + '</a> <span class="muted">' + escapeHtml(s.batchDate) + ' · ' + escapeHtml(s.poolRole || '') + '</span></div>';
      }).join('');
...
async function loadWritingDesk() {
  ...
  state.candidates = candidates.filter((item) => item.brief_status === 'LOCKED' || (item.brief_status == null && item.status === 'locked'));
...
async function loadSelectedDocument() {
  ...
  if (!document && kind === 'draft') {
    try {
      ...
      // 按 batch 和 candidate 查找匹配的初稿产物
      const draftArtifact = artifacts.find((item) => item.kind === '文章初稿' && item.batch_id === state.activeBatchId && item.file_path && item.file_path.includes(candidate ? candidate.candidate_id.toLowerCase() : ''));
      if (draftArtifact) {
        const res = await fetch(`/api/artifacts/${draftArtifact.id}/content`);
        if (res.ok) {
          const text = await res.text();
          document = { title: '草稿', content: text };
        }
      }
    } catch {}
  }
...
function renderImageWorkspace() {
  ...
  const btn = $('#run-local-typeset');
  btn.disabled = !hasCandidate || !data.planned || data.unresolved.length > 0;
  btn.title = !hasCandidate ? '请先运行完整成稿链'
    : !data.planned ? '请先点击「AI 规划必要配图」'
    : data.unresolved.length ? `以下图片待上传：${data.unresolved.join('、')}`
    : '生成公众号排版 HTML';
}
...
async function addSubscriptionFromForm(event) {
  ...
  if (event.currentTarget) event.currentTarget.reset(); updateSubscriptionComposer(); renderSubscriptions();
...
async function loadCalendar(year, month) {
  const now = new Date();
  if (year == null) { year = now.getFullYear(); month = now.getMonth() + 1; }
  state.calYear = year; state.calMonth = month;
  const monthStr = year + "-" + String(month).padStart(2,"0");
  document.getElementById("cal-month-label").textContent = monthStr;
  fetch("/api/articles?month=" + encodeURIComponent(monthStr)).then(function(r){return r.json();}).then(function(articles){
    document.getElementById("cal-count").textContent = articles.length + " \u7bc7";
    var dayMap = {};
    for (var i = 0; i < articles.length; i++) {
      var a = articles[i];
      var d = a.batch_date ? new Date(a.batch_date + "T00:00:00") : new Date(a.updated_at);
      if (!isNaN(d.getTime())) {
        var day = d.getDate();
    ...
    for (var ni = 0; ni < names.length; ni++) html += '<div class="cal-header">' + names[ni] + '</div>';
    html += '</div>';
    var day = 1;
    var ended = false;
    for (var row = 0; row < 6 && !ended; row++) {
      html += '<div class="cal-week-row">';
      for (var col = 0; col < 7; col++) {
        if ((row === 0 && col < startDay) || day > lastDate) {
          html += '<div class="cal-cell cal-empty-cell"></div>';
        } else {
          var items = dayMap[day] || [];
          var isToday = day === now.getDate() && month === now.getMonth()+1 && year === now.getFullYear();
          html += '<div class="cal-cell"><div class="cal-date-label">' + day;
          if (isToday) html += ' <span class="cal-today-dot">\u25cf</span>';
          html += '</div>';
          if (items.length) {
            for (var ai = 0; ai < items.length; ai++) {
              var a = items[ai];
              var t = (a.title || a.hotspot_title || '').slice(0, 22);
