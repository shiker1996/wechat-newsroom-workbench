...
function getWeekParam(date) {
  const d = new Date(date);
  d.setDate(d.getDate() - d.getDay() + 1);
  const start = new Date(d.getFullYear(),0,1);
  const weekNum = Math.ceil((d - start) / 604800000);
  return d.getFullYear() + '-W' + String(weekNum).padStart(2,'0');
}
async function loadCalendar(week) {
  const now = new Date();
  const weekParam = week || getWeekParam(now);
  state.calendarWeek = weekParam;
  const articles = await request('/api/articles?week=' + encodeURIComponent(weekParam));
  $('#cal-week-label').textContent = weekParam;
  $('#cal-count').textContent = articles.length + ' 篇';
  const year = parseInt(weekParam.slice(0,4)); const w = parseInt(weekParam.slice(6));
  const firstDay = new Date(year, 0, 1 + (w - 1) * 7);
  const dayOfWeek = firstDay.getDay() || 7;
  const monday = new Date(firstDay); monday.setDate(firstDay.getDate() - dayOfWeek + 1);
  const dayMap = {};
  for (let i = 0; i < 7; i++) { const d = new Date(monday); d.setDate(monday.getDate() + i); dayMap[d.getDay()] = []; }
  for (const a of articles) {
    const d = new Date(a.updated_at);
    if (!isNaN(d.getTime()) && dayMap[d.getDay()]) dayMap[d.getDay()].push(a);
  }
  for (const col of $$('.cal-col')) {
    const dayDiv = col.querySelector('.cal-day');
    const dayNum = parseInt(dayDiv.dataset.day);
    const date = new Date(monday); date.setDate(monday.getDate() + (dayNum === 0 ? 6 : dayNum - 1));
    const items = dayMap[dayNum] || [];
    const dateStr = (date.getMonth()+1) + '/' + date.getDate();
    if (items.length) {
      dayDiv.innerHTML = '<div class="cal-date-label">' + dateStr + '</div>' + items.map(function(a) {
        const t = (a.title || a.hotspot_title || '').slice(0, 28);
        return '<div class="cal-article" title="' + escapeHtml(a.batch_date) + ' \u00b7 ' + escapeHtml(a.pool_role || '') + '"><span style="cursor:pointer" data-cal-article="' + a.id + '">' + escapeHtml(t) + '</span></div>';
      }).join('');
    } else {
      dayDiv.innerHTML = '<div class="cal-date-label muted">' + dateStr + '</div><div class="cal-empty">\u2014</div>';
    }
  }
}
...
﻿function bind() {
  ...
  document.addEventListener('click', (event) => {
    ...
    const calArticle=event.target.closest('[data-cal-article]');if(calArticle){document.getElementById('artifact-dialog').showModal();document.querySelector('#artifact-dialog iframe').src='/api/artifacts/'+calArticle.dataset.calArticle+'/content';}
