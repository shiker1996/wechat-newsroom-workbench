...
function renderPoolSources(items, query = '') {
  ...
  for (const [eventKey, group] of grouped) {
    ...
    for (const item of group) {
      ...
      if(!pooled.has(item.id)&&!item.is_stale){var r=tryParseJson(item.raw_json);if(r.eliminationReason){html+='<div style="font-size:9px;color:var(--muted);padding:2px 0 4px 28px">\u6dd8\u6c70\uff1a'+escapeHtml(r.eliminationReason)+'<\/div>';}}
    }
    html += '</div>';
  }
...
function tryParseJson(str){try{return JSON.parse(str||"{}");}catch{return {};}}
