  ...
  recordModelCall(input) {
    ...
    const result = this.db.prepare(`INSERT INTO model_calls
      (provider,model,purpose,batch_id,candidate_row_id,estimated_input_tokens,prompt_tokens,
       completion_tokens,reasoning_tokens,compressed,latency_ms,status,error,generation_snapshot_id,output_budget_json,output_text,created_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      ...
      input.outputText ?? null, new Date().toISOString());
