...
export class BatchQueryService {
  ...
  getBatch(id) {
    ...
    batch.pipeline_failures = this.db.prepare(`SELECT * FROM pipeline_failures WHERE batch_id=?
      ORDER BY CASE stage WHEN 'collect' THEN 1 WHEN 'tag' THEN 2 WHEN 'event-card' THEN 3 ELSE 4 END,last_failed_at DESC,id DESC`).all(id)
      .map((row) => { let detail={};try{detail=JSON.parse(row.detail_json||'{}');}catch{}return {...row,detail}; });
    batch.ai_status = {
