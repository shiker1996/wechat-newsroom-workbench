...
// R3：远程 Manifest 声明目录外能力时生成目录条目草案（保守占位，needsCompletion 标记需人工补全）
export function catalogDraftsForManifest(root,manifest){
  ...
  return (manifest.capabilities||[]).filter((id)=>!catalog.capabilities[id]).map((id)=>({
    id,name:id,description:`远程插件「${manifest.name||manifest.id}」声明的能力，待人工补全名称、描述与分类。`,category:'扩展能力',needsCompletion:true,
  }));
}
