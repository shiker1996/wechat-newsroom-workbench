...
import { privateIp } from '../../plugins/shared/network-safety.mjs';

export { privateIp } from '../../plugins/shared/network-safety.mjs';
...
export function createRemoteAdapter({root,manifest,dependencies={}}){
  ...
  return {
    async execute(input){
      try{
        const entry=readRemotePluginCatalog(root).plugins[manifest.id];
        if(entry&&entry.status!=='uninstalled'&&!entry.firstRunConfirmedAt){
          return failure('FIRST_RUN_CONFIRM_REQUIRED',`远程插件「${manifest.name||manifest.id}」首次执行前，请在「技能与插件」页面确认其域名与权限`,{retryable:false});
        }
