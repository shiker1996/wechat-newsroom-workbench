...
export function readCapabilityRoutes(root){const file=fileFor(root);if(!fs.existsSync(file))return {schemaVersion:1,routes:{}};const value=JSON.parse(fs.readFileSync(file,'utf8'));if(value.schemaVersion!==1||!value.routes)throw new Error('统一能力路由配置无效');return value;}
