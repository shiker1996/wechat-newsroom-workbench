...
export class ToolRegistry{
  ...
  listPlugins(){return [...this.#plugins].sort((left,right)=>this.#state(right.manifest).priority-this.#state(left.manifest).priority||left.manifest.id.localeCompare(right.manifest.id)).map(({manifest})=>({...manifest,...this.#state(manifest)}));}
  ...
  resolveCandidates(capability,preferences={}){return orderedCandidates(this.#plugins.filter(({manifest})=>manifest.capabilities.includes(capability)&&this.#state(manifest).enabled),{preferredId:preferences.plugin,priorityOf:(item)=>this.#state(item.manifest).priority});}
  ...
  async execute(capability,input={},context={},preferences={}){
    ...
    for(let index=0;index<candidates.length;index+=1){const selected=candidates[index];configurationState=null;let result;
      ...
      if(!result)try{result=await selected.adapter.execute(input,Object.freeze({...context,capability,pluginId:selected.manifest.id,configuration:configurationState?.values||{},resolutionId,attempt:index+1}));const invalid=validateResult(result,selected.manifest.outputSchema);if(invalid)result=failure('OUTPUT_INVALID',invalid);}catch(error){result=thrown(error);}
