...
function legacyKeyFor(profile){return `REMOTE_PLUGIN_${crypto.createHash('sha256').update(profile).digest('hex').slice(0,16).toUpperCase()}_TOKEN`;}
...
export function setRemoteCredential(root,pluginId,profile,token){
  ...
  current[keyFor(profile)]=token.trim();delete current[legacyKeyFor(profile)];writeEnv(root,current);
...
export function clearRemoteCredential(root,pluginId,profile){
  const current=fs.existsSync(envPath(root))?parseEnv(fs.readFileSync(envPath(root),'utf8')):{};
  delete current[keyFor(profile)];delete current[legacyKeyFor(profile)];writeEnv(root,current);
...
export function getRemoteCredential(root,profile){
  ...
  return values[keyFor(profile)]||values[legacyKeyFor(profile)]||'';
}
...
export function getCredentialFields(root,profile,fields=[]){
  ...
  return Object.fromEntries(fields.flatMap((field)=>{
    const value=values[keyFor(profile,field)]||(field==='token'?values[legacyKeyFor(profile)]:'');return value?[[field,value]]:[];
  }));
}
