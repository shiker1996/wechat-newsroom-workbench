...
// 同一视觉行中的同类型、近似体量内容属于同级信息，应使用等宽列；
// 稳定种子只负责选择视觉变体，不应把同级安装命令随机分成主次栏。
function peerBlocksPreferEven(blocks) {
  if(blocks.length<2||blocks[0]?.type!==blocks[1]?.type)return false;
  const first=cardBlockVolume(blocks[0]),second=cardBlockVolume(blocks[1]);
...
function peerGroupPrefersEven(blocks) {
  if(blocks.length===3)return peerBlocksPreferEven(blocks);
...
export function renderStoryboardHtml({ topic, repository, pages, visualStyle='ice-blue', themeDefinition:providedTheme=null, layoutStyle='auto', compositionMode='template', compositionSeed='', forceSafeComposition=false, expandedDensityPages=false, contentType='repository', sourceLabel='', disclosure='', channelMode='wechat' }) {
...
.page.density-expanded .page-content-stack{gap:calc(var(--card-gap) + 5px)}.page.density-expanded h1{font-size:max(var(--h1-size),min(calc(var(--h1-size) + 4px),34px))}.page.density-expanded .content-block h2{font-size:max(var(--h2-size),min(calc(var(--h2-size) + 2px),18px))}.page.density-expanded .content-block p,.page.density-expanded li{font-size:max(var(--body-size),min(calc(var(--body-size) + 1px),13px));line-height:max(var(--line-height),1.52)}.page.density-expanded .content-block{gap:calc(var(--paragraph-gap) + 3px);padding-block:6px}
...
export async function runSocialCardPipeline({ gateway, store, batchId, candidateId, provider, workspaceRoot, snapshotId=null, onProgress=()=>{} }) {
  ...
  const renderCurrentStoryboard=()=>renderStoryboardHtml({ topic:candidate.hotspot_title, repository:facts?.data?.repository, pages:cardPlan, visualStyle:editorial.visual_style, themeDefinition, layoutStyle:editorial.layout_style, compositionMode:editorial.composition_mode||'template',
    compositionSeed:`${candidate.batch_id}|${candidate.id}`,forceSafeComposition:safeCompositionApplied?(safeCompositionPages.size?[...safeCompositionPages]:true):false,expandedDensityPages,contentType,channelMode,sourceLabel:contentType==='event'?'事件专题':contentType==='custom'?facts?.data?.content_type_label||'自定义':'',disclosure:contentType==='event'?'据公开素材整理 · 未核实内容已标注':'' });
  ...
  for (let attempt=0; attempt<maxLayoutAttempts; attempt += 1) {
    ...
    const expandable=underfilledPageIndexes(report,expandedDensityPages);
    if(expandable.length){
      expandedDensityPages=new Set([...expandedDensityPages,...expandable]);
      deterministicRerender=true;
    }
    ...
    if(repairGuardIssues.length){if(attempt>=maxLayoutAttempts-2)throw new Error(`第 ${attempt + 1} 轮布局修复修改了受保护的故事板结构：${repairGuardIssues.join('；')}`);continue;}
    repairGuardIssues=[];
    cardPlan = newPlan;
  ...
  const compositionNotes=editorial.composition_mode==='smart'
    ...
    : [];
  record('layout-audit', generator.skillName, reportPath, `安全变体：${safeCompositionApplied?safeCompositionPages.size?`已启用（${[...safeCompositionPages].map((index)=>`P${index+1}`).sort().join('、')}）`:'已启用':'未触发'}；舒展排版：${expandedDensityPages.size?`已启用（${[...expandedDensityPages].map((index)=>`P${index+1}`).sort().join('、')}）`:'未触发'}；内容修复轮次：${repairCount}${compositionNotes.length?`；${compositionNotes.join('、')}`:''}`);
