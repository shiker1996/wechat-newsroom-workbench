...
function templateAuditFailurePayload({ requestedTemplate, renderedTemplate, auditAttempts, report, repairCount, safeCompositionPages, relaxedDensityPages, expandedDensityPages, maxLayoutAttempts, noProgressGuard = null }) {
  return {
    schemaVersion: 1,
...
export async function runSocialCardPipeline({ gateway, store, batchId, candidateId, provider, workspaceRoot, snapshotId=null, onProgress=()=>{} }) {
  ...
  const failStrictLayout=(message)=>{
    ...
    const failurePayload=templateAuditFailurePayload({ requestedTemplate, renderedTemplate, auditAttempts, report, repairCount, safeCompositionPages, relaxedDensityPages, expandedDensityPages, maxLayoutAttempts, noProgressGuard });
  ...
  for (let attempt=0; attempt<maxLayoutAttempts; attempt += 1) {
    ...
    if(report.valid && densityCalibration.pages.length){
      const newlyFitPages=densityCalibration.pages.map((item)=>Number(item.page)-1).filter((index)=>index>=0&&!fitContentPages.has(index));
