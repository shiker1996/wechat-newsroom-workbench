...
import { compileTemplateAwareCardPlan, estimateSocialCardPageLoad, rebalanceContinuationPages, scaleSocialCardCapacityProfile } from '../rendering/social-card-reflow.mjs';
...
function buildSocialCardPlannerPageScope(cardPlan = [], plannerPages = []) {
  ...
  // 同故事线续页必须一起提供，模型才知道 merge/move 的合法边界。
...
function buildSocialCardPlannerFactScope(factPayload = {}, factIndex = {}, contentComponents = null, scopedPlan = []) {
  ...
  const candidates = (Array.isArray(factIndex?.candidates) ? factIndex.candidates : [])
    ...
    .map((candidate) => ({ id: candidate.id, label: candidate.label, display_label: candidate.display_label, text: candidate.text, source_refs: candidate.source_refs, tags: candidate.tags, priority: candidate.priority, semantic_intent: candidate.semantic_intent }));
