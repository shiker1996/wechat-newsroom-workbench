...
export function describeCardLayouts(pages, { layoutStyle='auto', channelMode='wechat' }={}) {
  return (Array.isArray(pages) ? pages : []).map((page, index) => ({ page:index + 1, ...resolveCardLayoutDecision(page, layoutStyle, channelMode), density:cardPageDensity(page) }));
}
...
export function renderStoryboardHtml({ topic, repository, pages, visualStyle='ice-blue', layoutStyle='auto', contentType='repository', sourceLabel='', disclosure='', channelMode='wechat' }) {
  ...
  const sections = safePages.map((page, index) => {
    ...
    const layoutDecision = resolveCardLayoutDecision(page, layoutStyle, channelMode);
...
export async function runSocialCardPipeline({ gateway, store, batchId, candidateId, provider, workspaceRoot, onProgress=()=>{} }) {
  ...
  writeFile(planPath, JSON.stringify({ channel_mode:outputMode||editorial.output_mode || 'xiaohongshu', layout_style:editorial.layout_style||'auto', topic:candidate.hotspot_title, pages:cardPlan }, null, 2));
  ...
  let html = renderStoryboardHtml({ topic:candidate.hotspot_title, repository:facts?.data?.repository, pages:cardPlan, visualStyle:editorial.visual_style, layoutStyle:editorial.layout_style,
    contentType,channelMode,sourceLabel:contentType==='event'?'事件专题':contentType==='custom'?facts?.data?.content_type_label||'自定义':'',disclosure:contentType==='event'?'据公开素材整理 · 未核实内容已标注':'' });
  ...
  for (let attempt=0; attempt<4; attempt += 1) {
    ...
    if (attempt === 3) throw new Error(`布局审计五轮修复后仍未通过：${report.pages.filter((page)=>!page.valid).map((page)=>`P${page.page} ${page.issues.join('/')}`).join('；')}`);
    repairCount += 1;
    ...
    cardPlan = newPlan;
    writeFile(planPath, JSON.stringify({ channel_mode:editorial.output_mode || 'xiaohongshu', layout_style:editorial.layout_style||'auto', topic:candidate.hotspot_title, pages:cardPlan }, null, 2));
    html = renderStoryboardHtml({ topic:candidate.hotspot_title, repository:facts?.data?.repository, pages:cardPlan, visualStyle:editorial.visual_style, layoutStyle:editorial.layout_style,
