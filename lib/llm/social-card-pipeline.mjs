...
export function normalizeCardComposition(page={}, {pageIndex=0,seed='',forceSafe=false,avoidIds=[]}={}) {
  ...
  const adjusted=partial&&['columns','flow','alignment','decoration','overlap'].some((key)=>input[key]!=null&&!SMART_ENUMS[key].has(input[key]));
  const resolved=partial
    ...
    : {...recommended};
  // 以下情况分列构图都不合适，统一降级为单列堆叠：只有一个内容块（唯一块挤半列留白）、内容密度偏高（列宽不足导致溢出）、块间体量悬殊（两侧一高一低）
  const blocks=Array.isArray(page.content_blocks)?page.content_blocks:[];
  const composition=(blocks.length===1||cardPageDensity(page)==='compact'||imbalancedCardBlocks(blocks))&&resolved.columns!=='single'?{...resolved,columns:'single',flow:'stack'}:resolved;
...
export function resolveCardCompositionDecision(page,{compositionMode='smart',layoutStyle='auto',channelMode='wechat',pageIndex=0,seed='',forceSafe=false,avoidIds=[]}={}) {
  ...
  return {mode:'smart',role:smart.role,composition:smart.composition,variantIndex:smart.variantIndex,variantCount:smart.variantCount,layout:recommendedCardLayout(page,channelMode),source:smart.fallback?'fallback':smart.source,adjusted:smart.adjusted,reason:forceSafe?'布局审计触发安全变体':smart.fallback?'故事板构图参数不合法，已使用稳定种子推荐构图':smart.adjusted?'故事板构图部分字段不合法，已按注册变体补齐':`按${smart.role}页面角色选择稳定构图变体`};
}
...
export function renderStoryboardHtml({ topic, repository, pages, visualStyle='ice-blue', layoutStyle='auto', compositionMode='template', compositionSeed='', forceSafeComposition=false, contentType='repository', sourceLabel='', disclosure='', channelMode='wechat' }) {
  ...
  const sections = safePages.map((page, index) => {
    ...
    const compositionClasses = compositionDecision.mode === 'smart'
      ? `composition-smart role-${compositionDecision.role} comp-${composition.id} comp-cols-${composition.columns} comp-flow-${composition.flow} comp-align-${composition.alignment} decor-${composition.decoration} overlap-${composition.overlap}`
      : 'composition-template';
    const pageDensity = cardPageDensity(page);
    const blockCount = Math.min(4, Array.isArray(page.content_blocks) ? page.content_blocks.length : 0);
    ...
    const blocks = (Array.isArray(page.content_blocks) ? page.content_blocks : []).slice(0,4).map((block) => {
      ...
      const inferredSteps=pageLayout==='steps'&&block.type==='text'?numberedTextSteps(content):[];
      ...
      if(block.type==='list'){const cleanListItem=(item)=>String(item).replace(/^[-*+•·✓✔✅☑]\uFE0F?\s*/u,'').trim();const lines=listBlockValues(block).map((item)=>cleanListItem(typeof item==='string'?item:String(item.title||item.content||'')));return `<div class="content-block list-block">${title}<ul>${lines.filter(Boolean).map((item)=>`<li>${escapeHtml(item)}</li>`).join('')}</ul></div>`;}
...
.stat-row{display:flex;gap:8px}.stat{flex:1;min-width:0;padding:10px 8px;border:1px solid var(--line,rgba(27,109,152,.18));border-radius:12px;background:var(--surface,rgba(255,255,255,.66));text-align:center}.stat b{display:block;font-size:19px;line-height:1.2;color:var(--accent,#1b6d98);overflow-wrap:anywhere}.stat span{font-size:9px;color:var(--muted,#476176)}
.compare-block table{width:100%;border-collapse:collapse}.compare-block th{background:var(--accent,#1b6d98);color:var(--page,#fff);padding:6px 8px;text-align:left;font-size:9px;font-weight:700}.compare-block td{padding:6px 8px;border-bottom:1px solid var(--line,rgba(27,109,152,.18));font-size:11px;line-height:1.45;color:var(--ink,#102033);overflow-wrap:anywhere}
.step-col{display:grid;gap:8px}.step{display:flex;gap:9px;align-items:flex-start}.step>b{flex:0 0 22px;height:22px;border-radius:50%;background:var(--accent,#1b6d98);color:var(--page,#fff);font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center}.step h3{font-size:11px;margin:0;color:var(--ink,#102033);overflow-wrap:anywhere}.step p{font-size:11px;line-height:1.45;margin:2px 0 0;color:var(--muted,#476176);overflow-wrap:anywhere}
.tl{display:grid}.tl-node{position:relative;margin-left:5px;padding:0 0 9px 12px;border-left:2px solid var(--accent2,#ec6b43)}.tl-node:last-child{padding-bottom:0}.tl-node:before{content:"";position:absolute;left:-5px;top:3px;width:8px;height:8px;border-radius:50%;background:var(--accent2,#ec6b43)}.tl-time{font-size:9px;font-weight:800;color:var(--accent2,#ec6b43)}.tl-node h3{font-size:11px;margin:1px 0;color:var(--ink,#102033)}.tl-node p{font-size:11px;line-height:1.45;margin:0;color:var(--muted,#476176)}
.scene-row{display:flex;gap:8px}.scene{flex:1;min-width:0;padding:9px;border:1px solid var(--line,rgba(27,109,152,.18));border-radius:12px;background:var(--surface,rgba(255,255,255,.66))}.scene h3{font-size:11px;margin:0 0 3px;color:var(--accent,#1b6d98)}.scene p{font-size:11px;line-height:1.45;margin:0;color:var(--muted,#476176)}
.highlight-block{padding:12px 14px;border-left:4px solid var(--accent2,#ec6b43);border-radius:0 12px 12px 0;background:var(--surface,rgba(255,255,255,.66))}.highlight-block h2{font-size:13px}.highlight-block p{font-size:11px}
.layout-poster .page-content-stack{min-height:88%;justify-content:flex-end;padding:30px 24px}.layout-poster .eyebrow{order:-2}.layout-poster h1{font-size:38px;line-height:1.08;max-width:92%;margin-bottom:auto;padding-top:12px}.layout-poster .content-block{border-left-width:5px}.layout-poster.page-cover:before{content:"";position:absolute;inset:82px 22px 92px;border:1px solid var(--accent,#1b6d98);opacity:.18;transform:rotate(-2deg)}
...
.layout-data .page-content-stack{justify-content:flex-start}.layout-data h1{font-size:25px}.layout-data .content-block:has(.stat-row),.layout-data .compare-block{padding:12px;border:1px solid var(--line,rgba(27,109,152,.18));background:color-mix(in srgb,var(--surface,#fff) 88%,var(--accent,#1b6d98))}.layout-data .stat-row{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.layout-data .stat{text-align:left}.layout-data .stat b{font-size:25px}
.layout-checklist .page-content-stack{justify-content:flex-start}.layout-checklist h1{font-size:29px}.layout-checklist .list-block ul{counter-reset:item;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.layout-checklist .page li{min-height:62px;padding:12px 10px 10px 34px;display:flex;align-items:center}.layout-checklist .page li:before{counter-increment:item;content:counter(item);width:18px;height:18px;left:9px;top:50%;transform:translateY(-50%);display:grid;place-items:center;color:var(--page,#fff);font-size:9px;font-weight:800}
...
.layout-steps .page-content-stack{justify-content:flex-start}.layout-steps h1{font-size:27px}.layout-steps .step-col{gap:11px}.layout-steps .step{display:grid;grid-template-columns:38px 1fr;gap:12px;padding-bottom:10px;border-bottom:1px solid var(--line,rgba(27,109,152,.18))}.layout-steps .step>b{width:36px;height:36px;font-size:15px}.layout-steps .step h3{font-size:13px}.layout-steps .tl-node{padding-left:18px;padding-bottom:13px}.layout-steps .tl-time{font-size:11px}
...
export async function runSocialCardPipeline({ gateway, store, batchId, candidateId, provider, workspaceRoot, snapshotId=null, onProgress=()=>{} }) {
  ...
  record('planning', storyboardSkillId, planPath);
  ...
  const renderCurrentStoryboard=()=>renderStoryboardHtml({ topic:candidate.hotspot_title, repository:facts?.data?.repository, pages:cardPlan, visualStyle:editorial.visual_style, layoutStyle:editorial.layout_style, compositionMode:editorial.composition_mode||'template',
    compositionSeed:`${candidate.batch_id}|${candidate.id}`,forceSafeComposition:safeCompositionApplied?(safeCompositionPages.size?[...safeCompositionPages]:true):false,contentType,channelMode,sourceLabel:contentType==='event'?'事件专题':contentType==='custom'?facts?.data?.content_type_label||'自定义':'',disclosure:contentType==='event'?'据公开素材整理 · 未核实内容已标注':'' });
  ...
  for (let attempt=0; attempt<maxLayoutAttempts; attempt += 1) {
    ...
    if (attempt === maxLayoutAttempts-1) throw new Error(`布局审计 ${maxLayoutAttempts} 轮后仍未通过：${report.pages.filter((page)=>!page.valid).map((page)=>`P${page.page} ${page.issues.join('/')}`).join('；')}`);
    repairCount += 1;
  ...
  const compositionNotes=editorial.composition_mode==='smart'
    ...
    : [];
  record('layout-audit', generator.skillName, reportPath, `安全变体：${safeCompositionApplied?safeCompositionPages.size?`已启用（${[...safeCompositionPages].map((index)=>`P${index+1}`).sort().join('、')}）`:'已启用':'未触发'}；内容修复轮次：${repairCount}${compositionNotes.length?`；${compositionNotes.join('、')}`:''}`);
