...
import { applySocialCardRestructureOperations, buildDeterministicSocialCardPageCapOperations, buildDeterministicSocialCardRestructureOperations, cardPlanHash, structuralLayoutPages } from '../rendering/social-card-repair-policy.mjs';
import { applySocialCardContentPlannerOperations, applySocialCardContentPlannerOperationsPartial, buildSocialCardContentPlannerPrompt, validateSocialCardContentPlannerSchema } from '../rendering/social-card-content-planner.mjs';
...
import { buildSocialCardComponentPackingOperations, buildSocialCardContentComponents, buildSocialCardContinuationPackingOperations, sanitizeSocialCardPlanFactBindings, validateSocialCardContentComponents } from '../rendering/social-card-content-components.mjs';
...
export { applySocialCardRestructureOperations, buildDeterministicSocialCardPageCapOperations, buildDeterministicSocialCardRestructureOperations, cardPlanHash, classifySocialCardLayoutIssue, structuralLayoutPages, validateSocialCardRestructureOperations } from '../rendering/social-card-repair-policy.mjs';
export { applySocialCardContentPlannerOperations, applySocialCardContentPlannerOperationsPartial, buildSocialCardContentPlannerPrompt, validateSocialCardContentPlannerOperations, validateSocialCardContentPlannerSchema } from '../rendering/social-card-content-planner.mjs';
...
export function renderStoryboardHtml({ topic, repository, pages, visualStyle='ice-blue', themeDefinition:providedTheme=null, layoutStyle='auto', compositionMode='template', compositionSeed='', forceSafeComposition=false, relaxedDensityPages=false, expandedDensityPages=false, contentType='repository', sourceLabel='', disclosure='', channelMode='wechat', coverTitleLines=null, templatePackOverride='', templateCssOverride='' }) {
  ...
  const sections=templateContext.pack.id==='neon-v1'
    ? renderNeonStoryboardSections({topic,repository,pages,compiledTheme,compositionMode,compositionSeed,forceSafeComposition,relaxedDensityPages,expandedDensityPages,contentType,sourceLabel,disclosure,channelMode,coverTitleLines})
    : templateContext.pack.id==='brutalist-v1'
      ? renderBrutalistStoryboardSections({topic,repository,pages,compositionMode,compositionSeed,forceSafeComposition,relaxedDensityPages,expandedDensityPages,contentType,sourceLabel,disclosure,channelMode,coverTitleLines})
      : templateContext.pack.id==='editorial-v1'
      ? renderEditorialStoryboardSections({topic,repository,pages,compositionMode,compositionSeed,forceSafeComposition,relaxedDensityPages,expandedDensityPages,contentType,sourceLabel,disclosure,channelMode,coverTitleLines})
      : templateContext.pack.id==='clean-v1'
          ? renderCleanStoryboardSections({topic,repository,pages,compiledTheme,compositionMode,compositionSeed,forceSafeComposition,relaxedDensityPages,expandedDensityPages,contentType,sourceLabel,disclosure,channelMode,coverTitleLines})
          : renderStoryboardSections({topic,repository,pages,compiledTheme,layoutStyle,compositionMode,compositionSeed,forceSafeComposition,relaxedDensityPages,expandedDensityPages,contentType,sourceLabel,disclosure,channelMode,coverTitleLines,templatePackId,templatePack:customTemplatePack});
...
// 页面上限合并需要逐步执行。批量提交多个 merge_pages 时，通用校验器会把
// 中间状态仍高于上限判为失败，导致本来可以连续合并的续页一个都没合并。
function applyDeterministicPageCap(cardPlan, maxPages, capacityProfile = null) {
  let pages = structuredClone(Array.isArray(cardPlan) ? cardPlan : []);
  const operations = [];
  while (pages.length > Number(maxPages)) {
    const candidates = buildDeterministicSocialCardPageCapOperations(pages, {
      maxPages: pages.length - 1,
      canMerge: ({ mergedPage }) => {
        const capacity = capacityProfile?.roles?.[String(mergedPage?.role || '')] || null;
        return capacity ? !estimateSocialCardPageLoad(mergedPage, capacity).overCapacity : true;
      },
    });
    const operation = candidates[0];
    if (!operation) break;
    const result = applySocialCardRestructureOperations(pages, [operation], { maxPages: pages.length - 1, maxOperations: 1 });
    if (!result.valid || !result.changed) break;
    pages = result.pages;
...
function templateAuditFailurePayload({ requestedTemplate, renderedTemplate, auditAttempts, report, repairCount, safeCompositionPages, relaxedDensityPages, expandedDensityPages, maxLayoutAttempts }) {
  return {
    schemaVersion: 1,
...
export async function runSocialCardPipeline({ gateway, store, batchId, candidateId, provider, workspaceRoot, snapshotId=null, onProgress=()=>{} }) {
  ...
  const maxPagesForContent = contentType === 'repository' ? 7 : 10;
  ...
  const reflowResult = compileTemplateAwareCardPlan({
    ...
    maxPages: maxPagesForContent,
  });
  let cardPlan = reflowResult.pages;
  const pageCapResult = cardPlan.length > maxPagesForContent
    ? applyDeterministicPageCap(cardPlan, maxPagesForContent, templateCapabilities.capacityProfile)
    : { pages: cardPlan, operations: [] };
  let pageCapOperations = pageCapResult.operations;
  cardPlan = pageCapResult.pages;
  ...
  if (reflowResult.changed || pageCapOperations.length) persistEffectiveCardPlan(cardPlan, 'AI_READY');
  ...
  const planningMeta = { schemaVersion: 3, channel_mode:outputMode||editorial.output_mode || 'xiaohongshu', composition_mode:editorial.composition_mode||'template', layout_style:editorial.layout_style||'auto', template:{pack:templateCompatibility.templatePack.id,version:templateCompatibility.templatePack.version,source:templateCompatibility.source,fallback:templateCompatibility.fallback}, capacityProfileVersion:templateCapabilities.capacityProfileVersion, capacityProfile:templateCapabilities.capacityProfile, rolloutProfile, topic:candidate.hotspot_title, source_page_count: originalCardPlan.length, reflow: { changed: reflowResult.changed || pageCapOperations.length > 0, operations: [...reflowResult.operations, ...pageCapOperations], warnings: reflowResult.warnings, unresolved: reflowResult.unresolved, history: reflowHistory }, pages:cardPlan };
  writeFile(planPath, JSON.stringify(planningMeta, null, 2));
  record('planning', storyboardSkillId, planPath, reflowResult.changed ? `模板感知重排：${reflowResult.operations.map((item)=>item.op==='split_block'?`P${item.page} ${item.blockType} 拆为 ${item.createdChunks} 页`:item.op==='merge_pages'?`P${item.pages.join('/')} 合并过短续页`:item.op==='move_block'&&item.from_page?`P${item.from_page} ${item.blockType} 移至 P${item.to_page}`:`P${item.page} 移动 ${item.blockType}`).join('；')}${reflowResult.warnings.length?`；${reflowResult.warnings.join('；')}`:''}` : '模板容量预检通过，未触发续页');
  ...
  for (let attempt=0; attempt<maxLayoutAttempts; attempt += 1) {
    ...
    auditAttempts.push({ attempt: attempt + 1, template: { id: templateCompatibility.templatePack.id, version: templateCompatibility.templatePack.version, source: templateCompatibility.source }, valid: report.valid === true, densityCalibration, pages: layoutAuditPageSummary(report) });
    persistPlanBaseline();
    ...
    const structuralFallbackOperations=structuralPages.length
      ? buildDeterministicSocialCardRestructureOperations(cardPlan,structuralPages,{maxPages:maxPagesForContent})
      : [];
    if(structuralPages.length&&!structuralReflowAttempted){
      ...
      const structuralReflow=compileTemplateAwareCardPlan({cardPlan,capacityProfile:conservativeProfile,maxPages:maxPagesForContent});
      if(structuralReflow.changed&&cardPlanHash(structuralReflow.pages)!==cardPlanHash(cardPlan)){
        ...
        const capResult=reflowPages.length>maxPagesForContent
          ? applyDeterministicPageCap(reflowPages,maxPagesForContent,templateCapabilities.capacityProfile)
          : { pages: reflowPages, operations: [] };
        const appliedCapOperations=capResult.operations;
        reflowPages=capResult.pages;
    ...
    const canFitFactBlock=({page,block,role})=>{
      ...
      // 普通页留出模板边框、字体渲染和浏览器舍入误差的余量；已经拆出的
      // 明显偏空续页允许一档有限静态余量，最终仍由浏览器布局硬门禁裁决。
      const safeFactor=isUnderfilledContinuation?1.08:0.92;
      const withinStaticLimit=Boolean(bodyHeight)&&estimate.estimatedHeightPx<=bodyHeight*safeFactor;
      return withinStaticLimit&&(isUnderfilledContinuation||!estimate.overCapacity);
    };
    const factSupplementCapacityGuard=({operation,pages})=>{
      ...
      return page&&canFitFactBlock({page,block:operation.block,role})
        ? []
    ...
    const factSupplementFallbackOperations=structuralPages.length===0
      ...
      : [];
    // 只在没有未解决的结构问题、且页数已在上限内时调整内容计划。
    // 结构问题优先于内容补充，避免“补事实→溢出→拆页→再补事实”。
    const plannerEligible=plannerPages.length>0&&structuralPages.length===0&&cardPlan.length<=maxPagesForContent;
    ...
    if(plannerEligible&&contentPlanAdjustmentCount<rolloutProfile.maxPlanRounds&&contentPlanAdjustmentAttemptCount<maxContentPlanAttempts&&!contentPlanAdjustmentSignatures.has(plannerSignature)){
      ...
      try{
          const plannerPrompt=buildSocialCardContentPlannerPrompt({facts:plannerFacts,layoutReport:{pages:plannerPages},cardPlan,contentAtoms:currentAtoms.atoms,templateCapabilities:templateCapabilities.capacityProfile,factIndex,contentComponents,maxPages:maxPagesForContent,maxOperations:rolloutProfile.maxOperationsPerRound,maxFactBlocksPerRound,maxFactBlocksPerPage});
        const plannerResult=await gateway.complete({provider,purpose:'social-card-content-planner',batchId,candidateId,jsonMode:true,maxOutputTokens:Math.min(6000,providerConfig.maxOutputTokens),messages:[
          {role:'system',protected:true,content:`${plannerPrompt}\n\n当前运行阶段：浏览器审计后的内容计划调整。只返回契约规定的 JSON。`},
          {role:'user',protected:true,content:JSON.stringify({facts:plannerFacts,fact_candidates:JSON.parse(buildSocialCardFactCandidatePrompt(factIndex)),content_components:contentComponents,card_plan:cardPlan,content_atoms:currentAtoms.atoms,layout_report:{pages:plannerPages},template_capabilities:templateCapabilities.capacityProfile})},
        ]});
        ...
        if(!schemaResult.valid){
          ...
          applied=applySocialCardContentPlannerOperations(cardPlan,plannerRound.operations,{maxPages:maxPagesForContent,maxOperations:rolloutProfile.maxOperationsPerRound,maxFactBlocksAdded:maxFactBlocksPerRound,maxFactBlocksPerPage,contentAtoms:currentAtoms.atoms,supplementSlots,factIndex,contentComponents,operationGuard:contentPlanOperationGuard});
        ...
        if((!applied.valid||!applied.changed)&&structuralFallbackOperations.length){
          const fallback=applySocialCardRestructureOperations(cardPlan,structuralFallbackOperations,{maxPages:maxPagesForContent});
        ...
        if((!applied.changed)&&factSupplementFallbackOperations.length){
          const fallback=applySocialCardRestructureOperations(cardPlan,factSupplementFallbackOperations,{maxPages:maxPagesForContent,maxFactBlocksAdded:maxFactBlocksPerRound,maxFactBlocksPerPage,knownSourceRefs:factIndex.candidates.flatMap((item)=>item.source_refs||[]),supplementSlots,factIndex,operationGuard:contentPlanOperationGuard});
          if(fallback.valid&&fallback.changed){
            applied=fallback;
        ...
        if(applied.changed){
          ...
          const compiled=compileTemplateAwareCardPlan({cardPlan:applied.pages,capacityProfile:templateCapabilities.capacityProfile,maxPages:maxPagesForContent});
          ...
          if(splitTriggered){
            ...
            if(continuationCandidates.length){
              ...
              if(packed.valid&&packed.changed){
                ...
                const repacked=compileTemplateAwareCardPlan({cardPlan:packed.pages,capacityProfile:templateCapabilities.capacityProfile,maxPages:maxPagesForContent});
    ...
    if(report.valid && densityCalibration.pages.length){
    ...
    if(structuralPages.length&&!structuralFallbackOperations.length){
      onProgress(`结构问题暂不拆页：${cardPlan.length>=maxPagesForContent?'已达到页面上限':'目标页没有可安全拆分的完整条目'}，转入受控文字缩写修复`);
    }
    ...
    if (attempt === maxLayoutAttempts-1 || repairCount >= rolloutProfile.textRepairMaxRounds) {
      ...
      const failurePayload=templateAuditFailurePayload({ requestedTemplate, renderedTemplate, auditAttempts, report, repairCount, safeCompositionPages, relaxedDensityPages, expandedDensityPages, maxLayoutAttempts });
      ...
      throw new Error(`模板 ${requestedTemplate.id} v${requestedTemplate.version} 严格渲染未通过，未自动回退到 standard-v1。${layoutAuditFailureMessage(report,auditAttempts.length)}`);
    }
    repairCount += 1;
  ...
  return { workdir, copy:copyPath, html:htmlPath, layoutReport:reportPath, deliveryReport:deliveryPath, templateMetrics:templateMetricsPath, contentPlanAdjustments:contentPlanAdjustmentsPath, theme:{id:themeDefinition.id,version:themeDefinition.version,hash:themeDefinition.hash}, template:{pack:templateCompatibility.templatePack,source:templateCompatibility.source,fallback:templateCompatibility.fallback,warnings:templateCompatibility.warnings,capacityProfileVersion:templateCapabilities.capacityProfileVersion,capacityProfile:templateCapabilities.capacityProfile,rolloutProfile}, themeSnapshot:themeSnapshotPath, images, pageCount:images.length };
}
