...
import { fetchMaterialSource } from '../source-fetcher.mjs';
...
function parseJson(content){return JSON.parse(String(content||'').trim().replace(/^```(?:json)?\s*/i,'').replace(/\s*```$/,''));}
...
function markdown(analysis,sources){
  ...
  for(const item of facts.confirmedFacts)lines.push(`- ${item.claim}（来源 ${item.sourceIds.join('、')||'待补'}；置信度 ${item.confidence}）`);
  lines.push('','## 尚未核实的主张','');for(const item of facts.claims)lines.push(`- ${item.speaker?`${item.speaker}：`:''}${item.claim}（${item.status}；来源 ${item.sourceIds.join('、')||'待补'}）`);
  lines.push('','## 时间线','');for(const item of facts.timeline)lines.push(`- ${item.time||'时间待核'}：${item.event}（来源 ${item.sourceIds.join('、')||'待补'}）`);
...
export async function runBreakingAnalysisPipeline({gateway,store,batchId,provider,workspaceRoot,onProgress=()=>{}}){
  ...
  for(let attempt=0;attempt<2;attempt+=1){
    result=await gateway.complete({provider,purpose:'breaking-analysis',batchId,jsonMode:true,maxOutputTokens:Math.min(7000,providerConfig.maxOutputTokens),messages:[
      ...
      {role:'user',content:`${attempt?'上次输出疑似误用了 1～5 分制。请严格使用各字段完整分值区间重新评估；推荐生产与极低维度总分不能同时出现。\n\n':''}${JSON.stringify({eventTitle:hotspot.title,authorContext:batch.note,requestedTracks:batch.requested_tracks_list,sources:sourceInput})}`,protected:true},
    ]});
    ...
    const articleContradiction=articlePreview.baseScore<30&&String(raw.article?.recommendedType||'')!=='暂不生产'&&array(raw.article?.reasons).length>=2;
    const socialContradiction=socialPreview.baseScore<30&&String(raw.social?.recommendedFormat||'')!=='暂不生产'&&array(raw.social?.storyOutline).length>=4;
