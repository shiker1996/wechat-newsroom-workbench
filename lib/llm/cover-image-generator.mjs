...
约束：
- canvas 与 title 必选；title 的 lines 最多 ${COVER_LIMITS.titleLines} 行、每行 ≤${COVER_LIMITS.lineChars} 字（断行要在语义自然处），**lines 拼接后必须与文章标题一字不差，只允许换行位置不同，不得改写、删减或增补**；highlights 是最多 ${COVER_LIMITS.highlights} 个标题原文里的关键词（原样摘录，用于换强调色）；
...
${themeList}`;
  try {
    ...
    const validation = validateCoverSpec(parsed, title);
    if (!validation.ok) {
...
export async function runCoverImageJob({ gateway, store, batchId, candidateId, provider, workspaceRoot, theme = '', onProgress = () => {} }) {
  ...
  // 封面标题以正文 H1（实际发布的文章原题）为准，title 字段可能与之刻意不同
  const h1 = String(finalDoc?.content || '').split('\n').find((line) => line.startsWith('# '))?.slice(2).trim();
  const title = h1 || String(finalDoc?.title || candidate?.hotspot_title || '').trim() || (daily ? '批次早报' : '未命名文章');
