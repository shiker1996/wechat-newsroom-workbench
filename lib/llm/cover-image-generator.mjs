...
export async function generateCoverImage({ workspaceRoot, workdir, gateway, accountContext, title, summary = '', brand = '', themeId = '', provider = '', log = () => {} }) {
  const registry = getBuiltinThemeRegistry();
  const themes = registry.list({ target: 'cover' }).map((theme) => ({ id: theme.id, label: theme.label, description: theme.description }));
  const resolvedTitle = String(title || '').trim() || '未命名文章';

  let theme = themeId ? registry.get(themeId) : null;
  if (theme && !theme.targets.includes('cover')) theme = null;
  ...
  if (planned?.themeId && !theme) {
    const picked = registry.get(planned.themeId);
    if (picked?.targets.includes('cover')) theme = picked;
  }
...
export async function runCoverImageJob({ gateway, store, batchId, candidateId, provider, workspaceRoot, theme = '', onProgress = () => {} }) {
  ...
  const result = await generateCoverImage({
    workspaceRoot, workdir, gateway, accountContext, title, summary, brand,
