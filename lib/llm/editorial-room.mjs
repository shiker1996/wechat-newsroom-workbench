...
import { normalizeDistributionLane, resolveDistributionDecision } from '../domain/distribution-strategy.mjs';
...
分发与读者利益规则:
- 通知池允许为空。只有具体读者、明确动作或决策、具体后果三者齐备，通知适配与事实支持达到账号门槛，且风险等级未被禁止时才能保留。
- “影响职业方向”“影响技术选择”“影响工作环境”等泛化利益不合格；必须改写成谁在什么时间或场景需要做什么，否则会发生什么。
- 传闻、未证实重大事件、健康或生物安全题默认进入实验池；需要补证时使用 RESEARCH_FIRST，不得靠标题张力晋级。
...
async function requestMessages(current,answer,events=[],retrieve=null,workspaceRoot) {
  ...
  return [{role:'system',content:editorialSystem(workspaceRoot)+(webSearchContext?'  '+webSearchContext:''),protected:true},{role:'user',protected:true,content:JSON.stringify({
    candidate:{
      ...
      angle:current.angle,thesis:current.thesis,distributionLane:current.distribution_lane,
      readerStake:current.reader_stake,composite:Boolean(current.composite)
    },
    events:eventInputs,
...
function applyResult({store,candidateId,current,parsed,result}) {
  ...
  if(latest?.status==='locked'||latest?.editorial?.brief_status==='LOCKED'){
    ...
    return {candidate:store.getCandidate(candidateId),editorial:latest.editorial,usage:result.usage,model:result.model,reply,fetchEvents:[],ignoredBecauseLocked:true};
  }
  ...
  const requestedLane=normalizeDistributionLane(candidateUpdates.distribution_lane??current.distribution_lane);
  const readerStake=String(candidateUpdates.reader_stake??current.reader_stake??'').trim();
  const angle=String(candidateUpdates.angle??current.angle); const thesis=String(candidateUpdates.thesis??current.thesis);
  const distribution=resolveDistributionDecision({distributionLane:requestedLane,readerStake,
    notificationFit:candidateUpdates.notification_fit,factSupport:candidateUpdates.fact_support,
    notificationReason:candidateUpdates.notification_reason,riskLevel:current.risk_level,
    riskReason:current.risk_reason,title:current.hotspot_title,angle,thesis,
    evidenceBoundary:updates.forbidden_claims,materialGaps:updates.open_questions});
  store.updateCandidate(candidateId,{angle,thesis,
    distribution_lane:distribution.distributionLane,reader_stake:readerStake});
  if(reply)store.addEditorialMessage(candidateId,'assistant',reply);
...
export async function runEditorialTurnStream({gateway,store,candidateId,provider,answer='',webSearch=false,events=[],retrieve=null,workspaceRoot,onText=()=>{},onThinking=()=>{}}) {
  const candidate=store.getCandidate(candidateId);if(!candidate)throw new Error('候选不存在');
  if(candidate.editorial.brief_status==='LOCKED')throw new Error('简报已经锁定;如需改方向,请先建立新候选而不是暗改锁定决策');
  if(answer.trim())store.addEditorialMessage(candidateId,'user',answer.trim());
