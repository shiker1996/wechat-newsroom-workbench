...
async function requestMessages(current,answer,events=[],retrieve=null) {
...
export async function runEditorialTurn({gateway,store,candidateId,provider,answer='',webSearch=false,events=[],retrieve=null}) {
  ...
  const result=await gateway.complete({provider,purpose:'editorial-room',batchId:candidate.batch_id,candidateId,maxOutputTokens:Math.min(3500,providerConfig.maxOutputTokens),jsonMode:true,webSearch,messages:await requestMessages(current,answer,events,retrieve)});
  return applyResult({store,candidateId,current,parsed:parseResult(result,store),result});
}

export async function runEditorialTurnStream({gateway,store,candidateId,provider,answer='',webSearch=false,events=[],retrieve=null,onText=()=>{}}) {
  const candidate=store.getCandidate(candidateId);if(!candidate)throw new Error('候选不存在');
  if(candidate.editorial.brief_status==='LOCKED')throw new Error('简报已经锁定;如需改方向,请先建立新候选而不是暗改锁定决策');
  if(answer.trim())store.addEditorialMessage(candidateId,'user',answer.trim());
  ...
  const result=await gateway.streamComplete({provider,purpose:'editorial-room',batchId:candidate.batch_id,candidateId,maxOutputTokens:Math.min(3500,providerConfig.maxOutputTokens),jsonMode:true,webSearch,messages:await requestMessages(current,answer,events,retrieve)},
