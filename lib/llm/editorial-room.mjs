...
- sources:该事件下各热点报道的原文快照,是按需深挖事件 why/how 细节的补充材料;事件卡已给出 what 层面的事实基座,status 为 missing 表示该来源尚未抓取,这是常态而非缺陷,不得仅因 sources 缺失就中断讨论或要求补研究。
...
assistantReply 先用一两句话概括当前事实基座与本轮新确定的决策,再说明为何要问下一个问题;不要把事件卡和来源里已有的事实说成"未确认"。nextQuestion只能有一个问题;若next_action为WRITE_NOW或DROP则必须为空。只有作者观点、角度与命题边界明确且事实基座齐备时才能WRITE_NOW。公共资料分析可以experience_required=false,但必须禁止第一人称亲测。不要输出JSON之外的文字。`;

function requestMessages(current,answer,events=[]) {
  ...
  const eventInputs=(events||[]).map((event)=>({
    ...
    sources:(event.hotspots||[]).map((hotspot)=>{
      const doc=hotspot.sourceDoc;
      return doc?{ status:doc.status, url:doc.final_url||doc.url||hotspot.url, title:doc.title||hotspot.title,
...
export async function runEditorialTurn({gateway,store,candidateId,provider,answer='',webSearch=false,events=[]}) {
  ...
  const result=await gateway.complete({provider,purpose:'editorial-room',batchId:candidate.batch_id,candidateId,maxOutputTokens:Math.min(3500,providerConfig.maxOutputTokens),jsonMode:true,webSearch,messages:requestMessages(current,answer,events)});
  return applyResult({store,candidateId,current,parsed:parseResult(result,store),result});
}

export async function runEditorialTurnStream({gateway,store,candidateId,provider,answer='',webSearch=false,events=[],onText=()=>{}}) {
  const candidate=store.getCandidate(candidateId);if(!candidate)throw new Error('候选不存在');
  if(candidate.editorial.brief_status==='LOCKED')throw new Error('简报已经锁定;如需改方向,请先建立新候选而不是暗改锁定决策');
  if(answer.trim())store.addEditorialMessage(candidateId,'user',answer.trim());
  ...
  const result=await gateway.streamComplete({provider,purpose:'editorial-room',batchId:candidate.batch_id,candidateId,maxOutputTokens:Math.min(3500,providerConfig.maxOutputTokens),jsonMode:true,webSearch,messages:requestMessages(current,answer,events)},
