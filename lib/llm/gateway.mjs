...
export class ModelGateway {
  ...
  async rawComplete({ providerName, provider, apiKey, messages, maxOutputTokens, temperature = 0.2, jsonMode = false, webSearch = false }) {
    ...
    try {
      ...
      for(const useJsonMode of modes) {
        const payload = { model: provider.model, messages: messages.map(({ role, content }) => ({ role, content })), temperature };
  ...
  async rawStreamComplete({ providerName, provider, apiKey, messages, maxOutputTokens, temperature = 0.2, jsonMode = false, webSearch = false, onDelta = () => {} }) {
    ...
    try {
      const modes=jsonMode&&provider.supportsJsonMode===true?[true,false]:[false];
      for(const useJsonMode of modes) {
        const payload={model:provider.model,messages:messages.map(({role,content})=>({role,content})),temperature,stream:true};
  ...
  async complete(input) {
    ...
    const budget = contextBudget(provider, this.config, input.maxOutputTokens);
    let context;
    ...
    try {
      ...
      const result = await this.rawComplete({ providerName, provider, apiKey, messages: context.messages,
        maxOutputTokens: input.maxOutputTokens, temperature: input.temperature, jsonMode: input.jsonMode });
      const callId = this.store.recordModelCall({ provider: providerName, model: provider.model, purpose: input.purpose,
        ...
        promptTokens: Number(result.usage.prompt_tokens || 0) + compressionUsage.prompt_tokens,
        completionTokens: Number(result.usage.completion_tokens || 0) + compressionUsage.completion_tokens,
        compressed: context.compressed, latencyMs: Date.now() - started, status: 'completed' });
      return { ...result, callId, usage: { ...result.usage, compression: compressionUsage }, provider: providerName, model: provider.model, context };
    } catch (error) {
  ...
  async streamComplete(input,onDelta=()=>{}) {
    ...
    const budget=contextBudget(provider,this.config,input.maxOutputTokens);let context;
    const compressionUsage={prompt_tokens:0,completion_tokens:0,calls:0};
    try {
      const webSearch=input.webSearch===true;
      if(webSearch&&!provider.webSearchConfig&&this.config.tavily?.enabled){
      ...
      const result=await this.rawStreamComplete({providerName,provider,apiKey,messages:context.messages,maxOutputTokens:input.maxOutputTokens,
      ...
      const callId=this.store.recordModelCall({provider:providerName,model:provider.model,purpose:input.purpose,batchId:input.batchId,candidateId:input.candidateId,
        ...
        latencyMs:Date.now()-started,status:'completed'});
      ...
      this.store.recordModelCall({provider:providerName,model:provider.model,purpose:input.purpose,batchId:input.batchId,candidateId:input.candidateId,
        estimatedInputTokens:context?.afterTokens??estimateTokens(input.messages),compressed:context?.compressed,latencyMs:Date.now()-started,status:'failed',error:error.message});throw error;
    }
  }
}
