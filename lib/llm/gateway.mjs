  ...
  async streamComplete(input,onDelta=()=>{},onThinking=()=>{}) {
    ...
    try {
      ...
      const callId=this.store.recordModelCall({provider:providerName,model:provider.model,purpose:input.purpose,batchId:input.batchId,candidateId:input.candidateId,
        ...
        latencyMs:Date.now()-started,status:'completed',outputText:String(result.content??'').slice(0,20000),outputBudget:budgetAudit(input,provider,outputBudget,thinking,thinkingReserve),generationSnapshotId:input.generationSnapshotId});
