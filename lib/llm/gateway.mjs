...
import { applyModelProviderConfiguration, legacyModelProviderConfiguration } from '../extensions/model-provider-configuration.mjs';
...
export class ModelGateway {
  ...
  configuredProvider(name) {
    ...
    const fallback=legacyModelProviderConfiguration(legacy);
    const state=this.configurationResolver?this.configurationResolver(name,legacy):{configured:Boolean(fallback.apiKey),values:fallback};
    return {provider:applyModelProviderConfiguration(legacy,state.values||fallback),apiKey:state.values?.apiKey||fallback.apiKey||'',configured:state.configured!==false&&Boolean(state.values?.apiKey||fallback.apiKey)};
  }
  ...
  async rawStreamComplete({ providerName, provider, apiKey, messages, maxOutputTokens, temperature = 0.2, jsonMode = false, webSearch = false, onDelta = () => {}, thinking }, onThinking = () => {}) {
    ...
    try {
      ...
      for(const useJsonMode of modes) {
        ...
        if(finishReason==='content_filter')throw new Error(`${provider.label||providerName} 输出触发内容过滤，未返回内容`);
        if(finishReason==='insufficient_system_resource')throw new Error(`${provider.label||providerName} 服务端推理资源不足，生成被打断，请稍后重试`);
        if(!content)throw new Error(`${provider.label||providerName} 未返回流式文本内容（finish=${finishReason||'unknown'}${reasoning?`，推理已产生 ${reasoning.length} 字符后内容为空`:''}）`);
    ...
    try {
      ...
      try {
        result = await this.rawCompleteMaybeStream({ providerName, provider, apiKey, messages: context.messages,
          maxOutputTokens: outputBudget.initial + thinkingReserve, temperature: input.temperature, jsonMode: input.jsonMode, thinking }, { streamThinking });
      } catch (error) {
        // thinking 开启时推理可能吃光 max_tokens 导致内容为空（finish=length）：回落无思考重试一次
        if (thinking && /未返回流式文本内容/.test(String(error.message || ''))) {
          result = await this.rawCompleteMaybeStream({ providerName, provider, apiKey, messages: context.messages,
            maxOutputTokens: outputBudget.initial, temperature: input.temperature, jsonMode: input.jsonMode, thinking: false }, { streamThinking: false });
        } else {
          throw error;
        }
      }
      ...
      if (result.finishReason === 'length' && outputBudget.adaptive) {
        ...
        result = await this.rawCompleteMaybeStream({ providerName, provider, apiKey, messages: retryContext.messages,
          maxOutputTokens: outputBudget.retry + thinkingReserve, temperature: input.temperature, jsonMode: input.jsonMode, thinking }, { streamThinking });
      }
    ...
    try {
      ...
      try {
        result=await this.rawStreamComplete({providerName,provider,apiKey,messages:context.messages,maxOutputTokens:outputBudget.initial+thinkingReserve,
          temperature:input.temperature,jsonMode:input.jsonMode,webSearch,onDelta,thinking},onThinking);
      } catch (error) {
        // thinking 开启时推理可能吃光 max_tokens 导致内容为空（finish=length）：回落无思考重试一次
        if (thinking && /未返回流式文本内容/.test(String(error.message || ''))) {
          result=await this.rawStreamComplete({providerName,provider,apiKey,messages:context.messages,maxOutputTokens:outputBudget.initial,
            temperature:input.temperature,jsonMode:input.jsonMode,webSearch,onDelta,thinking:false},()=>{});
        } else {
          throw error;
        }
      }
      ...
      const callId=this.store.recordModelCall({provider:providerName,model:provider.model,purpose:input.purpose,batchId:input.batchId,candidateId:input.candidateId,
        ...
        completionTokens:Number(result.usage.completion_tokens||0)+compressionUsage.completion_tokens,reasoningTokens:reasoningTokens||null,compressed:context.compressed,
