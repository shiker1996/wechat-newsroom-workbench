...
import { formatAccountContext, getAccountContext } from '../account-context.mjs';
...
export function dimensionSelections(clusters, ranking = [], { whoLimit = 3, whatLimit = 3, whereLimit = 2, minWhoEvents = 2 } = {}) {
  ...
  const whoGroups = [...byWho.entries()]
    ...
    .map(([who, events]) => {
      ...
      const score = Math.min(100, Math.round(Math.max(...events.map(preScore)) + Math.min(events.length - 1, 3) * 4 + tension));
  ...
  const whatScore = (events) => {
    ...
    return Math.min(100, Math.round(Math.max(...events.map(preScore)) + Math.min(whos, 4) * 3 + tierBonus + Math.min(disagreements(events), 3) * 2));
  };
  for (const [action, events] of byAction) {
    if (distinctWhos(events).size < 2) continue;
  ...
  const whereGroups = [...byOccasion.entries()]
    ...
    .map(([occasion, events]) => {
      const whos = distinctWhos(events).size;
      const score = Math.min(100, Math.round(Math.max(...events.map(preScore)) + Math.min(events.length, 4) * 2 + Math.min(whos, 4) * 2));
...
// 维度分 = 结构分 + 账号契合加分（命中 contentPillars 对应类目 +6），按分数全局混排取核心 8，
// 黑马 2 仍按 blackHorseSignals 选择，候补 3 条保留。事件级排名回填入池身份与淘汰原因。
export function selectDimensionPool(clusters, ranking, { coreLimit = 8, blackLimit = 2, backupLimit = 3 } = {}) {
  ...
  const scored = groups.map((group) => {
    ...
    const accountFit = focused.has(category) ? 6 : 0;
    return { ...group, category, accountFit, score: Math.min(100, group.score + accountFit) };
  }).sort((a, b) => b.score - a.score);
  const core = scored.slice(0, coreLimit).map((group) => ({ ...group, poolRole: DIMENSION_POOL_ROLES[group.dimension] }));
  const remaining = scored.slice(coreLimit);
  ...
  const backup = remaining.filter((group) => !black.includes(group)).slice(0, backupLimit)
  ...
  return { selected: [...core, ...black], backup, groups: scored };
}
...
export function markdownRanked(scored, synthesis, dimensionGroups = []) {
...
export async function generateEventCards({ gateway, store, clusters, batchId, provider, onProgress = () => {} }) {
  const providerConfig = gateway.config.providers[provider || gateway.config.defaultProvider];
  const cards = new Map();
  const failed = [];
  ...
  async function processChunk(chunk, label, retry = false) {
    ...
    const result = await gateway.complete({ provider, purpose: 'event-card', batchId, jsonMode: true,
      ...
      messages: [
        { role: 'system', content: EVENT_CARD_SYSTEM, protected: true },
    ...
    for (const event of chunk) {
      if (!returned.has(event.event_id)) failed.push({ event_id: event.event_id, error: '模型未返回该事件的事件卡' });
    }
...
export async function ensureBatchEventCards({ gateway, store, batchId, provider, workspaceRoot, maxAgeHours = 168, regenerate = false, onProgress = () => {} }) {
  ...
  const clusters = clusterItems(tagged);
  ...
  const missing = clusters.filter((event) => !cachedCards.has(event.event_id));
  let failed = [];
  if (missing.length) {
    const result = await generateEventCards({ gateway, store, clusters: missing, batchId, provider, onProgress });
...
export async function runResearchPipeline({ gateway, store, batchId, provider, workspaceRoot, maxAgeHours = 168, onProgress = () => {} }) {
  ...
  const ranking = preselection(clusters, batch.batch_date);
  ...
  const pool = breaking
    ...
    : selectDimensionPool(clusters, ranking);
  ...
  const cards = await brainstorm(gateway,store,breaking?pool.selected:dimensionEntries,account,batchId,provider,onProgress);
  ...
  const synthesis = breaking
    ...
    : await synthesize(gateway,store,cards,batchId,provider,onProgress);
  ...
  writeFile(path.join(workdir,'topics-ranked.md'),markdownRanked(scored,synthesis,pool.selected.filter((group)=>group.events)));
