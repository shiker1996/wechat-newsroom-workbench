...
import { parseModelJson as parseSharedModelJson } from './model-json.mjs';
import { enforceNotificationQuota, isConcreteReaderStake, resolveDistributionDecision, resolveNotificationPolicy } from '../domain/distribution-strategy.mjs';
...
import { projectStableEvents } from '../domain/event-resolution-cluster-projection.mjs';
...
export function preselection(clusters, batchDate = new Date().toISOString().slice(0,10), scoring = DEFAULT_SCORING, eventHeat = []) {
  ...
  return clusters.map((event) => {
    ...
    return { eventId:event.event_id, hotspotId:event.representativeHotspotId, title:event.representative_title,
      ...
      duplicatePenalty: Number(event.duplicatePenalty ?? (heat.state === 'stale' ? 15 : Math.min(12, Math.max(0, Number(heat.repeatDays || 0) - 1) * 4))),
...
每条只给2个互不等价命题和反证；outline只给3项。通知池是稀缺池，允许整批为空；必须达到账号配置的通知适配分与事实支持分，风险等级不得命中禁入项。readerStake 必须写明具体读者、需要改变的决策或动作、以及工作/收入/岗位/效率/成本/选择中的具体后果；同时拆分填写 readerTarget、readerAction、readerConsequence 和 readerStakeEvidence，并给出 0～5 的 readerStakeScore。readerStakeScore 只进入 B 的受众项一次，不要再用同一读者利益重复加分；“影响职业方向”“影响技术选择”之类泛化表述不合格。传闻、未证实重大事件、健康或生物安全题默认进入实验池，不能靠情绪或标题张力晋级。没有读者利益的宏观事件不得仅凭主体知名度获得高 readerStakeScore。除标题外，每个字符串控制在80个汉字以内。没有可靠事实支撑时降低 factSupport 并写明待核验，不能用流畅包装掩盖证据缺口。不要输出 Markdown、解释或 JSON 之外的文字。`;
...
export async function brainstorm(gateway, store, selected, account, batchId, provider, onProgress, workspaceRoot) {
  ...
  async function processGroup(group,label,retry=false) {
    ...
    try { parsed=parseModelJson(result,store); }
      ...
      const candidate=group[0];store.recordPipelineFailure?.({batchId,stage:'research',objectType:'brainstorm-card',objectKey:candidate?.candidateId||label,title:candidate?.title||candidate?.hotspot_title||'',errorCode:'invalid_output',errorMessage:error.message,detail:{label,retry:true}});onProgress(`单张分析卡 ${candidate?.candidateId||label} 失败，已记录并继续其余候选`);return;
    }
...
const SYNTHESIS_SYSTEM = `你是热点综合研判器。比较全部临时包装后，只输出竞争修正，不直接计算最终总分。返回严格 JSON：{"items":[{"candidateId":字符串,"saturationPenalty":0到15,"readerStakeScore":0到5,"reason":字符串}],"metaNarratives":[字符串],"combination":{"primary":字符串,"stable":字符串,"darkHorse":字符串,"reason":字符串}}。readerStakeScore 只校正结构化读者利益，不得把同一读者利益重复叠加；旧数据中的 audienceRelevance 仅作为兼容输入。S 是同类内容与角度饱和度（市场同类选题泛滥程度）。同一事件换主体、维度或标题重复出现时，必须提高后出现候选的饱和度并在 reason 点明重复对象；组合推荐不得同时选择实质相同的事件角度。风险标签不参与竞争分，但通知资格由下游硬门禁单独处理。reason不超过40个汉字，metaNarratives最多3条且每条不超过50字。不要输出JSON之外的文字。`;

export function breakingSynthesis(cards){return {items:cards.map((card)=>{const readerStakeScore=Number(card.packaging?.readerStakeScore ?? card.bScores?.readerStakeScore ?? card.bScores?.audienceRelevance ?? 0);return {candidateId:card.candidateId,saturationPenalty:0,readerStakeScore,audienceRelevance:readerStakeScore,reason:'突发单题不参与批次竞争'};}),metaNarratives:[],combination:{}};}
...
export function scoreCards(cards, synthesis, scoring = resolveScoring()) {
  ...
  const normalized = cards.filter((card) => card.status !== 'NO_ANGLE').map((card) => {
    ...
    // 双跑基线：沿用旧的“文章分 - 竞争扣分”口径，只用于回放审计，不参与新排序。
    const legacyF = clamp(A-S-D, 0, 100);
    ...
    return { ...card, topicValue: Number(card.source?.topicValue ?? 0) || 0, eventValue, t: eventValue, recommendedSkill, distributionLane:distribution.distributionLane, readerStake:distribution.readerStake,
      ...
      a:Number(A.toFixed(1)), h:H, b:B, p:P, s:S, d:D, duplicatePenalty:D, legacyF:Number(legacyF.toFixed(1)),
      f:Number(F.toFixed(1)), scoreDelta:Number((F-legacyF).toFixed(1)), bParts,
...
/**
 * 生成新旧公式双跑审计，不改变生产排序或入池门槛。
 * 旧公式只保留作对照：legacyF = A - S - D；新公式为 scoreCards() 的 F。
 */
export function buildScoreDualRun(scored = [], { draftFloor = 55 } = {}) {
  const byLegacy = [...scored].sort((a, b) => Number(b.legacyF ?? (b.a - b.s - b.d)) - Number(a.legacyF ?? (a.a - a.s - a.d)) || String(a.candidateId).localeCompare(String(b.candidateId)));
  const legacyRank = new Map(byLegacy.map((item, index) => [item.candidateId, index + 1]));
  const items = scored.map((item) => {
    const legacyF = Number(Number(item.legacyF ?? (Number(item.a || 0) - Number(item.s || 0) - Number(item.d || 0))).toFixed(1));
    ...
    return {
      ...
      t, a, legacyF, currentF,
      delta: Number((currentF - legacyF).toFixed(1)),
      legacyRank: legacyRank.get(item.candidateId) || null,
      currentRank: item.finalRank || null,
      legacyDraftable: legacyF >= draftFloor,
      currentDraftable: currentF >= draftFloor,
      flags,
    };
  });
  ...
  return {
    ...
    formula: { legacy: 'F_legacy = A - S - D', current: 'F = A×(1-eventValueWeight) + T×eventValueWeight - S - D' },
    draftFloor,
    summary: {
      candidateCount: items.length,
      legacyDraftableCount: items.filter((item) => item.legacyDraftable).length,
      currentDraftableCount: items.filter((item) => item.currentDraftable).length,
      poolChangedCount: items.filter((item) => item.legacyDraftable !== item.currentDraftable).length,
      rankChangedCount: items.filter((item) => item.legacyRank !== item.currentRank).length,
      meanDelta: mean(deltas),
      highTLowACount: items.filter((item) => item.flags.includes('high_t_low_a')).length,
      lowTHighACount: items.filter((item) => item.flags.includes('low_t_high_a')).length,
      repeatPenaltyCount: items.filter((item) => item.flags.includes('repeat_penalty')).length,
      readerStakeMissingCount: items.filter((item) => item.flags.includes('reader_stake_missing')).length,
    },
    items,
  };
}

function markdownScoreDualRun(dualRun) {
  const s = dualRun.summary;
  return `# 选题评分新旧公式双跑审计\n\n> 本文件只用于回放和运营校准，不改变当前批次的排序或入池结果。\n\n- 旧公式：${dualRun.formula.legacy}\n- 新公式：${dualRun.formula.current}\n- 成稿线：F ≥ ${dualRun.draftFloor}\n\n## 批次摘要\n\n| 候选数 | 旧公式达线 | 新公式达线 | 入池变化 | 排名变化 | 平均分差（新−旧） |\n|---:|---:|---:|---:|---:|---:|\n| ${s.candidateCount} | ${s.legacyDraftableCount} | ${s.currentDraftableCount} | ${s.poolChangedCount} | ${s.rankChangedCount} | ${s.meanDelta} |\n\n## 定向回放样本\n\n- 高 T / 低 A：${s.highTLowACount} 条\n- 低 T / 高 A：${s.lowTHighACount} 条\n- 重复事件扣分：${s.repeatPenaltyCount} 条\n- 缺少读者利益：${s.readerStakeMissingCount} 条\n\n## 候选明细\n\n| 候选 | 选题 | T | A | 旧 F | 新 F | Δ | 旧排名 | 新排名 | 旧达线 | 新达线 | 样本标签 |\n|---|---|---:|---:|---:|---:|---:|---:|---:|:---:|:---:|---|\n${dualRun.items.map((item) => `| ${item.candidateId} | ${String(item.title).replace(/\|/g, '/')} | ${item.t} | ${item.a} | ${item.legacyF} | ${item.currentF} | ${item.delta} | ${item.legacyRank ?? '—'} | ${item.currentRank ?? '—'} | ${item.legacyDraftable ? '是' : '否'} | ${item.currentDraftable ? '是' : '否'} | ${item.flags.join('、') || '—'} |`).join('\n')}`;
}
...
export async function runResearchPipeline({ gateway, store, batchId, provider, workspaceRoot, maxAgeHours = 168, onProgress = () => {} }) {
  ...
  try {
    ...
    shadowResult = resolveEventShadow({ batch, hotspots: eligibleHotspots, legacyClusters: allClusters, history });
    ...
    onProgress(`事件归并影子运行完成：旧 ${shadowResult.legacy.event_count} 组 → 新 ${shadowResult.shadow.event_count} 组，待复核 ${shadowResult.differences.review_queue.length} 条`);
  } catch (error) {
    shadowResult = { schema_version: 1, resolver_version: 'shadow-v1', algorithm_version: 'structured-v1', mode: 'shadow',
      generated_at: new Date().toISOString(), batch_id: batch.id, status: 'error', error: String(error?.message || error) };
    writeFile(shadowPath, JSON.stringify(shadowResult, null, 2));
    writeFile(shadowDiffPath, JSON.stringify(shadowResult, null, 2));
    onProgress(`事件归并影子运行失败，已保留旧聚类结果：${shadowResult.error}`);
  }
  const eventHeatPath = path.join(sourcesDir, 'event-heat-ranking.json');
  let eventHeatRanking;
  try {
    let previousItems = [];
    try { previousItems = JSON.parse(fs.readFileSync(eventHeatPath, 'utf8'))?.items || []; } catch {}
  ...
  const legacyClusters = allClusters.filter((event)=>!skippedEventIds.has(event.event_id));
  if(!researchHotspots.length)throw new Error('所有有效事件均已跳过，当前批次没有可研判内容');
  if (legacyClusters.reduce((sum,event)=>sum+event.report_count,0) !== researchHotspots.length) throw new Error('事件聚类门禁失败：报道数不守恒');
  ...
  const clustersJson = { generated_at:new Date().toISOString(), total_articles:researchHotspots.length,excluded_stale_count:staleCount,excluded_skipped_event_count:skippedEvents.length,total_events:legacyClusters.length,events:legacyClusters.map(({tags,representativeHotspotId,...event})=>event) };
  ...
  writeFile(path.join(workdir,'hotspot-overview.html'),overviewHtml(legacyClusters));
  onProgress('检查事件事实卡');
  ...
  for (const event of legacyClusters) { const card = cardsByEvent.get(event.event_id); if (card) event.card = card; }
  const heatByEvent = new Map((eventHeatRanking.items || []).map((item) => [item.eventId, item]));
  const stableClusters = shadowResult?.status === 'error' ? legacyClusters : projectStableEvents({
    shadowEvents: (shadowResult?.events || []).map((event) => ({ ...event,
      hotspot_ids: (event.hotspot_ids || []).filter((hotspotId) => !skippedHotspotIds.has(Number(hotspotId))),
    })).filter((event) => event.hotspot_ids.length),
    legacyClusters, hotspots: researchHotspots, heatByEvent,
  });
  const clusters = stableClusters.length ? stableClusters : legacyClusters;
  for (const event of clusters) {
  ...
  const scoreDualRun = buildScoreDualRun(scored);
  ...
  const scoreDualRunPath = path.join(sourcesDir, 'score-dual-run.json');
  const scoreDualRunMarkdownPath = path.join(workdir, 'score-dual-run.md');
  writeFile(scoreDualRunPath, JSON.stringify({ ...scoreDualRun, batchId, generatedAt: new Date().toISOString() }, null, 2));
  writeFile(scoreDualRunMarkdownPath, markdownScoreDualRun(scoreDualRun));
  if (!breaking) {
  ...
  const artifacts = [
    ...
    ['评分新旧公式双跑','score-dual-run.json',scoreDualRunPath],
    ...
    ['临时选题总榜','topics-ranked.md',path.join(workdir,'topics-ranked.md')],['评分双跑审计','score-dual-run.md',scoreDualRunMarkdownPath],
  ];
