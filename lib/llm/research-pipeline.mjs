...
export function choosePool(ranking) {
  ranking.forEach((item,index) => { item.preRank=index+1; item.poolRole='未入选'; });
...
export async function runResearchPipeline({ gateway, store, batchId, provider, workspaceRoot, maxAgeHours = 168, onProgress = () => {} }) {
  ...
  const ranking = preselection(clusters,batch.batch_date); const pool = choosePool(ranking);
