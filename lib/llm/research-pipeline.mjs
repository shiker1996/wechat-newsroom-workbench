...
export async function runResearchPipeline({ gateway, store, batchId, provider, workspaceRoot, maxAgeHours = 168, onProgress = () => {} }) {
  ...
  const ranking = preselection(clusters, batch.batch_date, resolveScoring());
  ...
  const scored = scoreCards(cards,synthesis,resolveScoring());
  ...
  writeFile(path.join(workdir,'topics-ranked.md'),markdownRanked(scored,synthesis,pool.selected.filter((group)=>group.events),resolveScoring()));
