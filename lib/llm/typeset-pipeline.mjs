...
export async function runTypesetPipeline({ gateway, store, batchId, candidateId, documentKind = null, provider, workspaceRoot, skillsWorkspaceRoot = workspaceRoot, draftMode = 'deterministic', theme = 'auto', onProgress = () => {} }) {
  ...
  for (const [pattern, skill, script, label, fileName] of chartSteps) {
    ...
    try {
      const run = await runScript(skillScript(skills[skill], 'scripts', script), [chartReadyPath, chartPath], workdir);
    ...
    chartNotes.push(`${label} ${chartReport.converted} 张`);
  }
  ...
  record('images', 'wechat-article-typeset', imagesPath, 'completed', chartNotes.length ? `显式视觉模块已转图：${chartNotes.join('、')}；最终 HTML 允许引用可解析的本地图片路径` : '项目内最终 HTML 允许引用可解析的本地图片路径');
