...
// 排版主题：版式结构由主题决定（variants），配色/字阶由 design tokens 决定。
// 主题的 tokens 作为底色，LLM design 阶段的 tokens 叠加覆盖。
export const TYPESET_THEMES = Object.freeze({
  'magazine-warm': {
    label: '暖纸杂志风',
    // 米白纸底 + 棕褐点缀，呼应「暖纸」命名（参考 GPT 暖纸主题 #F5EFE3/#76533B）
    tokens: { colors: { background: '#F5EFE3', text: '#30261F', muted: '#786F66', accent: '#76533B' } },
    variants: { kicker: 'chip', h2: 'numbered-rule', quote: 'warm-card', divider: 'ornament', lead: true, serifHeadings: true, justify: true, strong: 'accent' },
  },
  'gossip-card': {
    label: '卡片吃瓜风',
    // 橙红爆料配色 + 深色大引述块，比原靛蓝更贴「吃瓜」气质（参考 GPT 卡片主题 #FF6B35/#111）
    tokens: { colors: { background: '#FFFFFF', text: '#1F2937', muted: '#4B5563', accent: '#FF6B35' } },
    variants: { kicker: null, h2: 'eyebrow-border', quote: 'dark-block', divider: 'hairline', lead: false, serifHeadings: false, justify: false, strong: 'ink' },
  },
  // 🤖 AI/技术动态：暗色终端——整篇像一份控制台输出，等宽标题、# 号章节、绿色光标点缀
  'tech-wire': {
    label: '暗色终端',
    tokens: { colors: { background: '#0D1117', text: '#E6EDF3', muted: '#8B949E', accent: '#39D353' } },
    variants: { kicker: 'mono-line', h1: 'sans', h2: 'terminal', quote: 'terminal-panel', divider: 'mono-comment', lead: false, serifHeadings: false, justify: false, strong: 'accent', list: 'chevron', bodyFont: 'sans' },
  },
  // 📈 行业趋势 / 严肃大厂战略 / 综合文：财经印刷——居中大标题双线压顶、方框引述、墨色加粗
  'research-report': {
    label: '财经印刷',
    tokens: { colors: { background: '#FFFFFF', text: '#1A1A1A', muted: '#6B7280', accent: '#0F2B4C' } },
    variants: { kicker: 'center-label', h1: 'center-double', h2: 'center-double', quote: 'boxed', divider: 'rule-mark', lead: true, serifHeadings: true, justify: true, strong: 'ink' },
  },
  // 💼 职场人文：书信手账——米色纸底、正文也排衬线、「一、」章节、大引号拉页引文
  'career-essay': {
    label: '书信手账',
    tokens: { colors: { background: '#F6EFDF', text: '#3B3226', muted: '#8A7B63', accent: '#A35D2F' } },
    variants: { kicker: 'center-label', h1: 'serif-display', h2: 'cn-number', quote: 'quote-marks', divider: 'stars', lead: true, serifHeadings: true, justify: true, strong: 'accent', bodyFont: 'serif' },
  },
  // 📰 综合资讯：黑白快讯——黑底白字标题块、粗黑分隔杠、红色只留给重点
  'news-digest': {
    label: '黑白快讯',
    tokens: { colors: { background: '#FFFFFF', text: '#111111', muted: '#666666', accent: '#D61F26' } },
    variants: { kicker: 'chip', h1: 'invert-block', h2: 'invert-tag', quote: 'plain-bar', divider: 'thick-bar', lead: false, serifHeadings: false, justify: true, strong: 'accent' },
  },
});
...
function buildInlineStyles(tokens, themeName = 'magazine-warm') {
  const theme = TYPESET_THEMES[themeName] || TYPESET_THEMES['magazine-warm'];
  ...
  const styles = {
    article: `box-sizing:border-box;margin:0;padding:24px 18px;background:${background};color:${ink};font-family:${bodyFont};font-size:${bodyPx}px;line-height:${lineHeight};letter-spacing:.02em${variants.justify ? ';text-align:justify' : ''}`,
    ...
    h1: variants.h1 === 'invert-block'
      ? `background:${ink};color:${background};padding:18px 16px;font-size:${h2Px + 8}px;line-height:1.35;margin:0 0 ${sectionPx}px;font-weight:900;letter-spacing:.01em`
      : variants.h1 === 'center-double'
        ? `font-family:${serif};text-align:center;font-size:${h2Px + 10}px;line-height:1.35;margin:0 0 ${sectionPx}px;padding:18px 4px;border-top:6px double ${ink};border-bottom:1px solid ${hexToRgba(ink, 0.4)};font-weight:700`
        : variants.h1 === 'serif-display'
          ? `font-family:${serif};font-size:${h2Px + 12}px;line-height:1.3;margin:0 0 ${sectionPx}px;font-weight:700;color:${ink}`
          : variants.serifHeadings
            ...
            : `font-size:${h2Px + 6}px;line-height:1.4;margin:0 0 ${sectionPx}px;font-weight:800;color:${ink}`,
    ...
    h2Wrap: `margin:${sectionPx + 10}px 0 ${paragraphPx + 6}px;padding-left:16px;border-left:5px solid ${accent}`,
    ...
    p: `margin:0 0 ${paragraphPx}px`,
    lead: `margin:0 0 ${paragraphPx}px;font-size:${bodyPx + 1}px;line-height:${lineHeight}`,
    ...
    list: `margin:12px 0 ${paragraphPx + 4}px;padding-left:25px`,
    ...
    img: `display:block;max-width:100%;height:auto;margin:24px auto${radiusPx ? `;border-radius:${radiusPx}px` : ''}`,
    ...
    table: `width:100%;margin:0 0 ${paragraphPx}px;border-collapse:collapse;table-layout:fixed;font-size:${Math.max(bodyPx - 1, 13)}px`,
    th: `padding:9px 8px;border:1px solid ${hexToRgba(ink, 0.22)};background:${hexToRgba(accent, 0.12)};color:${ink};font-weight:700;text-align:left;word-break:break-word`,
    td: `padding:9px 8px;border:1px solid ${hexToRgba(ink, 0.18)};vertical-align:top;word-break:break-word`,
...
export async function runTypesetPipeline({ gateway, store, batchId, candidateId, documentKind = null, provider, workspaceRoot, skillsWorkspaceRoot = workspaceRoot, draftMode = 'deterministic', theme = 'auto', autoUploadGeneratedImages = true, onProgress = () => {} }) {
  ...
  if (!TYPESET_THEMES[theme]) throw new Error(`未知排版主题：${theme}（可选：auto、${Object.keys(TYPESET_THEMES).join('、')}）`);
  ...
  const themeColors = TYPESET_THEMES[theme]?.tokens?.colors || {};
  writeFile(chartTokensPath, JSON.stringify({ ...htmlTokens, colors:{ ...(htmlTokens.colors || {}), ...themeColors }, theme }, null, 2));
  ...
  if (/<(?:section|div)\b[^>]*(?:stats-grid|timeline)/i.test(rendered)) {
    record('images', 'wechat-article-typeset', '', 'blocked', '检测到 内联视觉模块（wechat-inline-modules-to-images）；项目技能未提供可直接调用的渲染脚本');
    throw new Error('文章含 内联视觉模块（wechat-inline-modules-to-images），对应项目技能当前只有执行契约、没有渲染脚本，已停止排版以避免丢图');
  }
  // Mermaid / ECharts 围栏由确定性脚本渲染为本地 PNG 并替换为图片引用；
  // 渲染失败的围栏保留原文并报错，绝不静默丢图
  const chartSteps = [
    [/```\s*mermaid\b/i, 'mermaid-render', 'render-mermaid.mjs', 'Mermaid', '09-FINAL.mermaid.md'],
    [/```\s*echarts\b/i, 'wechat-echarts-blocks-to-images', 'render-echarts.mjs', 'ECharts', '09-FINAL.echarts.md'],
  ];
  const chartNotes = [];
  let chartReadyPath = renderedPath;
  for (const [pattern, skill, script, label, fileName] of chartSteps) {
    ...
    try {
      const run = await runScript(skillScript(skills[skill], 'scripts', script), [chartReadyPath, chartPath, path.join(workdir, 'images'), chartTokensPath], workdir);
      chartReport = JSON.parse(run.stdout.trim().split(/\r?\n/).at(-1));
    } catch (error) {
      try { chartReport = JSON.parse(String(error.message).trim().split(/\r?\n/).at(-1)); } catch { /* 保留原始错误 */ }
      const detail = chartReport?.failed?.map((item) => `第 ${item.index} 个围栏：${item.error}`).join('；') || error.message;
    ...
    for (const item of autoUploadGeneratedImages ? pendingUploads : []) {
      ...
      await uploadImageToCdn(workdir, item.id);
    }
  ...
  if (draftMode === 'llm') {
    ...
    writeFile(draftHtml, markdownToHtml(imageResult.content, { ...htmlTokens, ...(TYPESET_THEMES[theme].tokens || {}), kicker, theme }));
    draftDetail = `确定性渲染：主题 ${theme}，按 design tokens 直接输出内联样式，未调用模型`;
  }
