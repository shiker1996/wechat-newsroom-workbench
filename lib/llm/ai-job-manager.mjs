...
export class AiJobManager {
  ...
  start({ batchId, type, provider, force = false, candidateId = null }) {
    if (!this.store.getBatch(batchId)) throw new Error('批次不存在');
    if (!['tag','retag','event-cards','research','breaking-analysis','article','typeset','social-card','auto'].includes(type)) throw new Error('未知 AI 任务');
    ...
    this.run(job,{force,candidateId}).catch(()=>{}); return job;
  }
  ...
  async run(job,{force,candidateId}) {
    try {
