...
import { AI_JOB_TYPES, BATCH_LEVEL_AI_JOB_TYPES, createAiJobHandlers } from '../jobs/ai-job-handlers.mjs';
import { recordResearchFailure } from '../jobs/research-failure.mjs';
...
export class AiJobManager {
  constructor(store, gateway, config) {
    ...
    this.handlers = createAiJobHandlers({ store, gateway, config, log: (job, message) => this.log(job, message) });
  }

  // 互斥键：批次级任务按批次互斥，候选级任务按候选互斥，二者互不阻塞。
  conflictKey(job) {
    return BATCH_LEVEL_AI_JOB_TYPES.has(job.type) ? `batch:${job.batchId}` : `candidate:${job.candidateId ?? 'none'}`;
  }
  ...
  async run(job, options = {}) {
    ...
    try {
      ...
      if(job.type==='research'||(job.type==='auto'&&job.phase==='research'))recordResearchFailure({store:this.store,job,error});
