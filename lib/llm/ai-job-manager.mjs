...
import { runCoverImageJob } from './cover-image-generator.mjs';
...
export class AiJobManager {
  ...
  start({ batchId, type, provider, force = false, candidateId = null, documentKind = null, theme = undefined, focus = null, focuses = [], snapshotId = null, skillSelection = null, stageSelections = null }) {
    ...
    if (!['tag','retag','event-cards','research','breaking-analysis','article','daily','tutorial','typeset','social-card','cover-image','auto'].includes(type)) throw new Error('未知 AI 任务');
    ...
    try {
      ...
      if (job.type === 'tag' || job.type === 'retag') {
        ...
        result=await runArticlePipeline({gateway:this.gateway,store:this.store,batchId:job.batchId,candidateId,
          provider:job.requestedProvider,workspaceRoot:this.config.workspaceRoot,snapshotId:job.snapshotId,skillSelection:job.skillSelection,stageSelections:job.stageSelections,articleLength:this.config.articleLength,onProgress:(m)=>this.log(job,m)});
        ...
        result=await runDailyPipeline({gateway:this.gateway,store:this.store,batchId:job.batchId,provider:job.requestedProvider,
          workspaceRoot:this.config.workspaceRoot,snapshotId:job.snapshotId,stageSelections:job.stageSelections,focus,focuses,articleLength:this.config.articleLength,onProgress:(m)=>this.log(job,m)});
      ...
      } else if (job.type === 'cover-image') {
        result=await runCoverImageJob({gateway:this.gateway,store:this.store,batchId:job.batchId,candidateId:job.candidateId,
          provider:job.requestedProvider,workspaceRoot:this.config.workspaceRoot,theme:job.theme||'',onProgress:(m)=>this.log(job,m)});
