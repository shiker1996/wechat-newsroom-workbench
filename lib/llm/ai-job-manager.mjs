    ...
    try {
      ...
      if (job.type === 'tag' || job.type === 'retag') {
        ...
        result=await runDailyPipeline({gateway:this.gateway,store:this.store,batchId:job.batchId,provider:job.requestedProvider,
          workspaceRoot:this.config.workspaceRoot,snapshotId:job.snapshotId,focus,focuses,onProgress:(m)=>this.log(job,m)});
        ...
        result=await runTutorialPipeline({gateway:this.gateway,store:this.store,batchId:job.batchId,candidateId,
          provider:job.requestedProvider,workspaceRoot:this.config.workspaceRoot,snapshotId:job.snapshotId,skillSelection:job.skillSelection,onProgress:(m)=>this.log(job,m)});
