...
export async function runTutorialPipeline({gateway,store,batchId,candidateId,provider,workspaceRoot,snapshotId=null,skillSelection=null,onProgress=()=>{}}){
  ...
  const titleGenerator=loadSkillBundle({workspaceRoot,skillName:'title-generator'});
  ...
  const seoOptimizer=loadSkillBundle({workspaceRoot,skillName:'seo-content-optimizer'});
  const runtime=await prepareSkillRun({gateway,store,batchId,candidateId,purpose:articleMode==='experience'?'personal-writing':'tutorial',bundles:[skill,titleGenerator,humanizer,reviewer,seoOptimizer],provider,snapshotId,
    selection:skillSelection?{...skillSelection,entryPoint:'independent-writing',contentType:articleMode}:null});
  ...
  const lengthGate=runtime.config?.gates?.length||{minVisibleChars:1000,maxVisibleChars:1800};
  const minChars=Number(lengthGate.minVisibleChars),maxChars=Number(lengthGate.maxVisibleChars);
  const repairAttempts=configuredRepairAttempts(runtime.config,1);
  const label=articleMode==='experience'?'心得经验':'使用教程';
  onProgress(`${label} 1/6 根据自主写作事实基座生成初稿`);
  ...
  onProgress(`${label} 2/6 根据初稿生成并锁定标题`);
  ...
  onProgress(`${label} 3/6 去除模板腔并保持事实不变`);
  ...
  onProgress(`${label} 4/6 审阅并修订事实、逻辑与风险`);
  ...
  onProgress(`${label} 5/6 执行 SEO 优化并保持事实与作者立场`);
  ...
  onProgress(`${label} 6/6 执行事实与真实性门禁并保存终稿`);
  ...
  if(count<minChars||count>maxChars)throw new Error(`${label}有 ${count} 个可见字符，未达到 ${minChars}–${maxChars} 字门禁`);
