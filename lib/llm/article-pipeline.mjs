...
function visibleChars(markdown) { return markdownVisibleChars(markdown); }
...
export function authorizedWritingBrief(brief) {
  const safe={...brief};
...
export const ARTICLE_LENGTH_RANGE=Object.freeze({min:1300,max:1700});
function parseJsonResult(result,store) {
  ...
  try{return JSON.parse(raw);}catch(error){
    ...
    const attempts=result.outputBudget?.attempts||1;
    const reason=result.finishReason==='length'
      ? `模型输出在${attempts>1?'自动扩容重试后仍':'当前阶段'}达到上限，JSON 被截断`
      : `模型返回无效 JSON:${error.message}`;
...
export function selectWriterSkill(candidate={}) {
  ...
  if(!serious&&/趣闻|离谱|八卦|段子|奇葩|荒诞|整活|吐槽/.test(angleText)){
    return {skill:'wechat-mp-gossip-chill',reason:'角度明确采用轻量趣闻或职场反差表达，且不涉及严肃伤害事件'};
  }
  ...
  if(deepIntent&&technicalSubject){
    return {skill:'wechat-mp-tech-deep',reason:'命题要求解释技术机制，或对性能、成本和指标进行可复算拆解'};
  }
  if(candidate.category==='🤖 AI/技术动态'||technicalSubject){
    return {skill:'wechat-mp-tech-hotspot',reason:'主题属于技术、产品或行业动态，重点是事件影响而非完整原理推导'};
  }
...
export function articleLengthStatus(article, range = ARTICLE_LENGTH_RANGE) {
  ...
  return {count,valid:count>=range.min&&count<=range.max,
    shortfall:Math.max(0,range.min-count),overflow:Math.max(0,count-range.max)};
}
...
export async function runArticlePipeline({gateway,store,batchId,candidateId,provider,workspaceRoot,snapshotId=null,skillSelection=null,stageSelections=null,onProgress=()=>{}}) {
  ...
  if(hasOpenQuestions(editorial.open_questions))throw new Error('仍有未决问题,不能进入成稿');
  ...
  if(visibleChars(final)<articleLengthRange.min||visibleChars(final)>articleLengthRange.max)throw new Error(`终稿有 ${visibleChars(final)} 个可见字符，未达到 ${articleLengthRange.min}–${articleLengthRange.max} 字门禁`);
