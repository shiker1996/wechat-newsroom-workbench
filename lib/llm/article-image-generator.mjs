...
export async function generateArticleImage({ workspaceRoot, workdir, slotId, generate, ratio }) {
  ...
  if (!images.length) throw new Error('配图生成未产出 PNG');
  const target = path.join(imageDir, `${safeName}.png`);
  if (path.resolve(images[0]) !== path.resolve(target)) fs.renameSync(images[0], target);
