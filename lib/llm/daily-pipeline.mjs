...
export async function runDailyPipeline({ gateway, store, batchId, provider, workspaceRoot, snapshotId=null, focuses = [], focus = null, onProgress = () => {} }) {
  ...
  const lengthGate=runtime.config?.gates?.length||{minVisibleChars:0,maxVisibleChars:1200};
  const minChars=Number(lengthGate.minVisibleChars||0),maxChars=Number(lengthGate.maxVisibleChars||1200);
  ...
  if (count<minChars||count>maxChars) throw new Error(`早报有 ${count} 个可见字符，未达到 ${minChars}–${maxChars} 字门禁`);
