  ...
  if(cardPageAiMatch&&request.method==='POST'){
    ...
    try{
      ...
      if(editMode==='restructure'){
        ...
        let applied=applySocialCardRestructureOperations(cardPlan,operations,{maxPages:maxPagesForContent});
        if(!applied.valid&&(!Array.isArray(operations)||!operations.length)){
          operations=buildDeterministicSocialCardRestructureOperations(cardPlan,[{page:pageIndex+1,issues:previousPage?.issues||[]}],{maxPages:maxPagesForContent});
          applied=applySocialCardRestructureOperations(cardPlan,operations,{maxPages:maxPagesForContent});
        }
        if(!applied.valid&&(!Array.isArray(operations)||!operations.length)){
          const reason=cardPlan.length>=maxPagesForContent
            ? `当前故事板已有 ${cardPlan.length} 页，已达到该内容类型的 ${maxPagesForContent} 页上限`
  ...
  if (cardEditorialAiMatch && request.method === 'POST') {
    ...
    try {
      ...
      const editorial=store.saveCardEditorial(candidate.id,{...current,
        ...
        recommended_pages:Math.max(4,Math.min(maxPages,Number(parsed.recommended_pages)||cardPlan.length||6)),card_plan_json:JSON.stringify(cardPlan),storyboard_theme_snapshot_json:JSON.stringify(createSocialCardStoryboardThemeSnapshot({themeDefinition:templateContext.themeDefinition,channelMode,contentType})),status:'AI_READY'});
      ...
      return json(response,200,{editorial,gate,themeState,cardPlan,contentType,eventAnalysis,layoutDecisions:describeCardLayouts(cardPlan,{layoutStyle:editorial.layout_style,compositionMode:editorial.composition_mode,channelMode}),
