...
import { runTutorialAgentTurn, tutorialProjectAttachmentArguments } from '../../agent/tutorial-adapter.mjs';
import { getFactAttachment, selectConversationSearchAttachments } from '../../agent/fact-attachments.mjs';
import { runCustomSocialAgentTurn } from '../../agent/custom-social-adapter.mjs';
...
export async function handleCandidateRoutes({ request, response, pathname, searchParams, root, config, store, body, json, models, aiJobs, localSecurity,
  ...
  if (customSocialMatch && request.method === 'POST') {
    ...
    try {
      const cachedAttachments=store.listConversationFactAttachments({batchId,entryPoint:'custom-social'}),materialCache=new Map(cachedAttachments.filter((item)=>item.capability==='content.url.fetch').map((item)=>[item.data.final_url||item.data.url||item.data.requested_url,item.data]).filter(([url])=>url));
      const fact = await buildCustomFactSheet({ input, root,materialCache }); const toolPolicy = await resolveSkillToolPolicy({ workspaceRoot: root, skillId: 'custom-card-storyboard' });
      for(const attachment of cachedAttachments){if(attachment.capability!=='content.repository.inspect')continue;if(String(attachment.data?._agentQuery||'').trim()!==String(fact.topic||'').trim())continue;fact.repository_inspection=attachment.data;}
      for(const [capability,attachment] of selectConversationSearchAttachments(cachedAttachments,fact.topic)){const query=String(attachment.data?._agentQuery||fact.topic||'');if(capability==='content.web.search')fact.web_search={query,provider:'conversation-agent',answer:attachment.data.answer||'',results:attachment.data.results||[],warnings:attachment.data.warnings||[],searched_at:attachment.updated_at};if(capability==='content.news.search')fact.news_search={query,provider:'conversation-agent',answer:attachment.data.answer||'',results:attachment.data.results||[],warnings:attachment.data.warnings||[],searched_at:attachment.updated_at};if(capability==='content.document.search')fact.document_search={query,provider:'conversation-agent',documents:attachment.data.documents||[],warnings:attachment.data.warnings||[],searched_at:attachment.updated_at};}
