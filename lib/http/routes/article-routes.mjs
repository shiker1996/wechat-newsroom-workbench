...
export async function handleArticleRoutes(context) {
  ...
  if(editorialStreamMatch&&request.method==='POST') {
    ...
    try {
      ...
      send({type:'done',data:{candidate:result.candidate,editorial:result.editorial,usage:result.usage,model:result.model,ignoredBecauseLocked:Boolean(result.ignoredBecauseLocked)}});
    } catch(error) {
