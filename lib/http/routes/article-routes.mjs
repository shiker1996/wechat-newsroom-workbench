...
export async function handleArticleRoutes(context) {
  ...
  if(editorialStreamMatch&&request.method==='POST') {
    ...
    try {
      if(suppliedUrl)await fetchSourceForCandidate(candidate,{force:true,urlOverride:suppliedUrl});
      const result=await runEditorialTurnStream({gateway:models,store,candidateId,provider:input.provider,answer,webSearch:true,events:candidateEventGroups(candidate,12000),retrieve:await editorialRetrieve(candidate),workspaceRoot:root,onText:(text)=>send({type:'delta',text})});
      const fetchNote=await autoFetchEditorialEvents(candidate,result.fetchEvents);
