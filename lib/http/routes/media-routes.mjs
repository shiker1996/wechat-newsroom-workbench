export async function handleMediaRoutes(context) {
  ...
  if (visualPlanMatch && request.method === 'POST') {
    ...
    const result = await planArticleVisuals({
      ...
      maxOutputTokens:Math.min(5000, providerConfig.maxOutputTokens),
    });
  ...
  if (pathname === '/api/visual-preview' && request.method === 'POST') {
    ...
    try {
      ...
      const script = type === 'mermaid'
        ? path.join(root, 'skills', 'mermaid-render', 'scripts', 'render-mermaid.mjs')
        : path.join(root, 'skills', 'wechat-echarts-blocks-to-images', 'scripts', 'render-echarts.mjs');
      await execFileAsync(process.execPath, [script, source, output, imageDir, tokensPath], { cwd:root, windowsHide:true, timeout:180000, maxBuffer:1_000_000 });
