...
export async function handleMediaRoutes(context) {
  ...
  if (pathname === '/api/visual-preview' && request.method === 'POST') {
    ...
    try {
      ...
      const rendered = await registry.execute(`diagram.${type}.render`, {
        inputPath:source, outputPath:output, imageDir, tokensPath,
      }, { allowedRoots:[tempDir], cwd:root, timeoutMs:180000,
        executionLog:createStoreExecutionLogger(store,{skillId:'visual-preview'}) });
