...
import { assistStaticPage } from '../../collectors/static-page-assistant.mjs';
  ...
  if(unifiedConfigurationMatch&&['GET','PUT'].includes(request.method)){
    ...
    try{
      ...
      if(request.method==='GET'){
        result=extensionConfiguration.describe({extensionType:type,extensionId:id,manifest:resource.manifest,fallbackValues:resourceFallback(resource)});
      }else{
        const input=await body(request);
        ...
        // 模型配置勾选「设为默认」：联动更新运行时默认渠道并持久化到 config.local.json
        if(type==='model-provider'&&input?.default===true&&config.llm?.providers?.[id]){
          ...
          local.llm=local.llm||{};local.llm.defaultProvider=id;
