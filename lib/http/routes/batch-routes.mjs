...
import { buildEventResolutionOperationsMetrics, readEventResolutionReview } from '../../domain/event-resolution-operations.mjs';
import { buildTopicScoreOperationsMetrics } from '../../domain/topic-score-operations.mjs';
...
export async function handleBatchRoutes({ request, response, pathname, searchParams, root, store, jobs, body, json,
  ...
  if (batchMatch && request.method === 'GET') {
    ...
    if (batch) {
      try {
        ...
        const cardTotal = clusterItems(tagged).filter((event)=>!skippedEventIds.has(event.event_id)).length;
  ...
  if (eventHeatMatch && request.method === 'GET') {
    ...
    if (!ranking || !Array.isArray(ranking.items) || ranking.titleVersion !== 2) ranking = buildEventHeatRanking({ store, batch });
  ...
  if (overviewMatch && request.method === 'GET') {
    ...
    const legacyClusters = clusterItems(eligible);
    ...
    if (!eventHeatRanking || !Array.isArray(eventHeatRanking.items) || eventHeatRanking.titleVersion !== 2) eventHeatRanking = buildEventHeatRanking({ store, batch });
    ...
    const atlasClusters = stableEvents.length ? projectStableEvents({ shadowEvents: stableEvents, legacyClusters, hotspots: eligible, heatByEvent }) : legacyClusters;
