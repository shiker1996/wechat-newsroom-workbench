...
function detail(row,target){const published=userThemeFromRow(row),draft=userThemeFromRow(row,{draft:true}),compatibility=auditThemeForPublish(draft,{target}),components=target==='social'?socialComponentEditorCatalog(draft.social?.recipes):articleComponentEditorCatalog(draft.article?.recipes);return {schemaVersion:1,id:row.id,label:row.label,target,source:'user',status:row.status,activeVersion:row.active_version||null,definition:published,draft,compatibility,editorMode:compatibility.compatible?'full':'read-only',editorCatalog:{recipes:themeRecipeEditorCatalog(target),components}};}
