...
export function themeVisualSimilarity(left,right){
  ...
  const recipeKeys=Object.keys(left[target]?.recipes||{}),recipes=average(recipeKeys.map((key)=>left[target]?.recipes?.[key]===right[target]?.recipes?.[key]?1:0)),leftComponents=componentEntries(left),rightComponents=componentEntries(right),componentKeys=Object.keys(leftComponents),components=componentKeys.length?average(componentKeys.map((key)=>leftComponents[key]===rightComponents[key]?1:0)):1;
