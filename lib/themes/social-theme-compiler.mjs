...
export function compileSocialTheme(theme){
  ...
  const id=definition.id,scope=`.theme-${id}`,baseRecipes=definition.social.recipes||{},derivedSkeleton={neon:'terminal-rail','tokyo-night':'terminal-rail',brutalist:'impact-band',solarized:'editorial-split','retro-terminal':'terminal-rail','paper-craft':'paper-offset',charcoal:'impact-band',orange:'impact-band',crimson:'impact-band','bone-white':'editorial-split'},recipes={coverTitle:'classic',skeleton:derivedSkeleton[id]||'stacked',coverSupport:derivedSkeleton[id]==='impact-band'?'statement':derivedSkeleton[id]==='terminal-rail'?'metric':'lead',...baseRecipes},effects=definition.social.effects||{},components=resolveSocialComponents(definition);
  ...
  if(recipes.skeleton==='editorial-split')css+=`${scope} .skeleton-editorial-split:not(.page-cover) .page-content-stack{display:grid;grid-template-columns:minmax(0,1.08fr) minmax(0,.92fr);align-content:center;gap:calc(var(--card-gap) + 2px) var(--section-gap)}${scope} .skeleton-editorial-split:not(.page-cover) .eyebrow,${scope} .skeleton-editorial-split:not(.page-cover) h1,${scope} .skeleton-editorial-split:not(.page-cover) .cover-support{grid-column:1/-1}${scope} .skeleton-editorial-split:not(.page-cover) .content-block:nth-child(odd){grid-column:1}${scope} .skeleton-editorial-split:not(.page-cover) .content-block:nth-child(even){grid-column:2}${scope} .skeleton-editorial-split:not(.page-cover) .content-block.stats-block,${scope} .skeleton-editorial-split:not(.page-cover) .content-block.compare-block,${scope} .skeleton-editorial-split:not(.page-cover) .content-block.code-block{grid-column:1/-1}`;
