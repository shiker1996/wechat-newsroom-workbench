...
export function validateThemeDefinition(input,{filePath='',expectedTarget=null,expectedSource=null}={}){
  ...
  if(!tokens||typeof tokens!=='object'||Array.isArray(tokens))issues.push({field:'tokens',code:'TYPE',message:'必须是对象'});
    ...
    for(const [group,fields] of Object.entries(TOKEN_FIELDS)){
      ...
      for(const key of fields)if(!OPTIONAL_TOKEN_KEYS.has(key)&&value[key]===undefined)issues.push({field:`tokens.${group}.${key}`,code:'REQUIRED',message:'不能为空'});
    }
    ...
    for(const [field,[min,max]] of Object.entries(limits)){const path=field.split('.'),group=path[1],key=path[2];if(OPTIONAL_TOKEN_KEYS.has(key)&&tokens[group]?.[key]===undefined)continue;numberIn(tokens[group]?.[key],min,max,field,issues);}
