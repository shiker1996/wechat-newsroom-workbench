...
const TOP_FIELDS=new Set(['schemaVersion','id','label','version','description','targets','status','source','basedOn','tags','tokens','article','social']);
const TOKEN_FIELDS={colors:new Set(['background','surface','page','text','muted','accent','accentSecondary','line','inverseText','codeBackground']),typography:new Set(['family','headingFamily','bodyPx','h1Px','h2Px','captionPx','lineHeight','letterSpacingEm']),spacing:new Set(['articlePaddingPx','sectionPx','paragraphPx','cardGapPx']),shape:new Set(['radiusPx','borderWidthPx','shadow'])};
const TARGETS=new Set(['article','social']), STATUSES=new Set(['draft','published','disabled','archived']), SOURCES=new Set(['builtin','user']);
...
function numberIn(value,min,max,field,issues){if(!Number.isFinite(value)||value<min||value>max)issues.push({field,code:'OUT_OF_RANGE',message:`必须是 ${min}–${max} 之间的有限数值`});}
function luminance(hex){const channels=[1,3,5].map((index)=>parseInt(hex.slice(index,index+2),16)/255).map((value)=>value<=.04045?value/12.92:((value+.055)/1.055)**2.4);return .2126*channels[0]+.7152*channels[1]+.0722*channels[2];}
...
export function validateThemeDefinition(input,{filePath='',expectedTarget=null,expectedSource=null}={}){
  const issues=[];
  ...
  if(!Array.isArray(input.targets)||!input.targets.length||new Set(input.targets).size!==input.targets.length||input.targets.some((v)=>!TARGETS.has(v)))issues.push({field:'targets',code:'ENUM',message:'必须是不重复的 article/social 数组'});
  ...
  if(!tokens||typeof tokens!=='object'||Array.isArray(tokens))issues.push({field:'tokens',code:'TYPE',message:'必须是对象'});
    ...
    for(const [group,fields] of Object.entries(TOKEN_FIELDS)){
      ...
      for(const key of fields)if(key!=='page'&&value[key]===undefined)issues.push({field:`tokens.${group}.${key}`,code:'REQUIRED',message:'不能为空'});
    }
    ...
    if(HEX.test(tokens.colors?.text||'')&&HEX.test(tokens.colors?.background||'')&&colorContrast(tokens.colors.text,tokens.colors.background)<4.5)issues.push({field:'tokens.colors.text',code:'LOW_CONTRAST',message:'正文与背景对比度必须至少为 4.5:1'});
    ...
    for(const [key,min,max] of [['bodyPx',9,18],['h1Px',22,44],['h2Px',11,28],['captionPx',8,15],['lineHeight',1.2,2.1],['letterSpacingEm',-.08,.2]])numberIn(tokens.typography?.[key],min,max,`tokens.typography.${key}`,issues);
    for(const [key,min,max] of [['articlePaddingPx',0,40],['sectionPx',12,48],['paragraphPx',6,28],['cardGapPx',0,28]])numberIn(tokens.spacing?.[key],min,max,`tokens.spacing.${key}`,issues);
    numberIn(tokens.shape?.radiusPx,0,32,'tokens.shape.radiusPx',issues);numberIn(tokens.shape?.borderWidthPx,0,8,'tokens.shape.borderWidthPx',issues);
    if(!SHADOWS.has(tokens.shape?.shadow))issues.push({field:'tokens.shape.shadow',code:'ENUM',message:'阴影配方不受支持'});
  }
  for(const target of ['article','social']){
    ...
    if(!config)continue;
    unknown(config,new Set(target==='article'?['recipes','behavior']:['recipes','effects']),target,issues);
    if(!config.recipes||typeof config.recipes!=='object'||Array.isArray(config.recipes))issues.push({field:`${target}.recipes`,code:'TYPE',message:'必须是对象'});
      ...
      for(const [key,allowed] of Object.entries(catalog))if(config.recipes[key]===undefined)issues.push({field:`${target}.recipes.${key}`,code:'REQUIRED',message:'不能为空'});else if(!allowed.includes(config.recipes[key]))issues.push({field:`${target}.recipes.${key}`,code:'ENUM',message:'配方不受支持'});
    }
    ...
    if(target==='social'&&config.effects){
      ...
      numberIn(config.effects.decorationOpacity,0,1,'social.effects.decorationOpacity',issues);
      numberIn(config.effects.contentTiltDeg,-2,2,'social.effects.contentTiltDeg',issues);
    }
  }
