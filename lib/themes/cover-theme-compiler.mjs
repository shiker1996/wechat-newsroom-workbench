...
export function buildCoverHtml({theme,spec}){
  ...
  const main=layout?layout.main:{left:0,right:0,padding:`${padY}px ${padX+8}px`};
  ...
  const titlePx=Math.max(22,Math.min(titleMaxPx,Math.floor(mainWidth/(maxLineChars*1.02))));
  ...
  const metaHtml=meta?`<div class="cover-meta" style="color:${mutedColor};left:${metaLeft}px;bottom:${metaBottom}px">${escapeHtml(meta.text)}</div>`:'';
