...
const DEFAULTS={
  ...
  cover:{typography:{family:'sans',headingFamily:'sans',bodyPx:12,h1Px:34,h2Px:14,captionPx:10,lineHeight:1.4,letterSpacingEm:0},spacing:{articlePaddingPx:24,sectionPx:24,paragraphPx:10,cardGapPx:12},shape:{radiusPx:4,borderWidthPx:1,shadow:'soft'}},
};
...
export function normalizeAiThemeCandidate(input,{target}){
  ...
  // 封面主题是纯 token 主题：不创建/修复 targetConfig、recipes、components
  const targetConfig=target==='cover'?null:ensureObject(state.value,'targetConfig'),recipes=targetConfig?ensureObject(targetConfig,'recipes'):{};
