...
export function validateAiThemeRequest(input){
  ...
  if(!TARGETS.has(input.target))issues.push(issue('target','ENUM','必须为 article 或 social'));
...
export function validateAiThemeCandidate(candidate,{target}={}){
  const issues=[];
  if(!TARGETS.has(target))throw new AiThemeContractError(AI_THEME_ERROR_CODES.TARGET_MISMATCH,'AI 主题目标无效',[issue('target','ENUM','必须为 article 或 social')]);
  ...
  if(!object(candidate.targetConfig))issues.push(issue('targetConfig','TYPE','必须是目标配置对象'));
