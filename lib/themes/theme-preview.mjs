...
import { buildCoverHtml } from './cover-theme-compiler.mjs';
...
const selectors={
  ...
  cover:{colors:'.cover',typography:'.cover-title',shape:'.cover-block',surface:'.cover-block',accentSecondary:'.cover-eyebrow',line:'.cover-subtitle.with-bar',inverseText:'.cover-eyebrow',codeBackground:'.cover-block'},
};
...
export function compileThemePreview({target,definition,highlightField=''}){
  ...
  if(target==='cover'){
    const {html:coverHtml}=buildCoverHtml({theme:clean,spec:COVER_THEME_SPECIMEN});
