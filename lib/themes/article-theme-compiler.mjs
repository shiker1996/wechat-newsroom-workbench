...
export function compileArticleTheme(theme){
  ...
  const {tokens,article}=definition,{behavior}=article,derivedRhythm=['magazine-warm','career-essay'].includes(definition.id)?'airy':['tech-wire','news-digest','gossip-card'].includes(definition.id)?'dense':'standard',recipes={rhythm:derivedRhythm,...article.recipes},components=resolveArticleComponents(definition);
