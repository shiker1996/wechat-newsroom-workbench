...
// 许可证与署名文件允许无扩展名（LICENSE、NOTICE 等）
const ALLOWED_LICENSE_FILES = new Set(['license', 'license.md', 'license.txt', 'notice', 'notice.md', 'notice.txt']);

function allowedSkillFile(filePath) {
  if (ALLOWED_EXTENSIONS.has(path.extname(filePath).toLowerCase())) return true;
  return ALLOWED_LICENSE_FILES.has(path.basename(filePath).toLowerCase());
}
...
export function validateSkillPackageDirectory(directory) {
  ...
  for (const filePath of files) {
    ...
    if (!allowedSkillFile(filePath)) throw new Error(`技能包包含不允许的文件：${relative}`);
...
export function readSkillZip(buffer) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (!allowedSkillFile(name)) throw new Error(`ZIP 包含不允许的文件：${name}`);
