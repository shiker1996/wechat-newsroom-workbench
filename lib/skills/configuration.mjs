...
export function normalizeSkillConfig(input = {}, base = {}) {
  ...
  const authorizationInput=input.capabilityAuthorization?.capabilities??input.allowedTools??base.capabilityAuthorization?.capabilities??base.allowedTools??[];
  const tools = [...new Set(authorizationInput.map(String).filter(Boolean))];
  return {
    prompt:String(input.prompt ?? base.prompt ?? ''),
    defaultModel:String(input.defaultModel ?? base.defaultModel ?? ''),
    capabilityAuthorization:{mode:'allow-list',capabilities:tools},allowedTools:tools,
    gates:{
