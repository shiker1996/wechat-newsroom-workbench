...
export async function resolveSkillToolPolicy({ workspaceRoot, skillId, snapshot = null }) {
  ...
  const allowedCapabilities=config ? [...new Set(config.allowedTools || [])] : null;
...
export async function prepareSkillRun({ gateway, store, batchId, candidateId = null, purpose, bundles, provider, snapshotId = null, selection = null }) {
  ...
  const available=(await getToolRegistry()).listCapabilities();
  const configured=Boolean(primary?.config);
  const allowed=primary?.config?.allowedTools || [];
  const tools=historical?.snapshot.tools || (configured ? available.filter((item)=>allowed.includes(item.capability)) : available);
  if(historical){
  ...
  if(configured && allowed.length!==tools.length)throw new Error('技能工具白名单包含已禁用或不存在的能力');
  ...
  snapshot.skillConfig={...(snapshot.skillConfig||{}),defaultModel:primary?.config?.defaultModel||'',allowedTools:historical?.snapshot.skillConfig?.allowedTools||allowed,
  ...
  return {provider:selectedProvider,providerConfig:resolved,config:primary?.config||null,tools,
    allowedCapabilities:historical?.snapshot.skillConfig?.allowedTools||(configured?allowed:null),bundles,snapshotId:savedSnapshot?.id||historical?.id||null};
}
