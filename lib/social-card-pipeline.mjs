...
import { resolveEventAnalysis } from './event-fact-base.mjs';
...
export function renderStoryboardHtml({ topic, repository, pages, visualStyle='ice-blue', contentType='repository', sourceLabel='', disclosure='', channelMode='wechat' }) {
  ...
  const sections = safePages.map((page, index) => {
    ...
    const blocks = (Array.isArray(page.content_blocks) ? page.content_blocks : []).slice(0,4).map((block) => {
      ...
      if(block.type==='list'){const lines=content?content.split(/\n+/).map((item)=>item.replace(/^[-*+]\s*/, '').trim()):items.map((item)=>typeof item==='string'?item:String(item.title||item.content||''));return `<div class="content-block list-block">${title}<ul>${lines.filter(Boolean).map((item)=>`<li>${escapeHtml(item)}</li>`).join('')}</ul></div>`;}
