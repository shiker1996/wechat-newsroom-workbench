...
export class Store {
  ...
  updateBatch(id, fields) {
    const allowed = ['title', 'status', 'stage', 'note', 'max_age_hours'];
  ...
  getCardEditorial(candidateId) {
    return this.db.prepare('SELECT * FROM card_editorial_sessions WHERE candidate_row_id=?').get(Number(candidateId)) ?? {
      ...
      forbidden_claims:'',output_mode:'wechat-tool-cards',visual_style:'ice-blue',recommended_pages:6,card_plan_json:'[]',status:'DISCUSS',updated_at:''
    };
  }

  saveCardEditorial(candidateId, input) {
    const current=this.getCardEditorial(candidateId); const now=new Date().toISOString();
    const fields=['target_reader','pain_point','tool_positioning','must_highlight','must_disclose','getting_started','forbidden_claims','output_mode','visual_style','recommended_pages','card_plan_json','status'];
  ...
  overview() {
    const trackCounts = this.db.prepare(`SELECT
      ...
      SUM(CASE WHEN track='article' AND status IN ('locked','drafting','review','preview') THEN 1 ELSE 0 END) AS article_in_progress,
      SUM(CASE WHEN track='social_cards' AND status NOT IN ('pooled','published','removed') THEN 1 ELSE 0 END) AS social_in_progress
    ...
    return {
      ...
      articleCandidates: Number(trackCounts.article_candidates || 0),
      socialCandidates: Number(trackCounts.social_candidates || 0),
      articleInProgress: Number(trackCounts.article_in_progress || 0),
      socialInProgress: Number(trackCounts.social_in_progress || 0),
