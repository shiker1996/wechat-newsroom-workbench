...
async function retrySubscription({ failure, store, config, onProgress }) {
  ...
  if(sourceKey.startsWith('reddit:r/')) {
    ...
    let routes=[];let collectionScope=sourceGroup==='github'?'github':'rsshub';
    if(sourceKey.startsWith('direct:')) items=await collectFeed({url:sourceKey.slice('direct:'.length),label:failure.title},{maxAgeHours:config.rsshub?.maxAgeHours,allowUndated:config.rsshub?.allowUndated},onProgress,onSourceResult);
    ...
    if(!sourceKey.startsWith('direct:')) items=await collectRssHub({...config.rsshub,routes,disabledRoutes:[],collectionScope},onProgress,onSourceResult);
  }
