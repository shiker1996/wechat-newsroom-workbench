  ...
  return new Map([
    ['tag', async ({ job, maxAgeHours, options }) => {
      ...
      const result = await tagBatch({ gateway, store, batchId: job.batchId, provider: job.provider,
        force: Boolean(options.force), maxAgeHours, workspaceRoot: config.workspaceRoot, runId: job.id, onProgress: progress(job) });
    ...
    ['retag', async ({ job, maxAgeHours }) => {
      store.updateBatch(job.batchId, { stage: 'synthesis', status: 'running' });
      const result = await tagBatch({ gateway, store, batchId: job.batchId, provider: job.provider,
        force: true, maxAgeHours, workspaceRoot: config.workspaceRoot, runId: job.id, onProgress: progress(job) });
      store.updateBatch(job.batchId, { stage: 'synthesis', status: 'review' });
      return result;
    }],
    ['event-cards', async ({ job, maxAgeHours, options }) => {
      ...
      const result = await ensureBatchEventCards({ gateway, store, batchId: job.batchId, provider: job.provider,
        workspaceRoot: config.workspaceRoot, maxAgeHours, regenerate: Boolean(options.force), runId: job.id, onProgress: progress(job) });
      store.updateBatch(job.batchId, { stage: 'synthesis', status: 'review' });
      if (result.failed.length) throw new Error(`${result.failed.length} 张事件卡生成失败，可点击“继续生成事件卡”重试`);
      return result;
    }],
    ['research', async ({ job, maxAgeHours }) => { const result=await runResearchPipeline({ gateway, store, batchId: job.batchId,
      provider: job.provider, workspaceRoot: config.workspaceRoot, maxAgeHours, onProgress: progress(job) });
      for(const failure of store.listPipelineFailures(job.batchId,{statuses:['open','retrying'],stages:['research']}))store.resolvePipelineFailure(failure.id);
      return result; }],
    ...
    ['auto', async ({ job, batch, maxAgeHours }) => { const result=await runAutoPipeline({ gateway, store, config, job, batch, maxAgeHours,
      onProgress: progress(job) });
      for(const failure of store.listPipelineFailures(job.batchId,{statuses:['open','retrying'],stages:['research']}))store.resolvePipelineFailure(failure.id);
      return result; }],
  ]);
