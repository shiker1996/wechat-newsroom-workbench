...
export function buildSocialCardPlanRolloutReport(rows = [], { minSamples = 3 } = {}) {
  ...
  const comparisons = [...byTemplate.entries()].map(([templatePackId, items]) => {
    ...
    return {
      ...
      readyForPromotion: Boolean(enoughSamples && sourceAtomLossZero && successNotWorse && overflowNotWorse && repairNotWorse),
      gates: { enoughSamples, sourceAtomLossZero, successNotWorse, overflowNotWorse, repairNotWorse },
