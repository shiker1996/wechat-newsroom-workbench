...
const TAG_RULES = Object.freeze([
  ...
  ['run', /usage|use.?case|run|command|workflow|example|使用|运行|命令|流程|示例/i],
  ...
  ['limitation', /limit|boundar|constraint|warning|caveat|限制|边界|风险|注意/i],
...
function candidateId(path, value) {
  return `fact-${createHash('sha1').update(`${path}|${value}`).digest('hex').slice(0, 12)}`;
}
...
function collectFacts(value, path, inheritedRefs, output, rootRefs) {
  ...
  if (typeof value !== 'object') {
    ...
    const tags = addTags(path, key, claim);
...
export function buildSocialCardFactCandidatePrompt(index = {}, { roles = null, maxCandidates = 40 } = {}) {
  ...
  return JSON.stringify({ schemaVersion: SOCIAL_CARD_FACT_INDEX_SCHEMA_VERSION, roleSlots: roleText, candidates: list.map(({ id, label, display_label, text: claim, source_refs, tags, semantic_intent, semantic_intent_candidates, priority }) => ({ id, label, display_label, text: claim, source_refs, tags, semantic_intent, semantic_intent_candidates, priority })) });
}
...
export function buildSocialCardFactBlockFromCandidates(index = {}, { role = 'feature', slotId = '', blockType = 'note', factIds = [], title = '' } = {}) {
  ...
  const values = selected.slice(0, slot.maxItems).map((item) => item.text);
