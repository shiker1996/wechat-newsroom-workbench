import { cardPageDensity, listBlockValues, normalizeCoverTitleLines } from './social-card-plan.mjs';
...
import { escapeHtml, renderStoryboardBlock } from './storyboard-html-content.mjs';
...
function coverTitleMarkup(title, { topic, compiledTheme, coverTitleLines }) {
  ...
  if (!lines) {
    ...
    const lineCount = Math.max(1, Math.ceil(characters.length / 8));
    const size = Math.max(4, Math.ceil(characters.length / lineCount));
    lines = [];
...
export function renderStoryboardSections({
  ...
  sourceLabel = '', disclosure = '', channelMode = 'wechat', coverTitleLines = null,
}) {
