...
export function renderStoryboardBlock(block, { pageLayout = 'stacked', pageRole = '' } = {}) {
  ...
  const inferredSteps = pageLayout === 'steps' && pageRole === 'steps' && block.type === 'text'
    ...
    : [];
  if (inferredSteps.length) return `<div class="content-block steps-block">${title}<div class="step-col">${inferredSteps.map((item, index) => `<div class="step"><b>${index + 1}</b><div><h3>${escapeHtml(item.title)}</h3><p>${renderTechnicalText(item.content)}</p></div></div>`).join('')}</div></div>`;
  ...
  if (block.type === 'list') {
    ...
    return `<div class="content-block list-block">${title}<ul>${lines.filter(Boolean).map((item) => `<li>${renderTechnicalText(item)}</li>`).join('')}</ul></div>`;
  }
  if (block.type === 'note') return `<aside class="content-block note-block">${title}<p>${renderTechnicalText(content)}</p></aside>`;
  ...
  if (block.type === 'compare' && ((Array.isArray(block.headers) && block.headers.length) || (Array.isArray(block.rows) && block.rows.length))) {
    ...
    return `<div class="content-block compare-block">${title}<table><thead><tr>${headers.map((cell) => `<th data-text-role="auxiliary">${renderTechnicalText(cell)}</th>`).join('')}</tr></thead><tbody>${rows.map((row) => `<tr>${(Array.isArray(row) ? row : []).map((cell) => `<td>${renderTechnicalText(cell)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
  }
  if (block.type === 'steps' && items.length) return `<div class="content-block steps-block">${title}<div class="step-col">${items.map((item, index) => `<div class="step"><b>${index + 1}</b><div><h3>${escapeHtml(item.title || '')}</h3><p>${renderTechnicalText(item.content || '')}</p></div></div>`).join('')}</div></div>`;
  if (block.type === 'timeline' && items.length) return `<div class="content-block timeline-block">${title}<div class="tl">${items.map((item) => `<div class="tl-node"><span class="tl-time" data-text-role="auxiliary">${escapeHtml(item.time || '')}</span><h3>${escapeHtml(item.title || '')}</h3><p>${renderTechnicalText(item.content || '')}</p></div>`).join('')}</div></div>`;
  if (block.type === 'scenes' && items.length) return `<div class="content-block scenes-block">${title}<div class="scene-row">${items.map((item) => `<div class="scene"><h3>${escapeHtml(item.title || '')}</h3><p>${renderTechnicalText(item.content || '')}</p></div>`).join('')}</div></div>`;
  if (block.type === 'scenes' && content) {
    const lines = content.split(/\n+/).map((item) => item.replace(/^[-*+•·✓✔✅☑]\uFE0F?\s*/u, '').trim()).filter(Boolean);
    return `<div class="content-block list-block scenes-block">${title}<ul>${lines.map((item) => `<li>${renderTechnicalText(item)}</li>`).join('')}</ul></div>`;
  }
  if (block.type === 'highlight') return `<div class="content-block highlight-block">${title}<p>${renderTechnicalText(content)}</p></div>`;
  if ((block.type === 'steps' || block.type === 'timeline') && content) {
    const lines = content.split(/\n+/).map((item) => item.replace(/^[-*+]\s*/, '').trim()).filter(Boolean);
    return `<div class="content-block list-block">${title}<ul>${lines.map((item) => `<li>${renderTechnicalText(item)}</li>`).join('')}</ul></div>`;
  }
  return `<div class="content-block text-block">${title}<p>${renderTechnicalText(content)}</p></div>`;
}
