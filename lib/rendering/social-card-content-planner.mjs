...
import { socialCardSlotSemanticTags } from './social-card-page-component-contract.mjs';
 ...
 */
export function buildSocialCardContentPlannerPrompt({ facts = {}, layoutReport = {}, cardPlan = [], contentAtoms = [], templateCapabilities = {}, factIndex = null, contentComponents = null, maxPages = 10, recommendedPages = null, absoluteMaxPages = null, maxOperations = 4, maxFactBlocksPerRound = 2, maxFactBlocksPerPage = 1 } = {}) {
  ...
  const pageCapExceeded = Number(cardPlan.length) > softMaxPages;
  const absolutePageCapExceeded = Number(cardPlan.length) > hardMaxPages;
  return [
    ...
    ...(pageCapExceeded ? [`当前计划已有 ${cardPlan.length} 页，超过模板允许的 ${softMaxPages} 页（推荐页数）；本轮优先使用 merge_pages 合并相邻同故事线续页或移动完整内容块后再合并，但没有安全合并时不要为了回到推荐页数而删除事实。仅 add_component 不能解决超页问题。`] : []),
    ...(absolutePageCapExceeded ? [`当前计划已有 ${cardPlan.length} 页，超过绝对安全上限 ${hardMaxPages} 页；必须优先合并安全续页，无法降至绝对上限时返回 {"operations":[]}，程序将阻断而不会截断事实。`] : []),
