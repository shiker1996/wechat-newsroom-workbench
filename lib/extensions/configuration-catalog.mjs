...
function modelResources(config){return Object.entries(config.llm?.providers||{}).map(([id,provider])=>({id,name:provider.label||id,type:'model-provider',kind:'model-provider',manifest:modelProviderManifest(id,provider)}));}
