import { fetchUrlContent } from '../integrations/source-fetcher.mjs';
import { CUSTOM_CONTENT_TYPES, CUSTOM_SOURCE_LEVELS } from './social-card-gate.mjs';
...
export async function buildCustomFactSheet({ input, root, fetchImpl = fetchUrlContent }) {
  ...
  if (!points.some((item) => item.source_level !== 'model_suggestion')) throw new Error('至少需要一条要点来自作者真实体验（【体验】）或用户提供素材（【素材】）');
