...
function routeKind(route) {
  return /^\/twitter\/user\//i.test(route) ? 'twitter' : 'rsshub';
}
...
export function subscriptionTestInput(input) {
  const kind = cleanText(input.kind, 20);
  if (kind === 'twitter') return { kind, value: `/twitter/user/${twitterHandle(input.value)}?limit=3` };
  if (kind === 'rsshub') return { kind, value: normalizeRoute(input.value).replace(/([?&])limit=30(?:&|$)/, '$1limit=3&').replace(/&$/, '') };
