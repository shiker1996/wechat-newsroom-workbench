...
async function fetchSingleSource({store,hotspotId,url,hotspotTitle='',root,force,urlOverride}) {
  ...
  parsed.title=parsed.title||hotspotTitle;
  const cachePath=path.join(root,'data','source-cache',`${hotspotId}.json`);
  const record={...parsed,cache_path:cachePath}; writeJson(cachePath,record);
  return store.saveHotspotSource(hotspotId,record);
}

export async function fetchCandidateSource({store,candidateId,root,force=false,urlOverride=''}) {
  const candidate=store.getCandidate(candidateId); if(!candidate)throw new Error('候选不存在');
  if (candidate.composite) {
    const hotspots=candidate.hotspots||[];
    const allUrls=urlOverride?[{hotspotId:0,url:urlOverride}] : hotspots.map(h=>({hotspotId:h.id,url:h.url,title:h.title}));
