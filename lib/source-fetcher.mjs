...
// 阈值默认值与 lib/config.mjs 的 sourceFetch 段一致，调用方可用 config.sourceFetch 覆盖
...
export async function fetchUrlContent({targetUrl,title='',root,hotspot=null,firecrawlImpl=firecrawlScrape,inspectImpl=inspectRepository,pythonImpl=null,sourceFetch={}}) {
...
async function fetchSingleSource({store,hotspotId,url,hotspotTitle='',root,force,urlOverride,sourceFetch}) {
  ...
  const parsed=await fetchUrlContent({targetUrl,title:hotspotTitle,root,hotspot,sourceFetch});
...
export async function fetchMaterialSource({store,material,root,force=false,sourceFetch}) {
  if(!material?.url)throw new Error('素材没有可抓取 URL');
  if(!force&&material.status==='ok'&&material.content_chars>0)return material;
  const parsed=await fetchUrlContent({targetUrl:material.url,title:material.title,root,sourceFetch});
...
export async function fetchCandidateSource({store,candidateId,root,force=false,urlOverride='',hotspots=null,sourceFetch}) {
  ...
  if (list) {
    ...
    const results=await Promise.allSettled(allUrls.map(({hotspotId,url,title})=>
      fetchSingleSource({store,hotspotId,url,hotspotTitle:title||'',root,force,urlOverride:hotspotId===0?urlOverride:'',sourceFetch})));
  ...
  return fetchSingleSource({store,hotspotId:candidate.hotspot_id,url:candidate.url,hotspotTitle:candidate.hotspot_title,root,force,urlOverride,sourceFetch});
}
