import { requestGitHubJson } from './github-api.mjs';
...
export function parseRepositoryUrl(value) {
  try {
    ...
    return {owner,repo:repo.replace(/\.git$/i,''),repository:`${owner}/${repo.replace(/\.git$/i,'')}`,sourceUrl:`https://github.com/${owner}/${repo.replace(/\.git$/i,'')}`};
  } catch{return null;}
}
