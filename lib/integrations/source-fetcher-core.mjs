...
export async function fetchCandidateSourceImplementation({store,candidateId,root,force=false,urlOverride='',hotspots=null,sourceFetch,toolContext,fetchImpl=fetchUrlContentImplementation}) {
  ...
  if (list) {
    ...
    const results=await Promise.allSettled(allUrls.map(({hotspotId,url,title})=>
      fetchSingleSource({store,hotspotId,url,hotspotTitle:title||'',root,force,urlOverride:hotspotId===0?urlOverride:'',sourceFetch,toolContext,fetchImpl})));
