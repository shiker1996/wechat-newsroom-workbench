...
import { CandidateSelectionService } from '../application/candidate-selection-service.mjs';
...
export class Store {
  constructor(dbPath) {
    ...
    this.services = Object.freeze({
      candidateSelection: new CandidateSelectionService(this.db, this.repositories, this.queries.candidates),
  ...
  finishSourceRun(id, status, itemCount = 0, error = null) {
    return this.repositories.sourceRuns.finish(id, status, itemCount, error);
  }
  ...
  recordSubscriptionRun(batchId, result) {
    return this.repositories.sourceRuns.recordSubscription(batchId, result);
  }
  ...
  listToolExecutions({ batchId = null, candidateId = null, capability = null, limit = 100 } = {}) {
    return this.repositories.runtimeAudit.listToolExecutions({ batchId, candidateId, capability, limit });
  }
