...
function socialCandidatePresentation(rawJson, factSheet, socialScore) {
  ...
  // 描述用打标产出的中文相关度理由：能进图文池的热点必经打标，不再回退英文简介（手动补选等未打标场景留空，前端不渲染该行）
  ...
  if(channels.includes('ai-search')||raw.sourceType==='ai-search')reasons.push(raw.interestScore!=null?`AI 兴趣发现 · 兴趣契合 ${raw.interestScore}/10`:'AI 兴趣发现');
...
export class Store {
  ...
  migrate() {
    ...
    if(!modelCallColumns.has('reasoning_tokens'))this.db.exec('ALTER TABLE model_calls ADD COLUMN reasoning_tokens INTEGER');
    ...
    const themeTableSql=(name)=>this.db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name=?").get(name)?.sql||'';
    if(themeTableSql('theme_definitions').includes("CHECK(target IN ('article','social'))")||themeTableSql('theme_usage').includes("CHECK(target IN ('article','social'))")){
      ...
      try {
        if(themeTableSql('theme_usage').includes("CHECK(target IN ('article','social'))")){
          this.db.exec(`CREATE TABLE theme_usage_new (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            theme_id TEXT NOT NULL,
            version TEXT NOT NULL,
            target TEXT NOT NULL CHECK(target IN ('article','social','cover')),
          ...
          this.db.exec('INSERT INTO theme_usage_new (id,theme_id,version,target,source,batch_id,candidate_row_id,used_at) SELECT id,theme_id,version,target,source,batch_id,candidate_row_id,used_at FROM theme_usage');
          this.db.exec('DROP TABLE theme_usage');
          this.db.exec('ALTER TABLE theme_usage_new RENAME TO theme_usage');
        }
        if(themeTableSql('theme_definitions').includes("CHECK(target IN ('article','social'))")){
          this.db.exec(`CREATE TABLE theme_definitions_new (
            id TEXT PRIMARY KEY,
            owner_scope TEXT NOT NULL DEFAULT 'workspace',
            target TEXT NOT NULL CHECK(target IN ('article','social','cover')),
            label TEXT NOT NULL,
          ...
          this.db.exec('INSERT INTO theme_definitions_new (id,owner_scope,target,label,source,active_version_id,status,draft_json,created_at,updated_at) SELECT id,owner_scope,target,label,source,active_version_id,status,draft_json,created_at,updated_at FROM theme_definitions');
          this.db.exec('DROP TABLE theme_definitions');
          this.db.exec('ALTER TABLE theme_definitions_new RENAME TO theme_definitions');
        }
  ...
  // 按 ON DELETE SET NULL 脱钩保留。产物目录清理由 lib/domain/batch-deletion.mjs 负责。
  deleteBatch(id) {
    return this.db.prepare('DELETE FROM batches WHERE id=?').run(id).changes > 0;
  }
  ...
  listCandidateSources(candidateRowId) {
    return this.db.prepare('SELECT * FROM candidate_sources WHERE candidate_row_id=? ORDER BY url').all(Number(candidateRowId));
  }

  saveCandidateSource(candidateRowId, input) {
    this.db.prepare(`INSERT INTO candidate_sources
      (candidate_row_id,url,final_url,status,title,description,author,published_at,content,content_chars,fetched_at,error,cache_path,fetch_method,quality_json,evidence_level)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(candidate_row_id,url) DO UPDATE SET final_url=excluded.final_url,status=excluded.status,
        title=excluded.title,description=excluded.description,author=excluded.author,published_at=excluded.published_at,
        content=excluded.content,content_chars=excluded.content_chars,fetched_at=excluded.fetched_at,error=excluded.error,cache_path=excluded.cache_path,fetch_method=excluded.fetch_method,
        quality_json=excluded.quality_json,evidence_level=excluded.evidence_level`)
      .run(Number(candidateRowId),input.url||'',input.final_url||'',input.status||'error',input.title||'',input.description||'',input.author||'',
        input.published_at||'',input.content||'',Number(input.content_chars)||0,input.fetched_at||new Date().toISOString(),input.error||'',input.cache_path||'',input.fetch_method||'',
        input.quality?JSON.stringify(input.quality):(input.quality_json||''),input.evidence_level||'');
    return this.db.prepare('SELECT * FROM candidate_sources WHERE candidate_row_id=? AND url=?').get(Number(candidateRowId),input.url||'') ?? null;
  }
  ...
  recordModelCall(input) {
    ...
    const result = this.db.prepare(`INSERT INTO model_calls
      ...
      input.completionTokens ?? null, input.reasoningTokens ?? null, input.compressed ? 1 : 0, input.latencyMs ?? 0,
  ...
  listUserThemes({target=null,includeArchived=false}={}) {
    const where=[];const params=[];if(target){where.push('d.target=?');params.push(target);}if(!includeArchived)where.push("d.status!='archived'");
  ...
  getUserTheme(id) { return this.db.prepare(`SELECT d.*,v.version AS active_version,v.definition_json AS active_definition_json,v.content_hash AS active_hash
  ...
  saveUserThemeDraft({id,target,label,definitionJson}) {
    const now=new Date().toISOString(),existing=this.getUserTheme(id);
    ...
    return this.getUserTheme(id);
  }

  publishUserTheme({id,version,definitionJson,contentHash}) {
    const now=new Date().toISOString();this.db.exec('BEGIN IMMEDIATE');try{const result=this.db.prepare(`INSERT INTO theme_versions(theme_id,version,schema_version,definition_json,content_hash,status,created_at,published_at) VALUES(?,?,1,?,?,'published',?,?)`).run(id,version,definitionJson,contentHash,now,now);this.db.prepare("UPDATE theme_definitions SET active_version_id=?,status='published',draft_json=?,updated_at=? WHERE id=?").run(result.lastInsertRowid,definitionJson,now,id);this.db.exec('COMMIT');}catch(error){this.db.exec('ROLLBACK');throw error;}return this.getUserTheme(id);
  }

  archiveUserTheme(id) { const result=this.db.prepare("UPDATE theme_definitions SET status='archived',updated_at=? WHERE id=?").run(new Date().toISOString(),id);return result.changes?this.getUserTheme(id):null; }
  userThemeVersions(id) { return this.db.prepare('SELECT id,theme_id,version,schema_version,content_hash,status,created_at,published_at FROM theme_versions WHERE theme_id=? ORDER BY id DESC').all(id); }
  getUserThemeVersion(id,version) { return this.db.prepare('SELECT * FROM theme_versions WHERE theme_id=? AND version=?').get(id,version)||null; }
  recordThemeUsage({themeId,version,target,source,batchId=null,candidateId=null}) { this.db.prepare('INSERT INTO theme_usage(theme_id,version,target,source,batch_id,candidate_row_id,used_at) VALUES(?,?,?,?,?,?,?)').run(themeId,version,target,source,batchId,candidateId,new Date().toISOString()); }
  themeUsageStats(id) { const summary=this.db.prepare('SELECT COUNT(*) AS usage_count,MAX(used_at) AS last_used_at,COUNT(DISTINCT batch_id) AS batch_count FROM theme_usage WHERE theme_id=?').get(id);const versions=this.db.prepare('SELECT version,COUNT(*) AS usage_count,MAX(used_at) AS last_used_at FROM theme_usage WHERE theme_id=? GROUP BY version ORDER BY last_used_at DESC').all(id);return {usageCount:Number(summary.usage_count||0),lastUsedAt:summary.last_used_at||null,batchCount:Number(summary.batch_count||0),versions}; }
  themeArchiveImpact(id) { const theme=this.getUserTheme(id),usage=this.themeUsageStats(id);return {themeId:id,exists:Boolean(theme),status:theme?.status||null,activeVersion:theme?.active_version||null,usageCount:usage.usageCount,batchCount:usage.batchCount,lastUsedAt:usage.lastUsedAt,historicalVersions:this.userThemeVersions(id).length,canArchive:Boolean(theme&&theme.status!=='archived'),physicalDeleteAllowed:Boolean(theme&&theme.status==='archived'&&usage.usageCount===0)}; }
}
