...
export class Store {
  ...
  getBatch(id) {
    ...
    batch.ai_status = {
      ...
      latestResearch: batch.ai_runs.find((item) => item.type === 'research' || item.type === 'auto') ?? null,
    };
    return batch;
  }
  ...
  addManualHotspot(batchId, { title, url, category, notes, materialUrls = [] }) {
    const safeTitle = String(title || '').trim();
  ...
  getCardEditorial(candidateId) {
    return this.db.prepare('SELECT * FROM card_editorial_sessions WHERE candidate_row_id=?').get(Number(candidateId)) ?? {
      ...
      forbidden_claims:'',output_mode:'wechat-tool-cards',visual_style:'ice-blue',layout_style:'auto',recommended_pages:6,card_plan_json:'[]',status:'DISCUSS',updated_at:''
    };
  }

  saveCardEditorial(candidateId, input) {
    const current=this.getCardEditorial(candidateId); const now=new Date().toISOString();
    const fields=['target_reader','pain_point','tool_positioning','must_highlight','must_disclose','getting_started','forbidden_claims','output_mode','visual_style','layout_style','recommended_pages','card_plan_json','status'];
  ...
  overview() {
    ...
    const efficiency = latest ? (() => {
      const source = this.db.prepare(`SELECT
        COALESCE(SUM(duration_ms),0) AS cumulative_duration_ms,
        COALESCE(SUM(CASE WHEN id IN (
          SELECT MAX(id) FROM subscription_runs WHERE batch_id=? GROUP BY source_key
        ) THEN duration_ms ELSE 0 END),0) AS latest_duration_ms
        FROM subscription_runs WHERE batch_id=?`).get(latest.id,latest.id);
      ...
      const bottleneck=current.failedRuns?"存在失败任务，优先处理重试"
        ...
        :"当前生产链没有明显阻塞";
      return {
        collectionDurationMs:Number(source.latest_duration_ms||0),
        collectionCumulativeDurationMs:Number(source.cumulative_duration_ms||0),
        collectionRetryDurationMs:Math.max(0,Number(source.cumulative_duration_ms||0)-Number(source.latest_duration_ms||0)),
    ...
    })() : { collectionDurationMs:0, collectionCumulativeDurationMs:0, collectionRetryDurationMs:0, aiSuccessRate:null, candidateConversionRate:null, artifactCount:0, bottleneck:"建立当前批次后开始记录效率" };
    const baselineRows=latest?this.db.prepare(`SELECT b.id,
      COALESCE((SELECT SUM(duration_ms) FROM subscription_runs sr WHERE sr.batch_id=b.id),0) AS collection_duration_ms,
    ...
    const efficiencyBaseline={
      ...
      collectionDurationMs:average(baselineRows.map((row)=>Number(row.collection_duration_ms||0))),
