...
// 行为逐字节一致；messageKey 声明该档案在 options.messages 中使用的文案 key。
export const RESOURCE_KIND_PROFILES=Object.freeze({
  'project-path':{
    schema:RESOURCE_ID_SCHEMA,messageKey:'project',
    resolve(resource,args,{messages}){if(!resource?.path)throw resourceNotAllowed(messages.project||'项目资源不属于当前请求');return {path:resource.path,options:{}};},
  },
  'url-fetch':{
    schema:RESOURCE_ID_SCHEMA,messageKey:'urlFetch',
    resolve(resource,args,{messages,workspaceRoot}){if(!resource?.url)throw resourceNotAllowed(messages.urlFetch||'URL 资源不属于当前请求');return {targetUrl:resource.url,...(resource.title?{title:resource.title}:{}),root:workspaceRoot};},
  },
  'document-root':{
    schema:RESOURCE_ID_QUERY_SCHEMA,messageKey:'documentSearch',
    resolve(resource,args,{messages}){if(!resource?.root)throw resourceNotAllowed(messages.documentSearch||'文档目录未授权');return {root:resource.root,query:String(args?.query||'').slice(0,300),maxResults:5};},
  },
  'github-url':{
    schema:RESOURCE_ID_SCHEMA,messageKey:'repository',
    resolve(resource,args,{messages}){if(!resource?.url||!/^https:\/\/github\.com\//i.test(resource.url))throw resourceNotAllowed(messages.repository||'仓库不属于用户授权的 GitHub 素材');return {sourceUrl:resource.url};},
  },
  'passage-content':{
    schema:PASSAGE_RESOURCE_SCHEMA,messageKey:'passage',
    // resourceIds 命中资源目录时按已抓取正文严格执行；否则透传（tutorial/custom-social 的兼容回退）
    resolve(resource,args,{resources,messages}){
      ...
      if(!selected.length||selected.some((item)=>!item?.content))throw resourceNotAllowed(messages.passage||'段落资源不存在、未抓取或不属于当前候选');
...
export function resolveCatalogResourceProfiles(root){
  ...
  for(const [id,value] of Object.entries(parsed.capabilities||{})){
    ...
    if(!kind)continue;
    if(!RESOURCE_KIND_PROFILES[kind])throw new Error(`能力目录条目 resourceKind 无效：${id}（${kind}）`);
...
// options.messages：各入口既有的报错文案，逐条保持与迁移前一致；
...
export function resolveResourceArguments(args,request,{resources,workspaceRoot,messages={},searchMaxResults=5,profiles}={}){
  ...
  if(profile){
    ...
    return profile.resolve(resource,args,{resources,workspaceRoot,messages});
  }
...
export function buildAdaptation({adaptation={},inputs={},workspaceRoot,store,batchId,messages={},searchMaxResults=5}={}){
  ...
  return {
    ...
    sanitizeToolResult:(result,request,{agentRunId}={})=>{
      ...
      return handler(result,request,{store,batchId,agentRunId,state,options:handlerOptions,inputs});
    },
  };
}

// 结果处理器注册表（阶段 1 代码内注册表）。
// ctx 形态：{store, batchId, agentRunId, state, options, inputs}；state 为 buildAdaptation 提供的可变对象
...
export const RESULT_HANDLERS=Object.freeze({
  ...
  'fact-attachment':(result,request,{store,batchId,agentRunId,state,options={}}={})=>{
    if(result?.status!=='ok')return result;
