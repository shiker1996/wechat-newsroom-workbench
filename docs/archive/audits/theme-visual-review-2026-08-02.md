...
- 20 个内置主题在 `tags` 中声明内容场景标签（白名单：深度观点 / 技术教程 / 快讯 / 数据对比 / 事件图文），复用既有字段，无 schema 变更。
- `/api/themes` 目录条目新增 `scenes`（标签∩白名单）与 `density`（文章由 `rhythm` 推导、图文由密度阈值高字段数推导为 宽松/适中/紧凑）。
- 选择器对话框新增"全部场景"下拉筛选（`#theme-picker-scene`），主题卡展示场景标签与阅读密度；与既有 全部/内置/我的主题 筛选叠加生效。
- 回归：新增 2 项目录契约测试（场景/密度白名单与取值、选择器 UI 接线）。
