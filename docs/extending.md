...
| 远程工具插件 | [examples/remote-tool-plugin](./examples/remote-tool-plugin/) | 安装时由 `lib/tools/remote-package-manager.mjs` 校验 | 声明式 Manifest 登记 |
...
- **版本兼容**：`compatibleApp` 与当前应用版本（`lib/version.mjs`）比较；同技能多版本保留安装记录，可在界面切换启停。
...
- **失败语义**：健康检查失败或执行抛错时，该能力调用返回结构化错误并写入执行审计（`lib/tools/execution-log.mjs`），不中断所在流水线；流水线按「无此工具」降级继续。
...
- **权限说明**：`riskLevel` 仅允许 `network-read` / `external-write`；`external-write` 插件会向第三方发送内容，安装界面必须明确提示。凭据按 `credentialProfile` 隔离存储（`lib/tools/remote-credentials.mjs`），不写入数据库与备份。
...
4. 扩展点契约（entryPoints、input/outputContract、能力槽位）以 `lib/skills/skill-manifest.schema.json` 与 `lib/tools/manifest-loader.mjs` 为准。
