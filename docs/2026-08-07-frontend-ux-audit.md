...
- 已修复：
  ...
  - 可访问性：#32 #33 #34 #45
  - 布局与样式体系：#36 #61 #64 #69
  - 交互决策与收尾：#30 #35 #39 #41 #43 #52 #71 #73

### 第九轮：交互决策与收尾（#30 #35 #39 #41 #43 #52 #71 #73）

- **#30**：标题同步改为「仅当正文 H1 与标题原本一致时才联动改写」（`lastTitleValue` 跟踪），用户刻意不同的 H1 不再被静默覆盖。
- **#39**：`char-count` 分母由门禁上限 2000 改为写作目标 `currentWritingGoal()`（默认 1500），与「目标 1,500 字」统计同口径。
- **#35**：RSSHub KV 行新增「有 N 项待删除，保存后生效」提示；新行删除文案 ×/移除 统一为「删除」，aria-label 统一。
- **#41**（保守取舍）：skills 状态更新仍全量重渲染（局部更新需复制派生渲染逻辑，易与服务端状态漂移），但重渲染后恢复触发控件焦点；搜索词/筛选值在重渲染节点之外天然保留。
- **#43**：关闭 AI 主题创建器时清理 `aiCandidate` 与候选面板。
- **#52**：主题预览 iframe `sandbox=""`（发布门禁已拒绝一切脚本/事件属性/javascript:，预览不依赖同源能力），AI 候选 iframe 一并收紧。
- **#71**：subscriptions/artifacts 的 document 级委托加 `closest('#view-xxx')` 容器守卫。同模式其余 7 处（calendar/editorial/topics/preview/atlas/batch-drawer/social-editor）data 属性当前无冲突，记录在案未改。
- **#73**：createBreakingBatch 增加 URLs 格式校验（new URL 逐条）与标题非空校验；createBatch 增加日期有效性校验，标题为空时回退默认命名。

### 第八轮：布局与样式体系（#36 #61 #64 #69）

- **#69**：`:root` 新增 11 个 `--z-*` 变量（raised/sticky/elevated/dropdown/bar/toolbar/rail/noise/toast/modal/zoom），全文件 27 处 z-index 字面量替换为变量引用，数值层级不变。
- **#36**（保守方案）：专注模式改纯 flex 撑满视口，去掉写死 calc；普通模式 desk `min-height` 600→360 缓解矮屏嵌套滚动，但 `height:calc(100vh - 245px)` 保留（整体 flex 化会波及全部滚动式视图，风险过大）；清理被覆盖的死规则；`.editorial-messages` 上限改 `min(390px,52vh)`。手机预览 iframe 650px 保留。**经 WebBridge 1280×720 视口实测**：编辑器与编辑会页面表现正常，普通模式页面滚动 + desk 内滚仍存在（已知取舍）。
- **#61**：dashboard `className=""` 改 `classList.remove("empty-state")`。
- **#64**：内联样式收口为 `.modal-form-intro`/`.required-mark`/`.drawer-action-row` 三个类；topics.js 三处内联 `font-size:11px` 系冗余（父容器已有），直接删除；calendar.js 已无内联样式。

### 第七轮：可访问性（#32 #33 #34 #45）

- **#32**：dashboard 最新批次卡片明确整卡可点——`role="button"` + tabindex + aria-label + Enter/Space 键盘支持；内嵌按钮改纯视觉 span；点击加选区守卫，拖拽选中文本不再误开抽屉（batches.js 台账行同受益）。**取舍**：保留整卡可点交互面积，未改按钮独立绑定。
- **#33**：新增 `.inline-button` 样式，editorial 历史覆盖、calendar 日历条目、social-editor 相似卡片的假 span 均改 `<button type="button">`；atlas SVG 节点补 tabindex/role/aria-label + Enter/Space，抽出 toggleGraphNode 共用，键盘操作后焦点还原。
- **#34**：全部筛选 tab 组统一为 `role="tablist"` + `role="tab"` + `aria-selected`（atlas 两处、批次状态、技能分区、来源筛选、日志类型、skills capability tab），JS 切换逻辑同步；真正的 toggle 按钮保留 aria-pressed 未动。
- **#45**：preview 图片遮罩重写——Esc 具名监听所有关闭路径统一解绑，模块级 activeZoomClose 防重复放大泄漏；加 role="dialog" aria-modal、可见关闭按钮、焦点进入/还原；点击图片本身不再关闭（更符合 dialog 语义）。
...
- **#53**：styles.css 删除被后段覆盖的 `.editorial-layout` grid 列与 `.candidate-sidebar` 重复段；preview.js 封面按钮改按候选门控；models.js 移除未使用 import；theme-manager.js 化简无意义三元。
- **#57**：新增 `core/constants.js`——AUTOSAVE_DELAY_MS(1200)、DRAFT_SCORE_THRESHOLD(55)、MAX_IMAGE_BYTES(8MB)、LOG_LIST_LIMIT(150)、JOB_POLL_INTERVAL_MS(1500，tutorial/daily 1800 与 editorial 1200 收敛)、GRAPH_ZOOM_STEP(1.12，按钮 1.2 收敛到滚轮值)。main.js 的 4000ms 通知轮询为独立场景，保留独立命名。
- **#66**：新增 `.gitattributes`（`text=auto eol=lf`）。注：库内 blob 本已全为 LF，CRLF 只是 `core.autocrlf=true` 的检出产物；工作区残留 CRLF 已归一，剩余 `var` 声明改 const/let。
- **#72**：index.html 批次切换器加初始占位 option（"选择批次"）。
