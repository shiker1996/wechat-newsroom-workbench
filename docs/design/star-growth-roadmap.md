...
- [x] 加 2–3 张截图到标题下：`docs/screenshots/` 已有工作台总览 / 热点全景 / 文章选题池 / 图文选题池四张整页截图（`scripts/media/render-ui-shots.mjs` 可重新生成）。README 顶部最终采用「演示封面图 + 点击播放链接」（`docs/screenshots/ui-demo-cover.png`，`scripts/media/render-demo-cover.mjs`；GitHub README 不支持 `<video>` 标签，实测会被渲染器剥离，故用封面图链接到 CDN mp4）。
