...
| F 成稿门槛 | 已有 `T/J/A/C/F` 计算和 `F < 55` 默认不进文章池；H/B/P、S/D 仅作内部解释 | `scoreCards()`、`candidate-selection-service.mjs` |
...
| F | 已拆出 `A` 和研判价值 `J`，按 `F = A×55% + T×25% + J×20% − C` 计算；`C=S+D` 仅作为竞争扣分内部解释 | 连续批次运营校准仍待完成 |
...
- `S`：同题饱和；
- `D`：同一事件、同一论点和跨日重复。
- `J`：事件内反常/利益冲突/发散，以及事件间前后/回应/对比/趋势关系形成的研判价值；
- `C`：竞争与重复扣分，等于 `S + D`。

T 和 J 各进入 F 一次，默认权重分别为 25% 和 20%；热榜外事件不能通过 F 被“救回”。H/B/P 保留为 A 的内部拆分，S/D 保留为 C 的内部拆分，不再作为页面上的并列主维度。
...
- `scoreCards()` 先计算文章化质量 A 和研判价值 J，再按 `F = A×55% + T×25% + J×20% − C` 计算最终分；
- `eventValueWeight` 默认 0.25，新增 `researchValueWeight` 默认 0.20；两者分别限制在 0.25–0.40、0.10–0.30；
- 持久化 `event_value`（T）、`article_value`（A）和原有 `f_score`（F），保留 `topic_value` 兼容；
- 持久化 `research_value`（J）和 `competition_penalty`（C）；报告和选题卡主展示 `T/J/A/C/F`，H/B/P、S/D 仅在详情中解释；
...
- 每个研判批次生成 `score-dual-run.json` 和 `score-dual-run.md`，并行记录旧公式与当前 `T/J/A/C/F` 公式；双跑只做审计，不改变当前新公式排序；
...
- 指标只用于运营校准，T/J 权重仍由 `scoring.eventValueWeight`、`scoring.researchValueWeight` 控制。
