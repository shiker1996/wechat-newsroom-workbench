...
- 没有 `split_policy`：列表、步骤、时间线默认允许按完整条目拆分；代码块、指标卡和 CTA 默认不可拆分；
...
3. 时间线按完整节点拆分；
4. 对比表按完整行拆分；
5. 普通文本块只有在存在明确段落边界时才拆分；
6. 代码块、指标卡、CTA 和事实边界卡默认不拆分。
...
- Social 生产链路在模板渲染前执行预检：列表、步骤、时间线、场景和对比表按完整成员拆分；普通文本只在存在明确段落边界时拆分；代码块、指标卡和 CTA 等不可拆原子块不会被静默切掉。
