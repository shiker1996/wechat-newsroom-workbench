...
现有 Social 图文已经具备“核心组件 + 补充组件 + 固定模板盒子”的装箱骨架，但补充组件仍然从全局事实索引直接生成。事实索引中的字段名（例如 `coreCapabilities`、`sections`、`value`）可能泄漏到页面，AI 也仍然直接返回 `slot_id`，导致组件语义、页面角色和槽位校验相互耦合。
...
- 装箱器优先消费当前页面的专属候选，续页或历史页面缺少专属快照时才回退到全局来源池；
