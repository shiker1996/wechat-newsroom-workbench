...
> 状态速览（2026-08-01）：已知问题 3 项全部闭环；开源前置工作全部完成，仓库已翻 Public（shiker1996/wechat-newsroom-workbench）；项目日志已完善。三个信息工具能力全部实现并接入自主写作与自定义图文（检索自动执行、工具页统一启停）。技能调用链可视化已评估不实施（见对应章节）。文章配图接入图文确定性生成链已完成（IMG-DATA 可生成占位 + 工作台一键生成）。安装引导 CLI 已完成（`npm run setup`）。热点事实补充接入搜索已评估不实施，替代方案编辑室两步备料已落地。当前无进行中待办。
