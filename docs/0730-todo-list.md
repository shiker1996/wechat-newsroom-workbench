...
> 状态速览（2026-07-31）：已知问题 3 项全部闭环；开源前置工作全部完成，仓库待翻 Public；项目日志已完善（缘起、目标、思路转折历程已写入 `project-development-record.md`）。技能插件扩展点：网络 / 新闻搜索槽位已实现（`tavily-search` 内置插件），剩余待办——搜索能力接入创作流程（步骤 4）、文档检索槽位、技能调用链可视化。
...
4. ⬜ 将网络搜索接入自主写作资料发现，将新闻搜索接入热点事实补充（槽位已有实现，但创作流程尚未调用，是当前最大缺口）。
5. ⬜ 将文档检索接入用户主动选择的知识库场景，不默认扫描全部文档。
6. ✅ 搜索插件回归测试（缺凭据、结果归一化、新闻发布时间告警、健康检查）；⬜ 超时、限流、空结果、插件切换的端到端回归随步骤 4 补齐。
