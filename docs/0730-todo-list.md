...
> 状态速览（2026-07-31）：已知问题 3 项全部闭环；开源前置工作全部完成，仓库待翻 Public；项目日志已完善（缘起、目标、思路转折历程已写入 `project-development-record.md`）。技能插件扩展点：三个信息工具能力全部实现并接入自主写作与自定义图文（tavily-search 联网/新闻搜索 + document-folder-search 本地知识库检索）。剩余待办——技能调用链可视化（规划池）。
...
4. ✅ 网络搜索与新闻搜索已接入创作流程（2026-07-31）：自主写作（心得经验 / 使用教程）与自定义图文在创建事实基座时自动执行检索，不在创作表单提供单独开关，启停统一由「技能与工具 → 信息工具」槽位配置管控；结果作为带来源的外部素材持久化进事实基座（`web_search` / `news_search` 字段），重新生成复用不重复计费；检索失败记录为事实基座备注，不阻止创建。热点事实补充的接入仍待评估（编辑室链路已有 Tavily 注入先例，暂不重复建设）。
...
1. `article-image-placeholders` 占位格式扩展：新增「可生成」类别（区别于现有「手动供图」），占位附结构化数据（时间点 / 指标 / 数值），数据只能来自事实基座，不允许模型现编。
2. 配图规划技能规则：明确哪些配图需求归类为可生成信息图（事件线、数据卡、统计卡），其余来源图 / 实拍图 / 版权图仍必须作者提供，缺失阻断的纪律不变。
3. typeset 管线生成路由：遇到「可生成」占位时调用图文管线的确定性组装（复用 `composition-contract` 对应构图）+ 截图 → `upyun-upload-image` 传 CDN → URL 写回文章。
4. 回归测试：占位分类、数据来自事实基座的校验、typeset 路由与写回。

参考约束（来自外部技能 article-canvas-html-images-mobile，仅存规则不安装）：浅色风格强制、375/390px 视口、正文 ≥15px、deviceScaleFactor 2-3、单图信息密度过高拆图。
