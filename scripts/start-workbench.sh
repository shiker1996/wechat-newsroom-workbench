#!/usr/bin/env bash
# scripts/start-workbench.ps1 的 Linux/macOS 对应版本。
# 用法: scripts/start-workbench.sh [--port 4317] [--no-browser]
set -e
cd "$(dirname "$0")/.."
PORT=4317
NO_BROWSER=0
...
if ! health_ok; then
  if command -v ss >/dev/null 2>&1 && ss -ltn 2>/dev/null | grep -q ":$PORT "; then
    echo "端口 $PORT 已被其他进程占用，请先停止该进程（旧工作台可运行 scripts/stop-workbench.sh）。" >&2
    exit 1
  fi
  mkdir -p logs
