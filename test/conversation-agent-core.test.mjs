...
test('Agent 协议拒绝未知字段、重复 requestId 和混合 final/tool 输出',()=>{
  ...
  assert.throws(()=>validateAgentEnvelope({type:'tool_requests',assistant_note:'',requests:[
    {requestId:'tr_a',capability:'cap_content_demo_read',arguments:{query:'a'},reason:'a'},
    {requestId:'tr_a',capability:'cap_content_demo_read',arguments:{query:'b'},reason:'b'},
  ]}),/requestId 重复/);
});
...
test('协议允许省略过程说明字段，并在校验前补齐默认值',()=>{
  const envelope=validateAgentEnvelope({type:'tool_requests',requests:[{requestId:'tr_optional_meta',capability:'cap_content_demo_read',arguments:{query:'hello'}}]});
...
test('工具目录取入口、技能授权、启用实现和只读风险的交集',()=>{
  ...
  const catalog=buildConversationToolCatalog({registry:tools,entryCapabilities:['cap_content_demo_read','cap_content_demo_write'],allowedCapabilities:['cap_content_demo_read','cap_content_demo_write']});
  assert.deepEqual(catalog.map((item)=>item.capability),['cap_content_demo_read']);
  assert.equal(catalog[0].inputSchema.required[0],'query');
});

test('Agent 支持单工具结果回送模型后返回 final，并发送统一事件',async()=>{
  const tools=registry(),catalog=buildConversationToolCatalog({registry:tools,entryCapabilities:['cap_content_demo_read']});
  ...
  const result=await runConversationAgent({entryPoint:'editorial',registry:tools,catalog,onEvent:(event)=>events.push(event),modelStep:async({messages})=>{
    calls+=1;if(calls===1)return {type:'tool_requests',assistant_note:'查资料',requests:[{requestId:'tr_one',capability:'cap_content_demo_read',arguments:{query:'hello'},reason:'核对资料'}]};
...
test('Agent 将原生工具调用转换为可关联的 assistant/tool 历史并继续下一轮',async()=>{
  const tools=registry(),catalog=buildConversationToolCatalog({registry:tools,entryCapabilities:['cap_content_demo_read']});let nextHistory;
...
test('Agent 历史上下文超限时保留事实读取和最近审计并压缩旧轮次',()=>{
  const history=[
    ...
    {role:'tool',content:JSON.stringify([{capability:'cap_filesystem_project_read',status:'ok',data:{answer:'事实'.repeat(3000)}}])},
    {role:'assistant',content:'旧补丁 2'},
    {role:'tool',content:JSON.stringify([{capability:'cap_content_social_card_layout_audit',status:'ok',data:{summary:'旧审计'.repeat(1000)}}])},
    {role:'assistant',content:'最新补丁'},
    {role:'tool',content:JSON.stringify([{capability:'cap_content_social_card_layout_audit',status:'ok',data:{summary:'最新审计'.repeat(1000)}}])},
  ];
  ...
  assert.ok(compacted.some((message)=>message.role==='tool'&&message.content.includes('cap_filesystem_project_read')));
  assert.ok(compacted.at(-1).content.includes('最新审计'));
});

test('Agent 同一步并行多个只读工具并拒绝不可见能力',async()=>{
  const tools=registry(),catalog=buildConversationToolCatalog({registry:tools,entryCapabilities:['cap_content_demo_read']});let secondMessages;
  const result=await runConversationAgent({entryPoint:'custom-social',registry:tools,catalog,modelStep:async({step,messages})=>{
    if(step===0)return {type:'tool_requests',assistant_note:'并行',requests:[
      {requestId:'tr_a',capability:'cap_content_demo_read',arguments:{query:'a'},reason:'a'},
      {requestId:'tr_b',capability:'cap_content_demo_write',arguments:{},reason:'b'},
...
test('Agent 阻止相同调用重复执行并在模型步骤、工具数和时间超限时终止',async()=>{
  const tools=registry(),catalog=buildConversationToolCatalog({registry:tools,entryCapabilities:['cap_content_demo_read']});
  const repeated=await runConversationAgent({entryPoint:'independent-writing',registry:tools,catalog,budget:{maxModelSteps:2},modelStep:async()=>({type:'tool_requests',assistant_note:'again',requests:[{requestId:`tr_${Math.random().toString(36).slice(2)}`,capability:'cap_content_demo_read',arguments:{query:'same'},reason:'same'}]})});
...
test('Agent 运行与工具调用持久化关联，且不保存原始参数值',async(t)=>{
  ...
  const tools=registry(),catalog=buildConversationToolCatalog({registry:tools,entryCapabilities:['cap_content_demo_read']});let calls=0;
  const result=await runConversationAgent({entryPoint:'editorial',registry:tools,catalog,store,toolContext:{skillId:'editorial-room-chat',provider:'mock'},modelStep:async()=>++calls===1?{type:'tool_requests',assistant_note:'read',requests:[{requestId:'tr_db',capability:'cap_content_demo_read',arguments:{query:'secret-value'},reason:'read'}]}:{type:'final',assistantReply:'ok',output:{}}});
  ...
  const executions=store.listToolExecutions({capability:'cap_content_demo_read'});assert.equal(executions.length,1);assert.equal(executions[0].skill_id,'editorial-room-chat');
});
