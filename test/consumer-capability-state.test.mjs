...
function makeRoot(t,{consumers,activeConfigs={}}){
  ...
  fs.writeFileSync(path.join(dir,'config','capabilities.json'),JSON.stringify({schemaVersion:1,capabilities:{
    'content.web.search':{name:'网络搜索',description:'检索公开网页。',category:'信息获取'},
    'filesystem.project.read':{name:'本地项目读取',description:'读取本地项目。',category:'本地内容'},
  }}));
...
test('完整链路可用；degraded 适配只降级不阻断',(t)=>{
  const root=makeRoot(t,{consumers:[agentConsumer({dependencies:[
    {capability:'content.web.search',requirement:'optional',failurePolicy:'continue-with-warning',declaration:'optional',adapterStatus:'ready',resourceKinds:[],triggerPolicy:'model-request',authorizationAction:null,resultPolicy:'passthrough',source:'builtin'},
    {capability:'filesystem.project.read',requirement:'optional',failurePolicy:'continue-with-warning',declaration:'optional',adapterStatus:'degraded',resourceKinds:['local-project'],triggerPolicy:'deterministic-first-step',authorizationAction:'local-project-read',resultPolicy:'summary',source:'builtin'},
  ]})]});
  const graph=buildCapabilityGraph({root,tools:[tool('search-a',['content.web.search']),tool('search-b',['content.web.search']),tool('reader',['filesystem.project.read'])]});
  const ready=stateOf(graph,'agent.demo','content.web.search');
  assert.equal(ready.available,true);assert.equal(ready.status,'ready');assert.deepEqual(ready.reasons,[]);
  const degraded=stateOf(graph,'agent.demo','filesystem.project.read');
...
test('NO_ENABLED_IMPLEMENTATION 与 IMPLEMENTATION_UNHEALTHY 分别阻断',(t)=>{
  const root=makeRoot(t,{consumers:[agentConsumer({dependencies:[
    {capability:'content.web.search',requirement:'optional',failurePolicy:'continue-with-warning',declaration:'optional',adapterStatus:'ready',resourceKinds:[],triggerPolicy:'model-request',authorizationAction:null,resultPolicy:'passthrough',source:'builtin'},
    {capability:'filesystem.project.read',requirement:'optional',failurePolicy:'continue-with-warning',declaration:'optional',adapterStatus:'ready',resourceKinds:['local-project'],triggerPolicy:'explicit-resource',authorizationAction:'local-project-read',resultPolicy:'summary',source:'builtin'},
  ]})]});
  const graph=buildCapabilityGraph({root,tools:[tool('disabled-search',['content.web.search'],{enabled:false}),tool('unconfigured-reader',['filesystem.project.read'],{configuration:{fields:[]}})],configurationState:()=>({configured:false})});
  const disabled=stateOf(graph,'agent.demo','content.web.search');
  assert.equal(disabled.available,false);assert.deepEqual(disabled.reasons,['NO_ENABLED_IMPLEMENTATION']);
  const unhealthy=stateOf(graph,'agent.demo','filesystem.project.read');
...
test('SKILL_NOT_ALLOWED 来自运行时技能的活动配置白名单',(t)=>{
  const root=makeRoot(t,{consumers:[agentConsumer({dependencies:[
    {capability:'content.web.search',requirement:'optional',failurePolicy:'continue-with-warning',declaration:'optional',adapterStatus:'ready',resourceKinds:[],triggerPolicy:'model-request',authorizationAction:null,resultPolicy:'passthrough',source:'builtin'},
  ]})],activeConfigs:{'demo-skill':{prompt:'规则',allowedTools:['filesystem.project.read']}}});
  const graph=buildCapabilityGraph({root,tools:[tool('search-a',['content.web.search'])]});
  const state=stateOf(graph,'agent.demo','content.web.search');
...
test('缺口行产出 CONSUMER_NOT_DECLARED 与 ADAPTER_MISSING，原因码按固定优先级排序',(t)=>{
  const root=makeRoot(t,{consumers:[agentConsumer({runtimeSkillIds:['demo-skill'],dependencies:[],gaps:[{capability:'filesystem.project.read',reason:'未完成资源接入'}]})],activeConfigs:{'demo-skill':{prompt:'规则',allowedTools:['content.web.search']}}});
  const graph=buildCapabilityGraph({root,tools:[]});
  const gap=stateOf(graph,'agent.demo','filesystem.project.read');
  ...
  const root2=makeRoot(t,{consumers:[agentConsumer({id:'agent.combo',runtimeSkillIds:['demo-skill'],dependencies:[
    {capability:'content.web.search',requirement:'optional',failurePolicy:'continue-with-warning',declaration:'optional',adapterStatus:'missing',resourceKinds:[],triggerPolicy:'model-request',authorizationAction:null,resultPolicy:'passthrough',source:'builtin'},
  ]})]});
  const combo=stateOf(buildCapabilityGraph({root:root2,tools:[]}),'agent.combo','content.web.search');
...
test('三向反向查询：消费者→能力、能力→消费者、实现→消费者',async (t)=>{
  ...
  const graph=buildCapabilityGraph({root,tools:[tool('search-a',['content.web.search'])]});
  // 消费者 → 能力
  assert.deepEqual(graph.consumerStates.filter((item)=>item.consumerId==='agent.demo').map((item)=>item.capability),['content.web.search']);
  // 能力 → 消费者（状态已回填）
  const entry=graph.capabilities.find((item)=>item.id==='content.web.search').consumers.find((item)=>item.consumerId==='agent.demo');
...
test('真实仓库：三个 Agent 的现状状态与缺口',async ()=>{
  ...
  for(const consumerId of ['agent.custom-social','agent.independent-writing']){
    const passage=stateOf(graph,consumerId,'content.passage.retrieve');
    assert.equal(passage.available,true);assert.equal(passage.adapterStatus,'ready');assert.deepEqual(passage.warnings,[]);
  }
  // 扩展方案阶段 A：custom-social 已接入 filesystem.project.read（explicit-resource），缺口消除
  const projectRead=stateOf(graph,'agent.custom-social','filesystem.project.read');
...
test('只读接口返回消费者清单与详情，且不泄漏本地路径与授权边界',async ()=>{
  ...
  // 扩展方案阶段 A：filesystem.project.read 已接入，详情中应为声明且可用
  const projectRead=detail.data.capabilities.find((item)=>item.capability==='filesystem.project.read');
  assert.equal(projectRead.declared,true);assert.equal(projectRead.adapterStatus,'ready');assert.equal(projectRead.available,true);
  ...
  assert.ok(skill.data.skillAuthorizations[0].editable.includes('content.url.fetch'));
  const skillRow=skill.data.capabilities.find((item)=>item.capability==='content.url.fetch');
  ...
  const render=feature.data.capabilities.find((item)=>item.capability==='diagram.mermaid.render');
