...
test('阶段 A 插件通过白名单加载并暴露稳定能力', async () => {
  ...
  assert.deepEqual(registry.listCapabilities().map((item) => item.capability).sort(), [
    'content.document.search', 'content.news.search', 'content.passage.retrieve', 'content.repository.inspect', 'content.research.search', 'content.social_card.browser_audit', 'content.social_card.layout_audit', 'content.url.fetch', 'content.web.search', 'diagram.echarts.render', 'diagram.mermaid.render', 'filesystem.project.document_write', 'filesystem.project.read', 'image.cdn.upload',
  ]);
});
