...
test('R5.3 HTML、颜色、字体和模型 JSON 使用共享边界实现',()=>{assert.equal(escapeHtml(`<a x='1'>&"`),'&lt;a x=&#39;1&#39;&gt;&amp;&quot;');assert.equal(mixHex('#000000','#FFFFFF',.5),'#808080');assert.ok(colorContrast('#000000','#FFFFFF')>20);assert.match(fontStack('mono'),/ui-monospace/);assert.doesNotMatch(fontStack('sans'),/Microsoft YaHei/);assert.match(fontStack('song'),/SimSun/);assert.match(fontStack('kai'),/STKaiti/);assert.match(fontStack('hei'),/SimHei/);assert.deepEqual(parseJsonText('```json\n{"ok":true}\n```'),{ok:true});});
