...
test('RSSHub 生命周期路径相对项目根目录解析', () => {
  ...
  try {
    ...
    assert.equal(config.rsshub.startScript,path.join(root,'scripts','rsshub-start.ps1'));
    assert.equal(config.rsshub.stopScript,path.join(root,'scripts','rsshub-stop.ps1'));
...
test('RSSHub 启停脚本不依赖 OpenClaw 或机器绝对路径', () => {
  ...
  const start=fs.readFileSync(path.join(projectRoot,'scripts','rsshub-start.ps1'),'utf8');
  const stop=fs.readFileSync(path.join(projectRoot,'scripts','rsshub-stop.ps1'),'utf8');
