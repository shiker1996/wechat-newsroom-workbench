...
test('三个对话 agent 的技能是唯一事实源（代码不再内联 prompt）', () => {
  const cases = [
    ['editorial-room-chat', 'server/features/articles/llm/editorial-room.mjs', ['"briefUpdates"', 'assistantReply', '"forbidden_claims"', '不要再套 output 层'], '你是公众号编辑会主持人'],
    ['tutorial-chat', 'server/features/articles/llm/tutorial-chat.mjs', ['"briefUpdates"', 'assistantReply', '【体验】'], '你是微信公众号自主写作的策划编辑'],
    ['custom-social-chat', 'server/features/social-cards/llm/custom-social-chat.mjs', ['"briefUpdates"', 'assistantReply', '【素材】'], '你是图文策划编辑'],
  ];
