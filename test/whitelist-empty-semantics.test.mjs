...
test('normalizeSkillConfig：无白名单字段 → null（全放行）；显式空数组 → []（全禁）',()=>{
  ...
  const listed=normalizeSkillConfig({prompt:'规则',allowedTools:['cap_content_web_search']});
  assert.deepEqual(listed.allowedTools,['cap_content_web_search']);
});

test('能力图谱：活动配置显式空白名单阻断全部能力；未配置则放行',(t)=>{
  // 显式空数组 → 全部禁止（agent.editorial 已声明 cap_content_web_search）
  const rootBlocked=makeRoot(t,{activeConfigs:{'editorial-room-chat':{prompt:'规则',allowedTools:[]}}});
  const blocked=stateOf(buildCapabilityGraph({root:rootBlocked,tools:[SEARCH_TOOL]}),'cap_content_web_search');
  ...
  const open=stateOf(buildCapabilityGraph({root:rootOpen,tools:[SEARCH_TOOL]}),'cap_content_web_search');
