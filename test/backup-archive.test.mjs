...
test('工作台备份包校验清单、大小与 SHA-256', () => {
  ...
  try{
    ...
    const manifest={schemaVersion:1,createdAt:new Date().toISOString(),appVersion:APP_VERSION,files:[{
...
test('工作台备份包拒绝校验值不一致的内容', () => {
  ...
  try{
    ...
    fs.writeFileSync(manifestPath,JSON.stringify({schemaVersion:1,appVersion:APP_VERSION,files:[{name:'data/workbench.db',size:7,sha256:'0'.repeat(64)}]}));
    const zip=createZip([{name:'manifest.json',path:manifestPath},{name:'data/workbench.db',path:db}]);
