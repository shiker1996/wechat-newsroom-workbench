...
test('阶段 1 内置主题注册中心加载 6 个文章主题、14 个图文主题和 10 个封面主题', () => {
  const registry=getBuiltinThemeRegistry();
  assert.equal(registry.list().length,30);
  ...
  assert.deepEqual(new Set(registry.list({target:'cover'}).map((item)=>item.id)),new Set(['cover-navy-gold','cover-split-navy','cover-editorial-red','cover-forest-cream','cover-graphite-neon','cover-crimson-paper','cover-royal-data','cover-amber-cocoa','cover-violet-night','cover-jade-paper']));
