...
test('回填选择：query 精确等于 topic 时优先精确匹配', () => {
  const attachments = [
    attachment('content.web.search', '别的 query', '2026-08-14T02:00:00Z', { answer: '新但无关' }),
    attachment('content.web.search', '最终主题', '2026-08-14T01:00:00Z', { answer: '旧但精确' }),
  ];
  const selected = selectConversationSearchAttachments(attachments, '最终主题');
  assert.equal(selected.get('content.web.search').data.answer, '旧但精确');
});

test('回填选择：query 与 topic 不一致时回退到同会话最近一次检索结果', () => {
  const attachments = [
    attachment('content.news.search', '对话中的搜索词', '2026-08-14T02:00:00Z', { answer: '最近结果' }),
    attachment('content.news.search', '更早的搜索词', '2026-08-14T01:00:00Z', { answer: '较早结果' }),
  ];
  const selected = selectConversationSearchAttachments(attachments, '最终定稿主题');
  assert.equal(selected.get('content.news.search').data.answer, '最近结果');
  assert.equal(selected.has('content.web.search'), false);
...
test('回填复用后 attachInformationSearch 跳过已回填槽位，不再重复搜索', async (t) => {
  ...
  try {
    ...
    const attachments = [
      attachment('content.web.search', '对话搜索词', '2026-08-14T02:00:00Z', { answer: 'Agent 答案', results: [{ title: '官方', url: 'https://example.com' }] }),
      attachment('content.news.search', '对话新闻词', '2026-08-14T02:00:00Z', { answer: 'Agent 新闻', results: [] }),
    ];
    ...
    for (const [capability, item] of selectConversationSearchAttachments(attachments, fact.topic)) {
      ...
      if (capability === 'content.web.search') fact.web_search = base;
      if (capability === 'content.news.search') fact.news_search = base;
    }
...
test('事实附件按 batch 与 entryPoint 隔离，跨批次不串数据', (t) => {
  ...
  store.saveConversationFactAttachment({ batchId: batchA.id, entryPoint: 'independent-writing', capability: 'content.web.search', fingerprint: 'f1', agentRunId: null, data: { _agentQuery: 'A 的搜索', answer: 'A 结果' } });
  store.saveConversationFactAttachment({ batchId: batchB.id, entryPoint: 'independent-writing', capability: 'content.web.search', fingerprint: 'f2', agentRunId: null, data: { _agentQuery: 'B 的搜索', answer: 'B 结果' } });
  store.saveConversationFactAttachment({ batchId: batchA.id, entryPoint: 'custom-social', capability: 'content.web.search', fingerprint: 'f3', agentRunId: null, data: { _agentQuery: '图文会话搜索', answer: '图文结果' } });
