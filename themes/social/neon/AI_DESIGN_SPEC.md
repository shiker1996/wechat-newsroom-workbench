...
> 主题 ID：neon · 版本：1.0.0 · 目标：Social 图文视觉生成
...
主题配方 decoration=orbit、装饰透明度 0.28 必须落地为可执行的主题装饰实现；本节给出每页至少一个可见装饰元素的实现方式，实际生成与门禁由 AI 视觉技能统一执行。装饰必须使用主题变量，不能只写“可添加装饰”而不落地 CSS。
...
```html
<div class="neon-feat-card">
  <div class="neon-feat-icon">✦</div>
  <div class="neon-feat-title">核心亮点</div>
  <div class="neon-feat-body">这里放事实清单和故事板允许表达的一到两句内容。</div>
</div>
```
