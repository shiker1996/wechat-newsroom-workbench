# 雾桃白桃 · Social AI Design Spec

> 主题 ID：peach · 版本：1.1.0 · 目标：Social 图文视觉生成

## 主题定位

粉桃柔焦、细下划线和紧凑信息卡构成的轻盈主题。
...
|---|---|---|
| 画布背景 | #FFE8EE | var(--bg) |
| 卡片表面 | #FFFafb | var(--surface) |
| 页面底色 | #FFF5F7 | var(--page) |
| 正文/标题 | #5C3A4A | var(--ink) |
| 辅助文字 | #A06B7D | var(--muted) |
| 主强调色 | #C7416F | var(--accent) |
| 次强调色 | #FFB86C | var(--accent2) |
| 分隔线 | #F2BFD0 | var(--line) |
| 深色表面反色文字 | #FFFFFF | var(--inverse) |
| 代码/深色表面 | #5C3A4A | var(--code) |
...
- 字体：正文 sans；标题 sans。
- 字号档位：标题约 32px；小标题约 13px；正文参考 11px；辅助文字参考 10px。最小字号和最终范围以 `layout-guide.md` 与 AI 视觉技能为准。
- 行高：1.5；字距：0em。
...
```json
{
  ...
  "step": {
    "titleColorRole": "text",
    "bodyColorRole": "muted",
    "markerSurfaceRole": "accentSecondary"
  },
  "compareTable": {
    "headerTextColorRole": "inverseText",
    "headerSurfaceRole": "accent",
    "bodyTextColorRole": "text",
...
主题配方 decoration=soft-blur、装饰透明度 0.3 必须落地为可执行的主题装饰实现；本节给出每页至少一个可见装饰元素的实现方式，实际生成与门禁由 AI 视觉技能统一执行。装饰必须使用主题变量，不能只写“可添加装饰”而不落地 CSS。

使用低透明度的模糊渐变光斑作为氛围层，并保留一个清晰的主题边缘或分隔线；光斑不得承载信息，也不得降低文字对比度。
...
```html
<div class="peach-feat-card">
  <div class="peach-feat-icon">✦</div>
  <div class="peach-feat-title">核心亮点</div>
  <div class="peach-feat-body">这里放事实清单和故事板允许表达的一到两句内容。</div>
</div>
```
