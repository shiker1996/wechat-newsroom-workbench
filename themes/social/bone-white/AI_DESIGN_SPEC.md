...
主题配方 decoration=circle、装饰透明度 0.35 必须落地为可执行的主题装饰实现；本节给出每页至少一个可见装饰元素的实现方式，实际生成与门禁由 AI 视觉技能统一执行。装饰必须使用主题变量，不能只写“可添加装饰”而不落地 CSS。

使用一组低透明度圆形轮廓或圆弧作为背景装饰，并与主题强调色分隔线搭配；圆形只做氛围，不得遮挡内容。
...
```html
<div class="bone-feat-card">
  <div class="bone-feat-icon">✦</div>
  <div class="bone-feat-title">核心亮点</div>
  <div class="bone-feat-body">这里放事实清单和故事板允许表达的一到两句内容。</div>
</div>
```
