# 复古终端 · Social AI Design Spec

> 主题 ID：retro-terminal · 版本：1.0.0 · 目标：Social 图文视觉生成

## 主题定位

黑绿 CRT 扫描线与硬边框的复古终端主题。
...
|---|---|---|
| 画布背景 | #020502 | var(--bg) |
| 卡片表面 | #071507 | var(--surface) |
| 页面底色 | #061006 | var(--page) |
| 正文/标题 | #7DFF8F | var(--ink) |
| 辅助文字 | #38B84B | var(--muted) |
| 主强调色 | #00FF41 | var(--accent) |
| 次强调色 | #B6FF00 | var(--accent2) |
| 分隔线 | #167629 | var(--line) |
| 深色表面反色文字 | #000000 | var(--inverse) |
| 代码/深色表面 | #000000 | var(--code) |
...
- 主题纹理：scanlines；装饰强度只作参考，实际透明度由页面 Agent 按背景和可读性调整。
...
{
  "list": {
    "textColorRole": "inverseText",
    "surfaceRole": "inherit",
    "borderColorRole": "text",
    "borderWeight": "inherit"
  }
}
```
...
| signal-grid | 3–4 个并列问题、能力或选项 | hard-card | 用 surface 与 line 形成可扫描的并列层级 |
...
主题装饰方向：使用原尺寸可辨认的 CRT 横向扫描线或同类屏幕质感。 纹理可使用规律清晰但克制的重复横线。
