# 野兽派 · Social AI Design Spec

> 主题 ID：brutalist · 版本：1.0.0 · 目标：Social 图文视觉生成

## 主题定位

粗黑边框、硬阴影和原色块构成的野兽派主题。
...
|---|---|---|
| 画布背景 | #D8D2BF | var(--bg) |
| 卡片表面 | #FFF9DF | var(--surface) |
| 页面底色 | #F4EDCE | var(--page) |
| 正文/标题 | #111111 | var(--ink) |
| 辅助文字 | #3E392E | var(--muted) |
| 主强调色 | #F04B32 | var(--accent) |
| 次强调色 | #F2C94C | var(--accent2) |
| 分隔线 | #111111 | var(--line) |
| 深色表面反色文字 | #FFFFFF | var(--inverse) |
| 代码/深色表面 | #111111 | var(--code) |

正文使用 var(--ink)，辅助信息使用 var(--muted)；深色表面使用匹配的反色文字，强调色先检查实际背景对比度。

### 字体

- 字体：正文 serif；标题 serif。
- 字号、行高、字距、间距、安全区和内容占用统一遵循 AI 视觉技能提供的 Layout Guide，本主题不另设数值。

### 形状与效果

- 圆角：18px；边框：4px；阴影：hard。
- 可选主题视觉词汇（不要逐页机械复用）：{
  "surface": "brutalist",
  "frame": "brutalist-frame",
  "decoration": "orbit",
  "eyebrow": "stamp",
  "coverTitle": "poster",
...
|---|---|---|---|
| metric-focus | 数字、比例、价格、规模或版本 | statement | 用 accent 或 accent2 放大事实；使用 hard 投影，不新增数字 |
...
| warning-panel | 风险、限制、未验证项或证据边界 | brutalist-frame | 用 accent2、边框或状态底色提高边界信息辨识度 |
| terminal-panel | 代码、命令或安装指令 | hard-panel | 用 code 表面、mono 字体和主题边框保持操作感 |
| accent-fill | 结论、总结、适用人群或下一步 | hard-fill | 用 accent 与 inverse 形成结论色块，不扩展事实 |
